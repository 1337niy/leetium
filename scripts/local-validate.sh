...
macos_app_cmd="${LOCAL_VALIDATE_MACOS_APP_CMD:-./scripts/build-swift-bridge.sh && ./scripts/generate-swift-project.sh && ./scripts/lint-swift.sh && xcodebuild -project apps/macos/Leetium.xcodeproj -scheme Leetium -configuration Release -destination \"platform=macOS\" -derivedDataPath apps/macos/.derivedData-local-validate build}"
ios_app_cmd="${LOCAL_VALIDATE_IOS_APP_CMD:-cargo run -p leetium-schema-export -- apps/ios/GraphQL/Schema/schema.graphqls && ./scripts/generate-ios-graphql.sh && ./scripts/generate-ios-project.sh && xcodebuild -project apps/ios/Leetium.xcodeproj -scheme Leetium -configuration Debug -destination \"generic/platform=iOS\" CODE_SIGNING_ALLOWED=NO build}"
...
cleanup_e2e_ports() {
  ...
  local port
  for port in "${LEETIUM_E2E_PORT:-18789}" "${LEETIUM_E2E_ONBOARDING_PORT:-18790}"; do
    local pids
