...
SWIFT_BRIDGE_TARGETS_CSV="${LEETIUM_SWIFT_BRIDGE_TARGETS:-x86_64-apple-darwin,aarch64-apple-darwin}"
SKIP_WASM_PRECOMPILE="${LEETIUM_SWIFT_BRIDGE_SKIP_WASM_PRECOMPILE:-0}"
...
for raw_target in "${RAW_TARGETS[@]}"; do
  ...
  case "${target}" in
    x86_64-apple-darwin|aarch64-apple-darwin)
      ...
      ;;
    *)
      echo "error: unsupported target '${target}' in LEETIUM_SWIFT_BRIDGE_TARGETS" >&2
      exit 1
      ;;
  esac
done
...
if [ "${#TARGETS[@]}" -eq 0 ]; then
  echo "error: no valid bridge targets configured in LEETIUM_SWIFT_BRIDGE_TARGETS" >&2
  exit 1
fi
...
if [ "${SKIP_WASM_PRECOMPILE}" = "1" ]; then
  echo "Skipping wasm precompile (LEETIUM_SWIFT_BRIDGE_SKIP_WASM_PRECOMPILE=1)"
else
...
for target in "${TARGETS[@]}"; do
  cargo build -p leetium-swift-bridge --release --target "${target}"
done
...
if [ "${#TARGETS[@]}" -eq 1 ]; then
  cp \
    "${REPO_ROOT}/target/${TARGETS[0]}/release/libleetium_swift_bridge.a" \
    "${UNIVERSAL_DIR}/libleetium_bridge.a"
else
  LIPO_INPUTS=()
  for target in "${TARGETS[@]}"; do
    LIPO_INPUTS+=("${REPO_ROOT}/target/${target}/release/libleetium_swift_bridge.a")
  done
  lipo -create "${LIPO_INPUTS[@]}" -output "${UNIVERSAL_DIR}/libleetium_bridge.a"
fi
