# Windows Leetium App (WinUI 3 + Rust FFI) Plan

## Summary

Build a native Windows app equivalent to `apps/macos` using WinUI 3 + C#, reusing the existing Rust C ABI bridge (`leetium_*`) and shipping MSIX as the primary artifact.
...
- Add `crates/windows-bridge` as `cdylib` exporting the same `leetium_*` ABI.
- Add `apps/windows` WinUI app with:
  - `LeetiumNative` P/Invoke bindings
  - `LeetiumClient` JSON facade
...
5. Scaffold `apps/windows` solution/project and bundle `leetium_bridge.dll`.
