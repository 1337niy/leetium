...
Introduce a storage abstraction for configuration and credentials so Leetium can:
...
- Backward-compatible bootstrap from `leetium.toml` and existing JSON files.
- Tests and migration tooling.

Out of scope (for this branch):

- Full PostgreSQL implementation.
- Removal of `leetium.toml` support.
...
- `load_effective() -> LeetiumConfig` (resolved view)
- `load_raw() -> LeetiumConfig` (without env overrides)
- `save_raw(&LeetiumConfig)`
- `update_raw(fn(&mut LeetiumConfig))`
...
- `config_documents`
  - `id` (singleton key)
  - `format_version`
  - `payload_json` (serialized `LeetiumConfig`)
  - `updated_at`
...
- Do we keep full `LeetiumConfig` blob storage long-term, or normalize early?
