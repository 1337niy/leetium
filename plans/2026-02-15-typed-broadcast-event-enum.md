...
- `Channel` — `ChannelEvent` is in `leetium_channels` crate, already serialized
- `UpdateAvailable` — opaque update check result
- `LogsEntry` — opaque log entry
- `CronJob*` — cron job objects from `leetium_cron`
- `McpStatus` — array of server status objects from `leetium_mcp`
...
cargo check -p leetium-gateway

# Run gateway unit tests
cargo test -p leetium-gateway

# Run session-specific tests
cargo test -p leetium-gateway -- session
