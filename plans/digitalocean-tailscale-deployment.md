...
Provide a reliable way to run Leetium on DigitalOcean with secure remote access
...
- Tag nodes for policy-based ACL (`tag:leetium`)
- Expose Leetium to tailnet (`--advertise-tags` and optional funnel/serve rules)
...
- Start both `tailscaled` and Leetium (entrypoint supervisor script)
...
- Create production `docker-compose.yml` with Leetium + Tailscale sidecar
