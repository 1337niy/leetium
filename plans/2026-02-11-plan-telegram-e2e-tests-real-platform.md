...
1. **The leetium bot** — a real Telegram bot running inside the gateway
...
leetium, and the test user asserts on the response.
...
Helper to start a real leetium gateway with Telegram configured:
...
"#, env::var("TELEGRAM_BOT_TOKEN").unwrap());
    ...
    fs::write(config_dir.path().join("leetium.toml"), config).unwrap();
    ...
    let handle = tokio::spawn(async move {
        leetium_gateway::run(GatewayConfig {
            port,
  ...
  telegram-e2e:
    ...
    steps:
      ...
      - name: Run Telegram E2E tests
        run: cargo test --package leetium-telegram -- --ignored --test-threads=1
        env:
