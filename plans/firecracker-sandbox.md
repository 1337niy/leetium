...
backend for leetium on Linux. Firecracker provides hypervisor-level isolation
...
ROOTFS_SIZE="512M"
ROOTFS_FILE="leetium-rootfs.ext4"
...
# Copy guest agent
sudo cp "$AGENT_BINARY" "$MOUNT_DIR/usr/local/bin/leetium-agent"
sudo chmod +x "$MOUNT_DIR/usr/local/bin/leetium-agent"

# Create init script to start agent on boot
sudo tee "$MOUNT_DIR/etc/init.d/leetium-agent" > /dev/null <<'EOF'
#!/sbin/openrc-run
command="/usr/local/bin/leetium-agent"
command_background=true
pidfile="/run/leetium-agent.pid"
EOF
sudo chmod +x "$MOUNT_DIR/etc/init.d/leetium-agent"
sudo chroot "$MOUNT_DIR" rc-update add leetium-agent default
...
impl FirecrackerSandbox {
    ...
    fn socket_dir(&self) -> PathBuf {
        std::env::temp_dir().join("leetium-firecracker")
    }
...
fn select_backend(config: SandboxConfig) -> Arc<dyn Sandbox> {
    match config.backend.as_str() {
        ...
        "firecracker" => {
            ...
            let kernel = PathBuf::from("/var/lib/leetium/vmlinux");
            let rootfs = PathBuf::from("/var/lib/leetium/rootfs.ext4");
...
kernel = "/var/lib/leetium/vmlinux"
rootfs = "/var/lib/leetium/rootfs.ext4"
```
...
- leetium guest agent running on boot
...
# Run leetium with firecracker backend
LEETIUM_SANDBOX_BACKEND=firecracker leetium serve
```
