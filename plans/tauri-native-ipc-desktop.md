...
- Add a new crate: `crates/desktop`
  ...
  - Depends on existing workspace crates (`leetium-gateway`, `leetium-config`, etc.)
...
- `cargo build -p leetium-desktop` works on macOS.
- Existing `leetium` CLI build/test paths are unchanged.
...
# Rust compile checks
cargo check
cargo build -p leetium

# Desktop crate compile (macOS)
cargo build -p leetium-desktop
