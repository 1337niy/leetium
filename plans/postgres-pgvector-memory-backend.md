...
memory backend. The main database (`leetium.db`) stays SQLite — only the memory
...
memory-sqlite = ["leetium-memory/sqlite"]
memory-postgres = ["leetium-memory/postgres"]
```
...
database_url = "postgresql://user:pass@localhost/leetium"  # required for postgres
```
...
- pgvector `vector(N)` requires a fixed dimension. Store dimension in config
  ...
  explicit re-index (`leetium memory reindex`).
- HNSW index creation:
...
- On subsequent syncs, if the provider dimension changes, warn and require
  explicit re-index via `leetium memory reindex`
...
- Migrating main DB (`leetium.db`) to Postgres
