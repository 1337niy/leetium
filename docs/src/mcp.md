...
Leetium supports the [Model Context Protocol (MCP)](https://modelcontextprotocol.io) for connecting to external tool servers. MCP servers extend your agent's capabilities without modifying Leetium itself.
...
Leetium monitors MCP servers and automatically:
...
MCP server stderr is captured in Leetium logs:

```bash
# View gateway logs
tail -f ~/.leetium/logs.jsonl | grep -i mcp
```
...
Remote MCP servers can require OAuth 2.1 authentication. Leetium handles this automatically — when a server returns `401 Unauthorized`, the OAuth flow starts without any manual configuration.

### How It Works

1. Leetium connects to the remote MCP server
2. The server returns `401 Unauthorized` with a `WWW-Authenticate` header
3. Leetium discovers the authorization server via [RFC 9728](https://www.rfc-editor.org/rfc/rfc9728) (Protected Resource Metadata)
4. Leetium performs [dynamic client registration](https://www.rfc-editor.org/rfc/rfc7591) (RFC 7591)
...
If your session expires or tokens are revoked, Leetium automatically re-authenticates on the next `401` response. You can also trigger re-authentication manually via the `mcp.reauth` RPC method.

## Security Considerations

```admonish warning
MCP servers run with the same permissions as Leetium. Only use servers from trusted sources.
```
