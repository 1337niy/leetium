...
Leetium can read and manage remote calendars through the
...
Add a `[caldav]` section to your `leetium.toml` (usually `~/.leetium/leetium.toml`):
...
Passwords are stored in `leetium.toml` and redacted in logs, but the file itself
is plain text on disk. Consider using [Vault](vault.md) for encryption at rest.
```

## How it works

When Leetium starts and CalDAV is enabled with at least one account, a `caldav`
...
`leetium config check` validates CalDAV configuration and warns about unknown
