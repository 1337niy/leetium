...
Leetium supports WhatsApp as a messaging channel using the WhatsApp Linked Devices
...
```
┌────────────────┐   QR pair   ┌─────────────────┐   Signal   ┌──────────────┐
│  Your Phone    │ ──────────► │  Leetium Gateway  │ ◄────────► │  WhatsApp    │
...
1. Leetium registers as a **linked device** on your WhatsApp account
2. Messages sent to your WhatsApp number arrive at both your phone and Leetium
3. Leetium processes inbound messages through the configured LLM
...
yourself via WhatsApp's "Message Yourself" feature. Leetium automatically
detects self-chat and prevents reply loops. Convenient for personal use.
Note that Leetium (as a linked device) sees all your incoming messages —
...
[features]
default = ["whatsapp", ...]
whatsapp = ["leetium-gateway/whatsapp"]
```
...
That's it — messages to your WhatsApp account are now processed by Leetium.
...
You can also configure WhatsApp accounts in `leetium.toml`. This is useful for
automated deployments or when you want to pre-configure settings before pairing.

```toml
# ~/.leetium/leetium.toml
...
Start Leetium and the account will begin the pairing process. The QR code is
...
| `store_path` | string | — | Custom path to sled store; defaults to `~/.leetium/whatsapp/<account_id>/` |
...
When Leetium is linked to your personal WhatsApp, it sees **every** incoming
...
**Other people's messages follow `dm_policy`.** If you want Leetium to only
...
```admonish warning
Do not delete the sled store directory while Leetium is running. If you need
to re-pair, stop Leetium first, then delete the directory and restart.
```

## Self-Chat

Leetium automatically supports WhatsApp's "Message Yourself" feature. When you
...
an incoming message (since it's your own chat). Leetium uses two mechanisms to
...
WhatsApp supports rich media messages. Leetium handles each type:
...
- Verify the sled store directory is writable: `~/.leetium/whatsapp/`

### "Logged Out" After Restart

- This usually means the sled store was corrupted or deleted
- Check that `~/.leetium/whatsapp/<account_id>/` exists and has data files
