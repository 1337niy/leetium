...
Leetium includes a built-in cron system that lets the agent schedule and manage
...
other parts of Leetium trigger an immediate heartbeat when something noteworthy
...
Leetium maintains an in-memory bounded queue of **system events** — short text
...
When a background command finishes (via the `exec` tool), Leetium automatically
