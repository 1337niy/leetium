...
Loaded from `~/.leetium/IDENTITY.md` using YAML frontmatter. Fields:
...
Loaded from `~/.leetium/SOUL.md`. When the file is absent or empty, the
...
Loaded from `~/.leetium/USER.md` using YAML frontmatter.
...
Resolved by `leetium_projects::context::load_context_files()`. The loader walks
...
```
Host: host=leetium-devbox | os=macos | arch=aarch64 | shell=zsh | time=2026-02-17 16:18:00 CET | today=2026-02-17 | provider=openai | model=gpt-5 | session=main | sudo_non_interactive=true | timezone=Europe/Paris
Sandbox(exec): enabled=true | mode=all | backend=docker | scope=session | image=leetium-sandbox:abc123 | workspace_mount=ro | network=disabled
```
...
`leetium_skills::prompt_gen::generate_skills_prompt()`:
...
Optional markdown files from the data directory (`~/.leetium/`):
