...
Leetium includes comprehensive observability support through Prometheus metrics and
...
leetium-gateway = { version = "0.1", features = ["metrics"] }

# Enable metrics with Prometheus export (default)
leetium-gateway = { version = "0.1", features = ["metrics", "prometheus"] }

# Enable metrics for specific crates
leetium-agents = { version = "0.1", features = ["metrics"] }
leetium-cron = { version = "0.1", features = ["metrics"] }
```
...
```
# HELP leetium_http_requests_total Total number of HTTP requests handled
# TYPE leetium_http_requests_total counter
leetium_http_requests_total{method="GET",status="200",endpoint="/api/chat"} 42

# HELP leetium_llm_completion_duration_seconds Duration of LLM completion requests
# TYPE leetium_llm_completion_duration_seconds histogram
leetium_llm_completion_duration_seconds_bucket{provider="anthropic",model="claude-3-opus",le="1.0"} 5
```
...
1. Add leetium to your `prometheus.yml`:
...
2. Import or create Grafana dashboards using the `leetium_*` metrics.
...
restarts. The database is stored at `~/.leetium/metrics.db` (or the configured
data directory).
...
|--------|------|--------|-------------|
| `leetium_http_requests_total` | Counter | method, status, endpoint | Total HTTP requests |
| `leetium_http_request_duration_seconds` | Histogram | method, status, endpoint | Request latency |
| `leetium_http_requests_in_flight` | Gauge | — | Currently processing requests |

### LLM/Agent Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `leetium_llm_completions_total` | Counter | provider, model | Total completions requested |
| `leetium_llm_completion_duration_seconds` | Histogram | provider, model | Completion latency |
| `leetium_llm_input_tokens_total` | Counter | provider, model | Input tokens processed |
| `leetium_llm_output_tokens_total` | Counter | provider, model | Output tokens generated |
| `leetium_llm_completion_errors_total` | Counter | provider, model, error_type | Completion failures |
| `leetium_llm_time_to_first_token_seconds` | Histogram | provider, model | Streaming TTFT |
...
```
leetium_llm_input_tokens_total{provider="anthropic-work", model="claude-3-opus"} 5000
leetium_llm_input_tokens_total{provider="anthropic-personal", model="claude-3-opus"} 3000
```
...
| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `leetium_mcp_tool_calls_total` | Counter | server, tool | Tool invocations |
| `leetium_mcp_tool_call_duration_seconds` | Histogram | server, tool | Tool call latency |
| `leetium_mcp_tool_call_errors_total` | Counter | server, tool, error_type | Tool call failures |
| `leetium_mcp_servers_connected` | Gauge | — | Active MCP server connections |

### Tool Execution Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `leetium_tool_executions_total` | Counter | tool | Tool executions |
| `leetium_tool_execution_duration_seconds` | Histogram | tool | Execution time |
| `leetium_sandbox_command_executions_total` | Counter | — | Sandbox commands run |

### Session Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `leetium_sessions_created_total` | Counter | — | Sessions created |
| `leetium_sessions_active` | Gauge | — | Currently active sessions |
| `leetium_session_messages_total` | Counter | role | Messages by role |

### Cron Job Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `leetium_cron_jobs_scheduled` | Gauge | — | Number of scheduled jobs |
| `leetium_cron_executions_total` | Counter | — | Job executions |
| `leetium_cron_execution_duration_seconds` | Histogram | — | Job duration |
| `leetium_cron_errors_total` | Counter | — | Failed jobs |
| `leetium_cron_stuck_jobs_cleared_total` | Counter | — | Jobs exceeding 2h timeout |
| `leetium_cron_input_tokens_total` | Counter | — | Input tokens from cron runs |
| `leetium_cron_output_tokens_total` | Counter | — | Output tokens from cron runs |

### Memory/Search Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `leetium_memory_searches_total` | Counter | search_type | Searches performed |
| `leetium_memory_search_duration_seconds` | Histogram | search_type | Search latency |
| `leetium_memory_embeddings_generated_total` | Counter | provider | Embeddings created |

### Channel Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `leetium_channels_active` | Gauge | — | Loaded channel plugins |
| `leetium_channel_messages_received_total` | Counter | channel | Inbound messages |
| `leetium_channel_messages_sent_total` | Counter | channel | Outbound messages |

### Telegram-Specific Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `leetium_telegram_messages_received_total` | Counter | — | Messages from Telegram |
| `leetium_telegram_access_control_denials_total` | Counter | — | Access denied events |
| `leetium_telegram_polling_duration_seconds` | Histogram | — | Message handling time |

### OAuth Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `leetium_oauth_flow_starts_total` | Counter | — | OAuth flows initiated |
| `leetium_oauth_flow_completions_total` | Counter | — | Successful completions |
| `leetium_oauth_token_refresh_total` | Counter | — | Token refreshes |
| `leetium_oauth_token_refresh_failures_total` | Counter | — | Refresh failures |

### Skills Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `leetium_skills_installation_attempts_total` | Counter | — | Installation attempts |
| `leetium_skills_installation_duration_seconds` | Histogram | — | Installation time |
| `leetium_skills_git_clone_total` | Counter | — | Successful git clones |
| `leetium_skills_git_clone_fallback_total` | Counter | — | Fallbacks to HTTP tarball |

## Tracing Integration

The `leetium-metrics` crate includes optional tracing integration via the
`tracing` feature. This allows span context to propagate to metric labels.

### Enabling Tracing

```toml
leetium-metrics = { version = "0.1", features = ["prometheus", "tracing"] }
```

### Initialization

```rust
use leetium_metrics::tracing_integration::init_tracing;
...
Use the `metrics` macros re-exported from `leetium-metrics`:

```rust
use leetium_metrics::{counter, gauge, histogram, labels};
...
```rust
#[cfg(feature = "metrics")]
use leetium_metrics::{counter, histogram};
...
pub mod my_feature {
    ...
    pub const OPERATIONS_TOTAL: &str = "leetium_my_feature_operations_total";
    /// Operation duration in seconds
    pub const OPERATION_DURATION_SECONDS: &str = "leetium_my_feature_operation_duration_seconds";
}
```
...
```rust
use leetium_metrics::{counter, my_feature};
...
Metrics configuration in `leetium.toml`:
...
- `RUST_LOG=leetium_metrics=debug` — Enable debug logging for metrics initialization

## Best Practices

1. **Use consistent naming**: Follow the pattern `leetium_<subsystem>_<metric>_<unit>`
...
4. Check `leetium.toml` has `[metrics] enabled = true`
