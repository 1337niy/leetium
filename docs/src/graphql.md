...
Leetium exposes a GraphQL API that mirrors gateway RPC methods with typed query
...
If Leetium is built without this feature, `/graphql` is not registered.
...
- Valid session cookie (`leetium_session`)
- `Authorization: Bearer <api_key>`

```admonish warning
Do not expose Leetium to untrusted networks with authentication disabled.
```
