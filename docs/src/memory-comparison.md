# Memory: Leetium vs OpenClaw

This page provides a detailed comparison of the memory systems in Leetium and
...
| Aspect | Leetium | OpenClaw |
|--------|--------|----------|
| **Data directory** | `~/.leetium/` (configurable) | `~/.openclaw/workspace/` |
...
**Key difference:** Leetium uses a purpose-built `memory_save` tool with
...
| Setting | Leetium (`leetium.toml`) | OpenClaw (`openclaw.json`) |
|---------|------------------------|---------------------------|
...
| **Memory crate/module** | `leetium-memory` crate | `memory-core` plugin |
...
## What Leetium Has That OpenClaw Does Not
...
## What OpenClaw Has That Leetium Does Not (Yet)
...
1. **Tool approach**: Leetium provides a purpose-built `memory_save` tool with
   security validation; OpenClaw uses a general-purpose `write_file` tool
   guided by the system prompt.

2. **Write safety**: Leetium validates write paths at the tool level (allowlist
   + traversal checks); OpenClaw relies on workspace-level access control.

3. **Implementation**: Leetium is pure Rust with a `MemoryWriter` trait
...
4. **Maturity**: OpenClaw has more CLI tooling and configuration knobs for
   advanced memory management; Leetium has a simpler, more opinionated setup
   with a web UI.
