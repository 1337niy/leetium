# Native Swift App with Embedded Leetium Rust Core (POC)

This guide shows a **proof-of-concept path** to build a native Swift app where Swift is the UI layer and Leetium Rust code is embedded as a local library.
...
Rust core facade (thin FFI-safe layer)
        |
        v
Existing Leetium crates (chat/providers/config/etc.)
```
...
- `leetium_version()`
- `leetium_chat_json(request_json)`
- `leetium_free_string(ptr)`
- `leetium_shutdown()`
...
3. Link `Generated/libleetium_bridge.a` and include `Generated/leetium_bridge.h`.
4. Use a Swift facade (`LeetiumClient`) to own pointer and lifetime rules.
...
## Intel + Apple Silicon (Universal `libleetium`)

Yes — you can build `libleetium` for both Intel and Apple Silicon and merge them into one universal macOS static library.
...
lipo -create \
  target/x86_64-apple-darwin/release/libleetium_bridge.a \
  target/aarch64-apple-darwin/release/libleetium_bridge.a \
  -output target/universal-macos/release/libleetium_bridge.a

lipo -info target/universal-macos/release/libleetium_bridge.a
```

This universal `libleetium_bridge.a` can then be linked by your Swift macOS app, so one app build supports both Intel and M-series Macs.
...
Leetium is async-first. For a POC:
...
1. Add `swift-bridge` crate exposing one health function (`leetium_version`).
2. Add one end-to-end chat method (`leetium_chat_json`).
...
## Why This Fits Leetium

Leetium already has clear crate boundaries and async services. A thin FFI facade lets Swift own the native UX while reusing provider orchestration, config, and session logic from Rust.
