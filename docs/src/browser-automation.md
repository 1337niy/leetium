...
Leetium provides full browser automation via Chrome DevTools Protocol (CDP),
...
Browser automation is **enabled by default**. To customize, add to your `leetium.toml`:
...
# Container connectivity (for Leetium-in-Docker setups)
# container_host = "127.0.0.1"  # Default; change when Leetium runs inside Docker
```
...
installed, Leetium will attempt a best-effort auto-install, then retry
detection.
...
| Metric | Description |
|--------|-------------|
| `leetium_browser_instances_active` | Currently running browsers |
| `leetium_browser_instances_created_total` | Total browsers launched |
| `leetium_browser_instances_destroyed_total` | Total browsers closed |
| `leetium_browser_screenshots_total` | Screenshots taken |
| `leetium_browser_navigation_duration_seconds` | Page load time histogram |
| `leetium_browser_errors_total` | Errors by type |
...
### Leetium Inside Docker (Sibling Containers)

When Leetium itself runs inside a Docker container, the browser container is
launched as a sibling via the host's Docker socket. By default Leetium connects
to the browser at `127.0.0.1`, which points to the Leetium container's own
loopback — not the host where the browser port is mapped.

Set `container_host` so Leetium can reach the browser container through the
host's port mapping:
...
to the Leetium container's `docker run` command.
...
Leetium automatically detects installed Chromium-based browsers in the following order:
...
If no browser is found, Leetium displays platform-specific installation instructions.
...
tool calls with missing required fields. Leetium handles this gracefully:
