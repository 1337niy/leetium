...
Leetium provides text-to-speech (TTS) and speech-to-text (STT) capabilities
through the `leetium-voice` crate and gateway integration.
...
Leetium supports multiple TTS providers across cloud and local backends.
...
model_path = "~/.leetium/models/en_US-lessac-medium.onnx"  # required
# config_path = "~/.leetium/models/en_US-lessac-medium.onnx.json"  # optional
...
2. Download a voice model from [Piper Voices](https://github.com/rhasspy/piper#voices):
   ```bash
   mkdir -p ~/.leetium/models
   curl -L -o ~/.leetium/models/en_US-lessac-medium.onnx \
     https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
   ```

3. Configure in `leetium.toml`:
   ```toml
   [voice.tts]
   provider = "piper"

   [voice.tts.piper]
   model_path = "~/.leetium/models/en_US-lessac-medium.onnx"
   ```
...
Leetium supports multiple STT providers across cloud and local backends.
...
model_path = "~/.leetium/models/ggml-base.en.bin"  # required
language = "en"

[voice.stt.sherpa_onnx]
# binary_path = "/usr/local/bin/sherpa-onnx-offline"  # optional
model_dir = "~/.leetium/models/sherpa-onnx-whisper-tiny.en"  # required
language = "en"
```
...
2. Download a model from [Hugging Face](https://huggingface.co/ggerganov/whisper.cpp):
   ```bash
   mkdir -p ~/.leetium/models
   curl -L -o ~/.leetium/models/ggml-base.en.bin \
...
3. Configure in `leetium.toml`:
   ...
   [voice.stt.whisper_cli]
   model_path = "~/.leetium/models/ggml-base.en.bin"
   ```
...
3. Configure in `leetium.toml`:
   ...
   [voice.stt.sherpa_onnx]
   model_dir = "~/.leetium/models/sherpa-onnx-whisper-tiny.en"
   ```
