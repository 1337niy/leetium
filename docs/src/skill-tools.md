...
Leetium can create, update, and delete skills at runtime through agent tools,
...
| Tool | Description |
|------|-------------|
| `create_skill` | Write a new `SKILL.md` to `.leetium/skills/<name>/` |
...
`.leetium/skills/` folder. They become available on the next message
...
This writes `.leetium/skills/summarize-pr/SKILL.md` with the provided content.
...
This removes the entire `.leetium/skills/summarize-pr/` directory.
