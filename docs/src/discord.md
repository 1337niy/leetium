...
Leetium can connect to Discord as a bot, letting you chat with your agent from
...
Before configuring Leetium, create a Discord bot:
...
version control. Leetium stores it with `secrecy::Secret` and redacts it from
logs, but your `leetium.toml` file is plain text on disk. Consider using
[Vault](vault.md) for encryption at rest.
```

## Configuration

Add a `[channels.discord.<account-id>]` section to your `leetium.toml`:
...
3. The code appears in the Leetium web UI under **Channels → Senders**
...
```admonish tip
The Leetium web UI generates this invite link automatically when you paste your
...
start chatting — just connect the bot in Leetium and message it directly.
...
Discord enforces a **2,000-character limit** per message. Leetium automatically
...
The crate implements the same trait set as `leetium-telegram` and `leetium-msteams`:
...
- Look at logs: `RUST_LOG=leetium_discord=debug leetium`
