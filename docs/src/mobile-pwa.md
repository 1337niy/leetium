...
Leetium can be installed as a Progressive Web App (PWA) on mobile devices, providing a native app-like experience with push notifications.

## Installing on Mobile

### iOS (Safari)

1. Open leetium in Safari
2. Tap the Share button (box with arrow)
3. Scroll down and tap "Add to Home Screen"
4. Tap "Add" to confirm

The app will appear on your home screen with the leetium icon.

### Android (Chrome)

1. Open leetium in Chrome
...
When installed as a PWA, leetium provides:
...
1. Open the leetium app (must be installed as PWA on Safari/iOS)
2. Go to **Settings > Notifications**
3. Click **Enable** to subscribe to push notifications
4. When prompted, allow notification permissions

**Safari/iOS Note**: Push notifications only work when the app is installed as a PWA. If you see "Installation required", add leetium to your Dock first:
...
Leetium uses the Web Push API with VAPID (Voluntary Application Server Identification) keys:
...
[dependencies]
leetium-gateway = { default-features = false, features = ["web-ui", "tls"] }
```
...
3. Click "Unregister" on the leetium service worker
4. Reload the page
