...
Leetium runs LLM-generated commands inside containers to protect your host
system. The sandbox backend controls which container technology is used.

## Backend Selection

Configure in `leetium.toml`:
...
With `"auto"` (the default), Leetium picks the strongest available backend:
...
Once installed, restart `leetium gateway` — the startup banner will show
...
Docker and Podman containers launched by Leetium include the following security
...
`/home/sandbox/file.txt`) to host paths under `~/.leetium/sandbox/wasm/<id>/`.
...
```
~/.leetium/sandbox/wasm/<session-key>/
...
When the feature is disabled and the config requests `backend = "wasm"`, Leetium
...
- No image building — `leetium sandbox build` returns immediately
...
Leetium wraps the primary sandbox backend with automatic failover:

- **Apple Container → Docker → Restricted Host**: if Apple Container enters a
  corrupted state (stale metadata, missing config, VM boot failure), Leetium
...
- **Docker → Restricted Host**: if Docker loses its daemon connection during a
  session, Leetium fails over to the restricted-host sandbox.
...
Leetium stores persisted homes under `data_dir()/sandbox/home/`.
