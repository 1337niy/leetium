...
final class AuthManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: "org.leetium.ios", category: "auth")
    ...
    private func createApiKey(baseURL: URL, sessionCookie: String) async throws -> String {
        ...
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("leetium_session=\(sessionCookie)", forHTTPHeaderField: "Cookie")
        request.timeoutInterval = 10

        let body: [String: Any] = [
            "label": "Leetium iOS (\(UIDevice.current.name))",
    ...
    private static func sessionCookieValue(for url: URL, response: HTTPURLResponse) -> String? {
        ...
        if let sessionCookie = cookies.first(where: { $0.name == "leetium_session" }) {
            return sessionCookie.value
        }

        if let setCookie = response.value(forHTTPHeaderField: "Set-Cookie"),
           let range = setCookie.range(of: "leetium_session=")
        {
