                ...
                if !bonjourBrowser.servers.isEmpty {
                    ...
                    if needsCertificateTrustHelp {
                        Section("Trust This Certificate (iOS)") {
                            ...
                            Text("5. Enable full trust for Leetium Local CA, then return and tap Check Connection.")
                        }
                    }
                }
        ...
        do {
            ...
            if let status = authStatus {
                ...
                if graphQLEnabled == false {
                    showError(
                        message: "GraphQL is disabled on this server. Enable it in Leetium (Settings > GraphQL), then check connection again."
                    )
                } else if status.setupRequired || !status.setupComplete {
                    showError(
                        message: "Server auth is not fully configured for remote access. On the Leetium host, complete authentication setup (password or passkey), then try again from iOS."
                    )
            ...
            if isCertificateTrustError(error) {
                showError(message: "TLS certificate is not trusted yet. Download and trust the Leetium Local CA for this server.")
                await refreshNearbyServerTrustStates(force: true)
                return
            }
