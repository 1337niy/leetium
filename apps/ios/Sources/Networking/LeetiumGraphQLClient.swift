...
actor LeetiumGraphQLClient {
    private let logger = Logger(subsystem: "org.leetium.ios", category: "graphql")
    ...
    func fetchSessions() async throws -> [GQLSession] {
        let data = try await execute(LeetiumAPI.FetchSessionsQuery(), operationName: "FetchSessions")
    ...
    func searchSessions(query searchQuery: String) async throws -> [GQLSession] {
        let data = try await execute(
            LeetiumAPI.SearchSessionsQuery(query: searchQuery),
    ...
    func fetchModels() async throws -> [GQLModel] {
        let data = try await execute(LeetiumAPI.FetchModelsQuery(), operationName: "FetchModels")
    ...
    private func mapSession(_ s: LeetiumAPI.SessionFields) -> GQLSession {
        let key = s.key ?? ""
        return GQLSession(
            id: s.id ?? key,
            key: key,
    ...
    func fetchStatus() async throws -> GQLStatus {
        let data = try await execute(LeetiumAPI.FetchStatusQuery(), operationName: "FetchStatus")
        return GQLStatus(
    ...
    func updateUserLocation(latitude: Double, longitude: Double) async throws -> Bool {
        ...
        let data = try await execute(
            LeetiumAPI.UpdateUserLocationMutation(input: payloadString),
