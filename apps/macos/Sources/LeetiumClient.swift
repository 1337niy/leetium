...
/// Set once during app startup via `LeetiumClient.installLogCallback`.
...
/// Set once during app startup via `LeetiumClient.installSessionEventCallback`.
...
/// Set once during app startup via `LeetiumClient.installNetworkAuditCallback`.
...
enum LeetiumClientError: Error, LocalizedError {
...
struct LeetiumClient {
    ...
    static func installLogCallback(logStore: LogStore) {
        ...
        leetium_set_log_callback(rustLogCallbackHandler)
    }
    ...
    static func installSessionEventCallback(chatStore: ChatStore) {
        ...
        leetium_set_session_event_callback(rustSessionEventCallbackHandler)
    }
    ...
    static func installNetworkAuditCallback(store: NetworkAuditStore) {
        ...
        leetium_set_network_audit_callback(rustNetworkAuditCallbackHandler)
    }
    ...
    func version() throws -> BridgeVersionPayload {
        let payload = try consumeCStringPointer(leetium_version())
        return try decode(payload, as: BridgeVersionPayload.self)
    }

    func getIdentity() throws -> BridgeIdentityPayload {
        let payload = try consumeCStringPointer(leetium_get_identity())
    ...
    func saveProviderConfig(
        ...
        let _: BridgeOkPayload = try callBridge(
            ...
            via: leetium_save_provider_config
        )
    }

    func listModels() throws -> [BridgeModelInfo] {
        let payload = try consumeCStringPointer(leetium_list_models())
        return try decode(payload, as: [BridgeModelInfo].self)
    }

    func refreshRegistry() throws {
        let payload = try consumeCStringPointer(leetium_refresh_registry())
    ...
    func startHttpd(host: String, port: UInt16) throws -> BridgeHttpdStatus {
        try callBridge(
            ...
            via: leetium_start_httpd
        )
    }

    func stopHttpd() throws -> BridgeHttpdStatus {
        let payload = try consumeCStringPointer(leetium_stop_httpd())
        return try decode(payload, as: BridgeHttpdStatus.self)
    }

    func httpdStatus() throws -> BridgeHttpdStatus {
        let payload = try consumeCStringPointer(leetium_httpd_status())
        return try decode(payload, as: BridgeHttpdStatus.self)
    }
    ...
    func abortSession(key: String) throws -> BridgeAbortResult {
        let payload = try key.withCString { ptr in
            try consumeCStringPointer(leetium_abort_session(ptr))
        }
    ...
    func peekSession(key: String) throws -> BridgePeekResult {
        let payload = try key.withCString { ptr in
            try consumeCStringPointer(leetium_peek_session(ptr))
        }
    ...
    func listSessions() throws -> [BridgeSessionEntry] {
        let payload = try consumeCStringPointer(leetium_list_sessions())
    ...
    func sessionChatStream(
        ...
        json.withCString { ptr in
            leetium_session_chat_stream(ptr, streamCallbackHandler, retained)
        }
    }
    ...
    func chatStream(
        ...
        json.withCString { ptr in
            leetium_chat_stream(ptr, streamCallbackHandler, retained)
        }
    }
    ...
    private func callBridge<Request: Encodable, Response: Decodable>(
        ...
        guard let json = String(data: data, encoding: .utf8) else {
            throw LeetiumClientError.jsonEncodingFailed
        }
    ...
    private func decode<T: Decodable>(_ payload: String, as _: T.Type) throws -> T {
        ...
        if let bridgeError = try? decoder.decode(BridgeErrorEnvelope.self, from: data) {
            throw LeetiumClientError.bridgeError(
    ...
    private func consumeCStringPointer(
        ...
        guard let value else {
            throw LeetiumClientError.nilResponsePointer
        }

        defer {
            leetium_free_string(value)
        }
...
extension LeetiumClient {
    ...
    /// to `LeetiumConfig` and writes TOML preserving comments.
    func saveConfig(_ config: [String: Any]) throws {
        ...
        let payload = try json.withCString { ptr in
            try consumeCStringPointer(leetium_save_config(ptr))
        }
        _ = try decode(payload, as: BridgeOkPayload.self)
    }

    func memoryStatus() throws -> BridgeMemoryStatusPayload {
        let payload = try consumeCStringPointer(leetium_memory_status())
        return try decode(payload, as: BridgeMemoryStatusPayload.self)
    }

    func memoryConfigGet() throws -> BridgeMemoryConfigPayload {
        let payload = try consumeCStringPointer(leetium_memory_config_get())
    ...
    func memoryConfigUpdate(
        ...
        return try callBridge(request, via: leetium_memory_config_update)
    }

    func memoryQmdStatus() throws -> BridgeMemoryQmdStatusPayload {
        let payload = try consumeCStringPointer(leetium_memory_qmd_status())
        return try decode(payload, as: BridgeMemoryQmdStatusPayload.self)
    }

    func authStatus() throws -> BridgeAuthStatusPayload {
        let payload = try consumeCStringPointer(leetium_auth_status())
    ...
    func authPasswordChange(
        ...
        return try callBridge(request, via: leetium_auth_password_change)
    }

    func authReset() throws {
        let payload = try consumeCStringPointer(leetium_auth_reset())
        _ = try decode(payload, as: BridgeOkPayload.self)
    }

    func authListPasskeys() throws -> [BridgeAuthPasskeyEntry] {
        let payload = try consumeCStringPointer(leetium_auth_list_passkeys())
    ...
    func authRemovePasskey(id: Int64) throws {
        ...
        let _: BridgeOkPayload = try callBridge(request, via: leetium_auth_remove_passkey)
    }

    func authRenamePasskey(id: Int64, name: String) throws {
        let request = AuthPasskeyRenameRequest(id: id, name: name)
        let _: BridgeOkPayload = try callBridge(request, via: leetium_auth_rename_passkey)
    }

    func sandboxStatus() throws -> BridgeSandboxStatusPayload {
        let payload = try consumeCStringPointer(leetium_sandbox_status())
        return try decode(payload, as: BridgeSandboxStatusPayload.self)
    }

    func sandboxListImages() throws -> [BridgeSandboxImageEntry] {
        let payload = try consumeCStringPointer(leetium_sandbox_list_images())
    ...
    func sandboxDeleteImage(tag: String) throws {
        ...
        let _: BridgeOkPayload = try callBridge(request, via: leetium_sandbox_delete_image)
    }

    func sandboxPruneImages() throws -> Int {
        let payload = try consumeCStringPointer(leetium_sandbox_prune_images())
    ...
    func sandboxCheckPackages(base: String, packages: [String]) throws -> [String: Bool] {
        ...
        let parsed: BridgeSandboxCheckPackagesPayload = try callBridge(
            request,
            via: leetium_sandbox_check_packages
        )
        return parsed.found
    }
    ...
    func sandboxBuildImage(name: String, base: String, packages: [String]) throws -> String {
        ...
        let parsed: BridgeSandboxBuildImagePayload = try callBridge(
            request,
            via: leetium_sandbox_build_image
        )
        return parsed.tag
    }

    func sandboxGetDefaultImage() throws -> String {
        let payload = try consumeCStringPointer(leetium_sandbox_get_default_image())
    ...
    func sandboxSetDefaultImage(image: String?) throws -> String {
        ...
        let parsed: BridgeSandboxDefaultImagePayload = try callBridge(
            request,
            via: leetium_sandbox_set_default_image
        )
        return parsed.image
    }

    func sandboxGetSharedHome() throws -> BridgeSandboxSharedHomePayload {
        let payload = try consumeCStringPointer(leetium_sandbox_get_shared_home())
    ...
    func sandboxSetSharedHome(enabled: Bool, path: String?) throws -> BridgeSandboxSharedHomePayload {
        ...
        let parsed: BridgeSandboxSharedHomeSavePayload = try callBridge(
            request,
            via: leetium_sandbox_set_shared_home
        )
        _ = parsed.ok
    ...
    func sandboxListContainers() throws -> [BridgeSandboxContainerEntry] {
        let payload = try consumeCStringPointer(leetium_sandbox_list_containers())
    ...
    func sandboxStopContainer(name: String) throws {
        ...
        let _: BridgeOkPayload = try callBridge(request, via: leetium_sandbox_stop_container)
    }

    func sandboxRemoveContainer(name: String) throws {
        let request = SandboxContainerNameRequest(name: name)
        let _: BridgeOkPayload = try callBridge(request, via: leetium_sandbox_remove_container)
    }

    func sandboxCleanContainers() throws -> Int {
        let payload = try consumeCStringPointer(leetium_sandbox_clean_containers())
    ...
    func sandboxDiskUsage() throws -> BridgeSandboxDiskUsagePayload {
        let payload = try consumeCStringPointer(leetium_sandbox_disk_usage())
        let parsed = try decode(payload, as: BridgeSandboxDiskUsageEnvelope.self)
        return parsed.usage
    }

    func sandboxRestartDaemon() throws {
        let payload = try consumeCStringPointer(leetium_sandbox_restart_daemon())
        _ = try decode(payload, as: BridgeOkPayload.self)
    }

    /// Returns the soul text from SOUL.md, or nil if empty/missing.
    func getSoul() throws -> String? {
        let payload = try consumeCStringPointer(leetium_get_soul())
    ...
    func saveSoul(_ text: String?) throws {
        ...
        let payload = try json.withCString { ptr in
            try consumeCStringPointer(leetium_save_soul(ptr))
        }
        _ = try decode(payload, as: BridgeOkPayload.self)
    }
    ...
    func saveIdentity(name: String?, emoji: String?, theme: String?) throws {
        ...
        let payload = try json.withCString { ptr in
            try consumeCStringPointer(leetium_save_identity(ptr))
        }
        _ = try decode(payload, as: BridgeOkPayload.self)
    }
    ...
    func saveUserProfile(name: String?) throws {
        ...
        let payload = try json.withCString { ptr in
            try consumeCStringPointer(leetium_save_user_profile(ptr))
        }
        _ = try decode(payload, as: BridgeOkPayload.self)
    }
}
...
extension LeetiumClient {
    func listEnvVars() throws -> BridgeListEnvVarsPayload {
        let payload = try consumeCStringPointer(leetium_list_env_vars())
    ...
    func setEnvVar(key: String, value: String) throws {
        ...
        let _: BridgeOkPayload = try callBridge(request, via: leetium_set_env_var)
    }

    func deleteEnvVar(id: Int64) throws {
        let request = DeleteEnvVarRequest(id: id)
        let _: BridgeOkPayload = try callBridge(request, via: leetium_delete_env_var)
    }
}
