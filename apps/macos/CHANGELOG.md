...
All notable changes to the Leetium macOS app will be documented in this file.
...
- Settings panels connected to Rust backend — all settings (Identity, Soul,
  ...
  Channels, Hooks, MCP, Skills) now load from and save to `leetium.toml`,
  `IDENTITY.md`, `USER.md`, and `SOUL.md` via 6 new FFI functions
  (`leetium_get_config`, `leetium_save_config`, `leetium_get_soul`,
  `leetium_save_soul`, `leetium_save_identity`, `leetium_save_user_profile`)
...
- Environment panel now matches web UI behavior with env-var CRUD backed by the
  credential store (`leetium_list_env_vars`, `leetium_set_env_var`,
  `leetium_delete_env_var`), including encrypted/plaintext badges and vault
  status messaging
...
- Real-time session sync with web UI — sessions created, deleted, or patched in
  ...
  new `SessionEventBus` and `leetium_set_session_event_callback` FFI
- Full Leetium gateway in the HTTP Server pane — starts the complete gateway
...
- Identity loaded from `leetium.toml` + `SOUL.md` at startup via new
  `leetium_get_identity` FFI — Settings→Identity now shows agent name, emoji,
...
- Session CRUD FFI functions: `leetium_list_sessions`, `leetium_switch_session`,
  `leetium_create_session`, `leetium_session_chat_stream`
...
- `abortSession(key:)` and `peekSession(key:)` FFI bindings via `leetium_abort_session` and `leetium_peek_session`
- Chat messages are persisted to `~/.leetium/data/sessions/<key>.jsonl`
...
- New memory bridge endpoints for macOS settings:
  `leetium_memory_status`, `leetium_memory_config_get`,
  `leetium_memory_config_update`, `leetium_memory_qmd_status`
...
- Session metadata now stored in SQLite (`leetium.db`) instead of JSON file,
...
- Full-screen monospace TOML configuration editor with file I/O
  - Loads and saves `~/.leetium/leetium.toml` directly
  - Unsaved-changes indicator, Reveal in Finder button, Cmd+S save
- Streaming LLM responses via C function pointer callback (`leetium_chat_stream` FFI)
- Themed message bubbles matching web UI colors (light/dark mode)
- Live token bar with K/M formatting for input/output tokens
- In-app logging system with Rust-to-Swift bridge (`leetium_set_log_callback`)
...
- Swift-Rust FFI bridge via `leetium-swift-bridge` static library
  - JSON serialization between Swift and Rust
  - `with_ffi_boundary` panic safety wrapper
  - `consumeCStringPointer` + `leetium_free_string` memory management
...
- Retina app icon asset catalog (16–1024px from Leetium crab SVG)
- `LeetiumClient` helper with `callBridge` pattern for FFI encode-call-decode
