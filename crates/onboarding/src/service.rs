...
use leetium_config::{AgentIdentity, LeetiumConfig, UserProfile};
...
impl LiveOnboardingService {
    ...
    fn save(&self, config: &LeetiumConfig) -> Result<()> {
        leetium_config::loader::save_config_to_path(&self.config_path, config)
    ...
    pub fn wizard_start(&self, force: bool) -> Value {
        ...
        if self.config_path.exists()
            && let Ok(cfg) = leetium_config::loader::load_config(&self.config_path)
        {
    ...
    pub fn wizard_next(&self, input: &str) -> Result<Value> {
        ...
        if ws.is_done() {
            ...
            let mut config = if self.config_path.exists() {
                leetium_config::loader::load_config(&self.config_path).unwrap_or_default()
            } else {
            ...
            leetium_config::save_identity(&ws.identity).context("failed to save IDENTITY.md")?;
            leetium_config::save_user(&ws.user).context("failed to save USER.md")?;
            self.mark_onboarded();
    ...
    pub fn identity_update(&self, params: Value) -> Result<Value> {
        ...
        fn timezone_field(params: &Value, key: &str) -> Option<Option<leetium_config::Timezone>> {
            let raw = params.get(key).and_then(|v| v.as_str())?;
            let trimmed = raw.trim();
            if trimmed.is_empty() {
                return Some(None);
            }
            trimmed.parse::<leetium_config::Timezone>().ok().map(Some)
        }
        ...
        fn location_field(params: &Value) -> Option<Option<leetium_config::GeoLocation>> {
            let raw = params
                .get("user_location")
            ...
            Some(Some(leetium_config::GeoLocation::now(
        ...
        if let Some(v) = params.get("soul") {
            ...
            leetium_config::save_soul(soul.as_deref()).context("failed to save soul")?;
        }
        ...
        self.save(&config)?;
        leetium_config::save_identity(&identity).context("failed to save identity")?;
        leetium_config::save_user(&user).context("failed to save user")?;
    ...
    pub fn identity_get(&self) -> leetium_config::ResolvedIdentity {
            ...
            && let Ok(cfg) = leetium_config::loader::load_config(&self.config_path)
        {
            ...
            leetium_config::resolve_identity_from_config(&cfg)
        } else {
            info!(
                config_path = %self.config_path.display(),
                "identity_get: config not found, using defaults"
            );
            leetium_config::resolve_identity_from_config(&LeetiumConfig::default())
        };
        info!(
...
fn onboarded_sentinel() -> PathBuf {
    leetium_config::data_dir().join(".onboarded")
}
...
mod tests {
    ...
    fn wizard_round_trip() {
        ...
        leetium_config::set_data_dir(dir.path().to_path_buf());
        let config_path = dir.path().join("leetium.toml");
    ...
    fn cancel_wizard() {
        ...
        let dir = tempfile::tempdir().unwrap();
        leetium_config::set_data_dir(dir.path().to_path_buf());
        let svc = LiveOnboardingService::new(dir.path().join("leetium.toml"));
    ...
    fn identity_update_partial() {
        ...
        let user = leetium_config::load_user().expect("load user");
        assert_eq!(
