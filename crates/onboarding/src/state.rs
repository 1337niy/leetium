...
use leetium_config::{AgentIdentity, UserProfile};
...
impl WizardState {
    ...
    pub fn prompt(&self) -> &str {
        match self.step {
            WizardStep::Welcome => {
                "Welcome to leetium! Let's set things up. Press Enter to continue."
            },
        ...
        match self.step {
            ...
            WizardStep::AgentName => {
                if !input.is_empty() {
                    ...
                    self.identity.name = Some("leetium".to_string());
                }
...
mod tests {
    ...
    fn default_agent_name() {
        ...
        s.advance(""); // welcome
        s.advance("User");
        s.advance(""); // empty agent name → defaults to "leetium"
        assert_eq!(s.identity.name.as_deref(), Some("leetium"));
    }
}
