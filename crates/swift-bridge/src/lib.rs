//! C ABI bridge for embedding Leetium Rust functionality into native Swift apps.
...
use {
    ...
    leetium_config::validate::Severity,
    leetium_provider_setup::{
...
struct BridgeState {
    ...
    credential_store: Arc<leetium_gateway::auth::CredentialStore>,
...
impl BridgeState {
    fn new() -> Self {
        ...
        // Open the shared SQLite database (same leetium.db used by the gateway).
        ...
        let db_pool = runtime.block_on(async {
            ...
            let opts = SqliteConnectOptions::from_str(&format!("sqlite:{}", db_path.display()))
                .expect("invalid leetium.db path")
                .create_if_missing(true)
                .journal_mode(SqliteJournalMode::Wal)
                .synchronous(SqliteSynchronous::Normal);
            let pool = sqlx::SqlitePool::connect_with(opts)
                .await
                .unwrap_or_else(|e| panic!("failed to open leetium.db: {e}"));
            ...
            if let Err(e) = leetium_projects::run_migrations(&pool).await {
                emit_log("WARN", "bridge", &format!("projects migration: {e}"));
            }
            if let Err(e) = leetium_sessions::run_migrations(&pool).await {
                emit_log("WARN", "bridge", &format!("sessions migration: {e}"));
            }
            if let Err(e) = leetium_gateway::run_migrations(&pool).await {
        ...
        let credential_store = runtime.block_on(async {
            ...
            if let Err(e) = leetium_gateway::auth::leetium_vault::run_migrations(&db_pool).await {
                emit_log("WARN", "bridge", &format!("vault migration: {e}"));
            }

            let vault = match leetium_gateway::auth::leetium_vault::Vault::new(db_pool.clone()).await
            {
            ...
            match leetium_gateway::auth::CredentialStore::with_vault(
                db_pool.clone(),
                &leetium_config::discover_and_load().auth,
                vault,
            )
            .await
            {
...
fn init_swift_bridge_test_dirs() {
    ...
    TEST_DIRS_INIT.get_or_init(|| {
        ...
        leetium_config::set_config_dir(config_dir);
        leetium_config::set_data_dir(data_dir);
    });
}
...
struct HttpdHandle {
    ...
    state: std::sync::Arc<leetium_gateway::state::GatewayState>,
}
...
#[allow(unsafe_code)]
fn emit_network_audit(entry: &leetium_network_filter::NetworkAuditEntry) {
    if let Some(callback) = NETWORK_AUDIT_CALLBACK.get() {
        let source = match &entry.approval_source {
            Some(leetium_network_filter::ApprovalSource::Config) => "config",
            Some(leetium_network_filter::ApprovalSource::Session) => "session",
            Some(leetium_network_filter::ApprovalSource::UserPrompt) => "user",
            None => "unknown",
        };
...
struct ListEnvVarsResponse {
    env_vars: Vec<leetium_gateway::auth::EnvVarEntry>,
    vault_status: String,
}
...
struct AuthPasskeysResponse {
    passkeys: Vec<leetium_gateway::auth::PasskeyEntry>,
}
...
struct SandboxContainersResponse {
    containers: Vec<leetium_tools::sandbox::RunningContainer>,
}
...
struct SandboxDiskUsageResponse {
    usage: leetium_tools::sandbox::ContainerDiskUsage,
}
...
fn build_validation_summary(config_toml: Option<&str>) -> Option<ValidationSummary> {
    ...
    let result = leetium_config::validate::validate_toml_str(config_toml);
...
fn config_dir_string() -> String {
    match leetium_config::config_dir() {
...
fn data_dir_string() -> String {
    leetium_config::data_dir().display().to_string()
}
...
fn sandbox_effective_default_image(config: &leetium_config::LeetiumConfig) -> String {
    ...
    config
        ...
        .image
        .clone()
        .unwrap_or_else(|| leetium_tools::sandbox::DEFAULT_SANDBOX_IMAGE.to_owned())
}

fn sandbox_backend_name(config: &leetium_config::LeetiumConfig) -> String {
    let runtime_cfg = leetium_tools::sandbox::SandboxConfig::from(&config.tools.exec.sandbox);
    let backend = leetium_tools::sandbox::create_sandbox(runtime_cfg);
    backend.backend_name().to_owned()
}

fn sandbox_status_from_config(config: &leetium_config::LeetiumConfig) -> SandboxStatusResponse {
...
fn sandbox_container_prefix(config: &leetium_config::LeetiumConfig) -> String {
    let runtime_cfg = leetium_tools::sandbox::SandboxConfig::from(&config.tools.exec.sandbox);
    runtime_cfg
        .container_prefix
        .unwrap_or_else(|| "leetium-sandbox".to_owned())
}

fn sandbox_shared_home_config_from_config(
    config: &leetium_config::LeetiumConfig,
) -> SandboxSharedHomeConfigResponse {
    let runtime_cfg = leetium_tools::sandbox::SandboxConfig::from(&config.tools.exec.sandbox);
    let mode = match config.tools.exec.sandbox.home_persistence {
        leetium_config::schema::HomePersistenceConfig::Off => "off",
        leetium_config::schema::HomePersistenceConfig::Session => "session",
        leetium_config::schema::HomePersistenceConfig::Shared => "shared",
    };

    SandboxSharedHomeConfigResponse {
        enabled: matches!(
            config.tools.exec.sandbox.home_persistence,
            leetium_config::schema::HomePersistenceConfig::Shared
        ),
        mode: mode.to_owned(),
        path: leetium_tools::sandbox::shared_home_dir_path(&runtime_cfg)
            .display()
            .to_string(),
...
fn build_chat_response(request: ChatRequest) -> String {
    ...
    let response = ChatResponse {
        ...
        default_soul: leetium_config::DEFAULT_SOUL.to_owned(),
        validation,
        input_tokens,
        output_tokens,
        duration_ms,
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn leetium_chat_stream(
    request_json: *const c_char,
    callback: StreamCallback,
    user_data: *mut c_void,
) {
    record_call("leetium_chat_stream");
    trace_call("leetium_chat_stream");
    ...
    let raw = match read_c_string(request_json) {
        Ok(value) => value,
        Err(message) => {
            record_error("leetium_chat_stream", "null_pointer_or_invalid_utf8");
            send_error(message);
            return;
        },
    };
    ...
    let request = match serde_json::from_str::<ChatRequest>(&raw) {
        Ok(request) => request,
        Err(error) => {
            record_error("leetium_chat_stream", "invalid_json");
...
fn record_call(function: &'static str) {
    metrics::counter!("leetium_swift_bridge_calls_total", "function" => function).increment(1);
}
...
fn record_error(function: &'static str, code: &'static str) {
    metrics::counter!(
        "leetium_swift_bridge_errors_total",
        "function" => function,
        "code" => code
    )
    .increment(1);
...
fn trace_call(function: &'static str) {
    tracing::debug!(target: "leetium_swift_bridge", function, "ffi call");
}
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_version() -> *mut c_char {
    record_call("leetium_version");
    trace_call("leetium_version");

    with_ffi_boundary(|| {
        emit_log("DEBUG", "bridge", "leetium_version called");
        let response = VersionResponse {
            bridge_version: env!("CARGO_PKG_VERSION"),
            leetium_version: env!("CARGO_PKG_VERSION"),
            config_dir: config_dir_string(),
        };
        emit_log(
            "INFO",
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_get_identity() -> *mut c_char {
    record_call("leetium_get_identity");
    trace_call("leetium_get_identity");

    with_ffi_boundary(|| {
        let resolved = leetium_config::resolve_identity();
        emit_log("DEBUG", "bridge", "leetium_get_identity called");
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_chat_json(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_chat_json");
    trace_call("leetium_chat_json");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<ChatRequest>("leetium_chat_json", request_json) {
            Ok(request) => request,
            Err(e) => return e,
        };
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_known_providers() -> *mut c_char {
    record_call("leetium_known_providers");
    trace_call("leetium_known_providers");
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_detect_providers() -> *mut c_char {
    record_call("leetium_detect_providers");
    trace_call("leetium_detect_providers");
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_save_provider_config(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_save_provider_config");
    trace_call("leetium_save_provider_config");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SaveProviderRequest>(
            "leetium_save_provider_config",
            request_json,
        ) {
            Ok(request) => request,
            Err(e) => return e,
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_list_models() -> *mut c_char {
    record_call("leetium_list_models");
    trace_call("leetium_list_models");
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_refresh_registry() -> *mut c_char {
    record_call("leetium_refresh_registry");
    trace_call("leetium_refresh_registry");
...
/// `leetium_*` FFI functions from this crate. Passing any other pointer, or
/// freeing the same pointer more than once, is undefined behavior.
pub unsafe extern "C" fn leetium_free_string(ptr: *mut c_char) {
    record_call("leetium_free_string");
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn leetium_set_log_callback(callback: LogCallback) {
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn leetium_set_session_event_callback(callback: SessionEventCallback) {
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn leetium_set_network_audit_callback(callback: NetworkAuditCallback) {
    let _ = NETWORK_AUDIT_CALLBACK.set(callback);
    emit_log("INFO", "bridge", "Network audit callback registered");
}

/// Starts the embedded HTTP server with the full Leetium gateway.
/// Returns JSON with `{"running": true, "addr": "..."}`.
/// If already running, returns the current status without restarting.
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_start_httpd(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_start_httpd");
    trace_call("leetium_start_httpd");
    ...
    with_ffi_boundary(|| {
        ...
        let prepared =
            match BRIDGE
                .runtime
                .block_on(leetium_gateway::server::prepare_gateway_embedded(
                    &request.host,
                    request.port,
                    ...
                    Some(leetium_web::web_routes), // full web UI
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_stop_httpd() -> *mut c_char {
    record_call("leetium_stop_httpd");
    trace_call("leetium_stop_httpd");
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_httpd_status() -> *mut c_char {
    record_call("leetium_httpd_status");
    trace_call("leetium_httpd_status");
...
/// running (via `leetium_start_httpd`). Returns JSON with `{"aborted": bool}`.
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_abort_session(session_key: *const c_char) -> *mut c_char {
    record_call("leetium_abort_session");
    trace_call("leetium_abort_session");
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_peek_session(session_key: *const c_char) -> *mut c_char {
    record_call("leetium_peek_session");
    trace_call("leetium_peek_session");
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_list_sessions() -> *mut c_char {
    record_call("leetium_list_sessions");
    trace_call("leetium_list_sessions");
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_switch_session(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_switch_session");
    trace_call("leetium_switch_session");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SwitchSessionRequest>(
            "leetium_switch_session",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_create_session(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_create_session");
    trace_call("leetium_create_session");
...
/// # Safety
///
/// Same requirements as `leetium_chat_stream`.
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn leetium_session_chat_stream(
    request_json: *const c_char,
    callback: StreamCallback,
    user_data: *mut c_void,
) {
    record_call("leetium_session_chat_stream");
    trace_call("leetium_session_chat_stream");
...
/// Returns the full `LeetiumConfig` as JSON together with `config_dir` and
/// `data_dir` paths. Swift uses this to populate all settings panels.
#[unsafe(no_mangle)]
pub extern "C" fn leetium_get_config() -> *mut c_char {
    record_call("leetium_get_config");
    trace_call("leetium_get_config");

    with_ffi_boundary(|| {
        emit_log("DEBUG", "bridge", "leetium_get_config called");
        let config = leetium_config::discover_and_load();
...
/// Accepts a full `LeetiumConfig` JSON and saves it via `save_config()`.
/// The TOML writer preserves existing comments in the config file.
#[unsafe(no_mangle)]
pub extern "C" fn leetium_save_config(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_save_config");
    trace_call("leetium_save_config");

    with_ffi_boundary(|| {
        let config = match parse_ffi_request::<leetium_config::LeetiumConfig>(
            "leetium_save_config",
            request_json,
        ) {
            Ok(c) => c,
        ...
        match leetium_config::save_config(&config) {
            Ok(path) => {
                emit_log(
                    "INFO",
                    "bridge.config",
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_memory_status() -> *mut c_char {
    record_call("leetium_memory_status");
    trace_call("leetium_memory_status");
    ...
    with_ffi_boundary(|| {
        ...
        let db_path = leetium_config::data_dir().join("memory.db");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_memory_config_get() -> *mut c_char {
    record_call("leetium_memory_config_get");
    trace_call("leetium_memory_config_get");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_memory_config_update(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_memory_config_update");
    trace_call("leetium_memory_config_update");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<MemoryConfigUpdateRequest>(
            "leetium_memory_config_update",
            request_json,
        ) {
            Ok(request) => request,
            Err(e) => return e,
        ...
        let current = leetium_config::discover_and_load().memory;
        let backend = request
            .backend
        ...
        if let Err(error) = leetium_config::update_config(|cfg| {
            ...
            record_error("leetium_memory_config_update", "save_failed");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_memory_qmd_status() -> *mut c_char {
    record_call("leetium_memory_qmd_status");
    trace_call("leetium_memory_qmd_status");
    ...
    with_ffi_boundary(|| {
        ...
        let command = leetium_config::discover_and_load()
            .memory
            .qmd
            .command
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_get_soul() -> *mut c_char {
    record_call("leetium_get_soul");
    trace_call("leetium_get_soul");

    with_ffi_boundary(|| {
        emit_log("DEBUG", "bridge", "leetium_get_soul called");
        let soul = leetium_config::load_soul();
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_save_soul(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_save_soul");
    trace_call("leetium_save_soul");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SaveSoulRequest>("leetium_save_soul", request_json) {
            Ok(r) => r,
            Err(e) => return e,
        };

        emit_log("INFO", "bridge.config", "Saving soul from settings");
        match leetium_config::save_soul(request.soul.as_deref()) {
            Ok(path) => {
                emit_log(
                    "INFO",
                    "bridge.config",
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_save_identity(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_save_identity");
    trace_call("leetium_save_identity");

    with_ffi_boundary(|| {
        let request =
            match parse_ffi_request::<SaveIdentityRequest>("leetium_save_identity", request_json) {
                Ok(r) => r,
                Err(e) => return e,
            };

        let identity = leetium_config::AgentIdentity {
            name: request.name,
            emoji: request.emoji,
            theme: request.theme,
        };
        ...
        match leetium_config::save_identity(&identity) {
            Ok(path) => {
                emit_log(
                    "INFO",
                    "bridge.config",
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_save_user_profile(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_save_user_profile");
    trace_call("leetium_save_user_profile");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SaveUserProfileRequest>(
            "leetium_save_user_profile",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
        ...
        let user = leetium_config::UserProfile {
            name: request.name,
            ..Default::default()
        };

        emit_log("INFO", "bridge.config", "Saving user profile from settings");
        match leetium_config::save_user(&user) {
            Ok(path) => {
                emit_log(
                    "INFO",
                    "bridge.config",
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_list_env_vars() -> *mut c_char {
    record_call("leetium_list_env_vars");
    trace_call("leetium_list_env_vars");
    ...
    with_ffi_boundary(|| {
            ...
            .block_on(BRIDGE.credential_store.list_env_vars())
        {
            Ok(vars) => vars,
            Err(e) => {
                record_error("leetium_list_env_vars", "ENV_LIST_FAILED");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_set_env_var(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_set_env_var");
    trace_call("leetium_set_env_var");

    with_ffi_boundary(|| {
        let request =
            match parse_ffi_request::<SetEnvVarRequest>("leetium_set_env_var", request_json) {
                Ok(r) => r,
                Err(e) => return e,
            };
        ...
        if key.is_empty() {
            record_error("leetium_set_env_var", "ENV_KEY_REQUIRED");
            return encode_error("ENV_KEY_REQUIRED", "key is required");
        }
        if !key.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
            record_error("leetium_set_env_var", "ENV_KEY_INVALID");
            return encode_error(
                "ENV_KEY_INVALID",
            ...
            .block_on(BRIDGE.credential_store.set_env_var(key, &request.value))
        {
            ...
            Err(e) => {
                record_error("leetium_set_env_var", "ENV_SET_FAILED");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_delete_env_var(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_delete_env_var");
    trace_call("leetium_delete_env_var");

    with_ffi_boundary(|| {
        let request =
            match parse_ffi_request::<DeleteEnvVarRequest>("leetium_delete_env_var", request_json) {
                Ok(r) => r,
                Err(e) => return e,
            };
            ...
            .block_on(BRIDGE.credential_store.delete_env_var(request.id))
        {
            Ok(_) => encode_json(&OkResponse { ok: true }),
            Err(e) => {
                record_error("leetium_delete_env_var", "ENV_DELETE_FAILED");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_auth_status() -> *mut c_char {
    record_call("leetium_auth_status");
    trace_call("leetium_auth_status");
    ...
    with_ffi_boundary(|| {
            ...
            .block_on(BRIDGE.credential_store.has_password())
        {
            Ok(value) => value,
            Err(error) => {
                record_error("leetium_auth_status", "AUTH_STATUS_FAILED");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_auth_password_change(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_auth_password_change");
    trace_call("leetium_auth_password_change");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<AuthPasswordChangeRequest>(
            "leetium_auth_password_change",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
        ...
        if request.new_password.expose_secret().len() < 8 {
            record_error("leetium_auth_password_change", "AUTH_PASSWORD_TOO_SHORT");
            return encode_error(
            ...
            .block_on(BRIDGE.credential_store.has_password())
        {
            Ok(value) => value,
            Err(error) => {
                record_error("leetium_auth_password_change", "AUTH_STATUS_FAILED");
                return encode_error("AUTH_STATUS_FAILED", &error.to_string());
            },
        };
        ...
        if has_password {
            ...
            if let Err(error) = BRIDGE.runtime.block_on(
                ...
                record_error("leetium_auth_password_change", "AUTH_PASSWORD_CHANGE_FAILED");
        ...
        {
            record_error("leetium_auth_password_change", "AUTH_PASSWORD_SET_FAILED");
            ...
            match BRIDGE.runtime.block_on(vault.initialize(new_password)) {
                ...
                Err(leetium_gateway::auth::leetium_vault::VaultError::AlreadyInitialized) => {
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_auth_reset() -> *mut c_char {
    record_call("leetium_auth_reset");
    trace_call("leetium_auth_reset");

    with_ffi_boundary(
        || match BRIDGE.runtime.block_on(BRIDGE.credential_store.reset_all()) {
            Ok(()) => encode_json(&OkResponse { ok: true }),
            Err(error) => {
                record_error("leetium_auth_reset", "AUTH_RESET_FAILED");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_auth_list_passkeys() -> *mut c_char {
    record_call("leetium_auth_list_passkeys");
    trace_call("leetium_auth_list_passkeys");
    ...
    with_ffi_boundary(|| {
            ...
            .block_on(BRIDGE.credential_store.list_passkeys())
        {
            Ok(entries) => entries,
            Err(error) => {
                record_error("leetium_auth_list_passkeys", "AUTH_PASSKEY_LIST_FAILED");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_auth_remove_passkey(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_auth_remove_passkey");
    trace_call("leetium_auth_remove_passkey");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<AuthPasskeyIdRequest>(
            "leetium_auth_remove_passkey",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
            ...
            .block_on(BRIDGE.credential_store.remove_passkey(request.id))
        {
            Ok(()) => encode_json(&OkResponse { ok: true }),
            Err(error) => {
                record_error("leetium_auth_remove_passkey", "AUTH_PASSKEY_REMOVE_FAILED");
...
/// Renames a passkey.
#[unsafe(no_mangle)]
pub extern "C" fn leetium_auth_rename_passkey(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_auth_rename_passkey");
    trace_call("leetium_auth_rename_passkey");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<AuthPasskeyRenameRequest>(
            "leetium_auth_rename_passkey",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
        ...
        if name.is_empty() {
            record_error("leetium_auth_rename_passkey", "AUTH_PASSKEY_NAME_REQUIRED");
            ...
            .block_on(BRIDGE.credential_store.rename_passkey(request.id, name))
        {
            Ok(()) => encode_json(&OkResponse { ok: true }),
            Err(error) => {
                record_error("leetium_auth_rename_passkey", "AUTH_PASSKEY_RENAME_FAILED");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_status() -> *mut c_char {
    record_call("leetium_sandbox_status");
    trace_call("leetium_sandbox_status");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_list_images() -> *mut c_char {
    record_call("leetium_sandbox_list_images");
    trace_call("leetium_sandbox_list_images");
    ...
    with_ffi_boundary(|| {
        ...
        let (cached, sandbox) = BRIDGE.runtime.block_on(async {
            tokio::join!(
                builder.list_cached(),
                leetium_tools::sandbox::list_sandbox_images()
            )
        });
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_delete_image(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_sandbox_delete_image");
    trace_call("leetium_sandbox_delete_image");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SandboxDeleteImageRequest>(
            "leetium_sandbox_delete_image",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
        ...
        if tag.is_empty() {
            record_error("leetium_sandbox_delete_image", "IMAGE_TAG_REQUIRED");
            return encode_error("IMAGE_TAG_REQUIRED", "tag is required");
        }

        let result = BRIDGE.runtime.block_on(async {
            if tag.contains("-sandbox:") {
                leetium_tools::sandbox::remove_sandbox_image(tag).await
            } else {
                let builder = leetium_tools::image_cache::DockerImageBuilder::new();
                let full_tag = if tag.starts_with("leetium-cache/") {
                    tag.to_owned()
                } else {
                    format!("leetium-cache/{tag}")
                };
        ...
        match result {
            Ok(()) => encode_json(&OkResponse { ok: true }),
            Err(error) => {
                record_error("leetium_sandbox_delete_image", IMAGE_CACHE_DELETE_FAILED);
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_prune_images() -> *mut c_char {
    record_call("leetium_sandbox_prune_images");
    trace_call("leetium_sandbox_prune_images");
    ...
    with_ffi_boundary(|| {
        ...
        let (tool_result, sandbox_result) = BRIDGE.runtime.block_on(async {
            tokio::join!(
                builder.prune_all(),
                leetium_tools::sandbox::clean_sandbox_images()
            )
        });
        ...
        if let (Err(e1), Err(e2)) = (&tool_result, &sandbox_result) {
            ...
            record_error("leetium_sandbox_prune_images", IMAGE_CACHE_PRUNE_FAILED);
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_check_packages(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_sandbox_check_packages");
    trace_call("leetium_sandbox_check_packages");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SandboxCheckPackagesRequest>(
            "leetium_sandbox_check_packages",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
        ...
        if !is_valid_image_ref(&base) {
            record_error("leetium_sandbox_check_packages", SANDBOX_BASE_IMAGE_INVALID);
            return encode_error(
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_build_image(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_sandbox_build_image");
    trace_call("leetium_sandbox_build_image");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SandboxBuildImageRequest>(
            "leetium_sandbox_build_image",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
        ...
        let name = request.name.trim();
        if name.is_empty() {
            record_error("leetium_sandbox_build_image", SANDBOX_IMAGE_NAME_REQUIRED);
        ...
        {
            record_error("leetium_sandbox_build_image", SANDBOX_IMAGE_NAME_INVALID);
            return encode_error(
        ...
        if !is_valid_image_ref(&base) {
            record_error("leetium_sandbox_build_image", SANDBOX_BASE_IMAGE_INVALID);
            return encode_error(
                SANDBOX_BASE_IMAGE_INVALID,
                "base image contains invalid characters",
            );
        ...
        if let Some(bad) = packages.iter().find(|p| !is_valid_package_name(p)) {
            record_error("leetium_sandbox_build_image", SANDBOX_PACKAGE_NAME_INVALID);
            return encode_error(
                SANDBOX_PACKAGE_NAME_INVALID,
                &format!("invalid package name: {bad}"),
            );
...
WORKDIR /home/sandbox\n"
        ...
        let tmp_dir = std::env::temp_dir().join(format!("leetium-build-{}", uuid::Uuid::new_v4()));
        if let Err(error) = std::fs::create_dir_all(&tmp_dir) {
            record_error("leetium_sandbox_build_image", SANDBOX_TMP_DIR_CREATE_FAILED);
        ...
        match result {
            ...
            Err(error) => {
                record_error("leetium_sandbox_build_image", SANDBOX_IMAGE_BUILD_FAILED);
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_get_default_image() -> *mut c_char {
    record_call("leetium_sandbox_get_default_image");
    trace_call("leetium_sandbox_get_default_image");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_set_default_image(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_sandbox_set_default_image");
    trace_call("leetium_sandbox_set_default_image");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SandboxSetDefaultImageRequest>(
            "leetium_sandbox_set_default_image",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_get_shared_home() -> *mut c_char {
    record_call("leetium_sandbox_get_shared_home");
    trace_call("leetium_sandbox_get_shared_home");
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_set_shared_home(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_sandbox_set_shared_home");
    trace_call("leetium_sandbox_set_shared_home");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SandboxSharedHomeUpdateRequest>(
            "leetium_sandbox_set_shared_home",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
        ...
        let update_result = leetium_config::update_config(|cfg| {
            ...
            if request.enabled {
                ...
                cfg.tools.exec.sandbox.home_persistence =
                    leetium_config::schema::HomePersistenceConfig::Off;
            }
        });
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_list_containers() -> *mut c_char {
    record_call("leetium_sandbox_list_containers");
    trace_call("leetium_sandbox_list_containers");
    ...
    with_ffi_boundary(|| {
        ...
        match BRIDGE
            .runtime
            .block_on(leetium_tools::sandbox::list_running_containers(&prefix))
        {
            Ok(containers) => encode_json(&SandboxContainersResponse { containers }),
            Err(error) => {
                record_error(
                    "leetium_sandbox_list_containers",
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_stop_container(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_sandbox_stop_container");
    trace_call("leetium_sandbox_stop_container");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SandboxContainerNameRequest>(
            "leetium_sandbox_stop_container",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
        ...
        match BRIDGE
            .runtime
            .block_on(leetium_tools::sandbox::stop_container(name))
        {
            Ok(()) => encode_json(&OkResponse { ok: true }),
            Err(error) => {
                record_error(
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_remove_container(request_json: *const c_char) -> *mut c_char {
    record_call("leetium_sandbox_remove_container");
    trace_call("leetium_sandbox_remove_container");

    with_ffi_boundary(|| {
        let request = match parse_ffi_request::<SandboxContainerNameRequest>(
            "leetium_sandbox_remove_container",
            request_json,
        ) {
            Ok(r) => r,
            Err(e) => return e,
        ...
        match BRIDGE
            .runtime
            .block_on(leetium_tools::sandbox::remove_container(name))
        {
            Ok(()) => encode_json(&OkResponse { ok: true }),
            Err(error) => {
                record_error(
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_clean_containers() -> *mut c_char {
    record_call("leetium_sandbox_clean_containers");
    trace_call("leetium_sandbox_clean_containers");
    ...
    with_ffi_boundary(|| {
        ...
        match BRIDGE
            .runtime
            .block_on(leetium_tools::sandbox::clean_all_containers(&prefix))
        {
            Ok(removed) => encode_json(&SandboxCleanContainersResponse { ok: true, removed }),
            Err(error) => {
                record_error(
                    "leetium_sandbox_clean_containers",
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_disk_usage() -> *mut c_char {
    record_call("leetium_sandbox_disk_usage");
    trace_call("leetium_sandbox_disk_usage");

    with_ffi_boundary(|| {
        match BRIDGE
            .runtime
            .block_on(leetium_tools::sandbox::container_disk_usage())
        {
            Ok(usage) => encode_json(&SandboxDiskUsageResponse { usage }),
            Err(error) => {
                record_error("leetium_sandbox_disk_usage", SANDBOX_DISK_USAGE_FAILED);
...
#[unsafe(no_mangle)]
pub extern "C" fn leetium_sandbox_restart_daemon() -> *mut c_char {
    record_call("leetium_sandbox_restart_daemon");
    trace_call("leetium_sandbox_restart_daemon");

    with_ffi_boundary(|| {
        match BRIDGE
            .runtime
            .block_on(leetium_tools::sandbox::restart_container_daemon())
        {
            Ok(()) => encode_json(&OkResponse { ok: true }),
            Err(error) => {
                record_error(
                    "leetium_sandbox_restart_daemon",
...
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
pub extern "C" fn leetium_shutdown() {
    record_call("leetium_shutdown");
    trace_call("leetium_shutdown");
...
mod tests {
    ...
    fn version_returns_expected_payload() {
        let payload = json_from_ptr(leetium_version());
    ...
    fn chat_returns_error_for_null_pointer() {
        let payload = json_from_ptr(leetium_chat_json(std::ptr::null()));
    ...
    fn chat_returns_validation_counts() {
        ...
        let payload = json_from_ptr(leetium_chat_json(c_request.as_ptr()));
    ...
    fn known_providers_returns_array() {
        let payload = json_from_ptr(leetium_known_providers());
    ...
    fn detect_providers_returns_array() {
        let payload = json_from_ptr(leetium_detect_providers());
    ...
    fn save_provider_config_returns_error_for_null() {
        let payload = json_from_ptr(leetium_save_provider_config(std::ptr::null()));
    ...
    fn list_models_returns_array() {
        let payload = json_from_ptr(leetium_list_models());
    ...
    fn refresh_registry_returns_ok() {
        let payload = json_from_ptr(leetium_refresh_registry());
    ...
    fn free_string_tolerates_null_pointer() {
        ...
        unsafe {
            leetium_free_string(std::ptr::null_mut());
        }
    }
    ...
    fn chat_stream_sends_error_for_null_pointer() {
        ...
        unsafe {
            leetium_chat_stream(std::ptr::null(), test_callback, user_data);
        }
    ...
    fn httpd_start_and_stop() {
        ...
        let payload = json_from_ptr(leetium_start_httpd(c_request.as_ptr()));
        assert_eq!(
        ...
        let status = json_from_ptr(leetium_httpd_status());
        assert_eq!(status.get("running").and_then(Value::as_bool), Some(true),);

        // Stop.
        let stopped = json_from_ptr(leetium_stop_httpd());
        assert_eq!(stopped.get("running").and_then(Value::as_bool), Some(false),);

        // Status after stop.
        let status2 = json_from_ptr(leetium_httpd_status());
    ...
    fn httpd_stop_when_not_running() {
        ...
        let payload = json_from_ptr(leetium_stop_httpd());
    ...
    fn chat_stream_sends_error_for_no_provider() {
        ...
        unsafe {
            leetium_chat_stream(c_request.as_ptr(), test_callback, user_data);
        }
    ...
    fn list_sessions_returns_array() {
        let payload = json_from_ptr(leetium_list_sessions());
        assert!(
            payload.as_array().is_some(),
    ...
    fn create_and_switch_session() {
        ...
        let c_request = CString::new(request).unwrap_or_else(|e| panic!("{e}"));
        let payload = json_from_ptr(leetium_create_session(c_request.as_ptr()));
        ...
        let history = json_from_ptr(leetium_switch_session(c_switch.as_ptr()));
    ...
    fn create_session_with_null_uses_defaults() {
        let payload = json_from_ptr(leetium_create_session(std::ptr::null()));
    ...
    fn get_config_returns_config_and_paths() {
        let payload = json_from_ptr(leetium_get_config());
    ...
    fn save_config_returns_error_for_null() {
        let payload = json_from_ptr(leetium_save_config(std::ptr::null()));
    ...
    fn save_config_returns_error_for_invalid_json() {
        ...
        let payload = json_from_ptr(leetium_save_config(bad.as_ptr()));
    ...
    fn memory_status_returns_expected_fields() {
        let payload = json_from_ptr(leetium_memory_status());
    ...
    fn memory_config_get_returns_expected_fields() {
        let payload = json_from_ptr(leetium_memory_config_get());
    ...
    fn memory_config_update_returns_error_for_null() {
        let payload = json_from_ptr(leetium_memory_config_update(std::ptr::null()));
    ...
    fn memory_config_update_round_trip() {
        ...
        .to_string();
        let c_request = CString::new(request).unwrap_or_else(|e| panic!("{e}"));
        let payload = json_from_ptr(leetium_memory_config_update(c_request.as_ptr()));
    ...
    fn memory_qmd_status_returns_expected_fields() {
        let payload = json_from_ptr(leetium_memory_qmd_status());
    ...
    fn get_soul_returns_soul_field() {
        let payload = json_from_ptr(leetium_get_soul());
    ...
    fn save_soul_returns_error_for_null() {
        let payload = json_from_ptr(leetium_save_soul(std::ptr::null()));
    ...
    fn save_identity_returns_error_for_null() {
        let payload = json_from_ptr(leetium_save_identity(std::ptr::null()));
    ...
    fn save_user_profile_returns_error_for_null() {
        let payload = json_from_ptr(leetium_save_user_profile(std::ptr::null()));
    ...
    fn list_env_vars_returns_env_vars_and_vault_status() {
        let payload = json_from_ptr(leetium_list_env_vars());
    ...
    fn set_env_var_returns_error_for_null() {
        let payload = json_from_ptr(leetium_set_env_var(std::ptr::null()));
    ...
    fn set_env_var_rejects_invalid_key() {
        ...
        let c_request = CString::new(request).unwrap_or_else(|e| panic!("{e}"));
        let payload = json_from_ptr(leetium_set_env_var(c_request.as_ptr()));
    ...
    fn delete_env_var_returns_error_for_null() {
        let payload = json_from_ptr(leetium_delete_env_var(std::ptr::null()));
    ...
    fn set_and_delete_env_var_round_trip() {
        ...
        let set_payload = json_from_ptr(leetium_set_env_var(c_set_request.as_ptr()));
        assert_eq!(set_payload.get("ok").and_then(Value::as_bool), Some(true));

        let list_payload = json_from_ptr(leetium_list_env_vars());
        ...
        let delete_payload = json_from_ptr(leetium_delete_env_var(c_delete_request.as_ptr()));
        assert_eq!(
    ...
    fn auth_status_returns_expected_fields() {
        let payload = json_from_ptr(leetium_auth_status());
    ...
    fn auth_list_passkeys_returns_array() {
        let payload = json_from_ptr(leetium_auth_list_passkeys());
        assert!(
    ...
    fn auth_password_change_rejects_short_password() {
        ...
        let c_request = CString::new(request).unwrap_or_else(|e| panic!("{e}"));
        let payload = json_from_ptr(leetium_auth_password_change(c_request.as_ptr()));
    ...
    fn auth_remove_passkey_returns_error_for_null() {
        let payload = json_from_ptr(leetium_auth_remove_passkey(std::ptr::null()));
