...
impl MsTeamsPlugin {
    ...
    async fn process_activity(
        ...
        leetium_metrics::counter!(
            ...
            leetium_metrics::labels::CHANNEL => "msteams"
        )
        .increment(1);
...
impl ChannelPlugin for MsTeamsPlugin {
    ...
    async fn start_account(
        ...
        accounts.insert(account_id.to_string(), AccountState {
            ...
            http: leetium_common::http::shared_http_client().clone(),
    ...
    fn channel_webhook_verifier(
        &self,
        account_id: &str,
    ) -> Option<Box<dyn leetium_channels::channel_webhook_middleware::ChannelWebhookVerifier>> {
        let accounts = self.accounts.read().unwrap_or_else(|e| e.into_inner());
...
mod tests {
    use {
        super::*,
        leetium_channels::{InboundMode, plugin::ChannelType},
    };
