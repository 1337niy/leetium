...
//! `x-leetium-webhook-secret` header by the gateway route handler) with
...
/// and places it in the `x-leetium-webhook-secret` header before calling this
/// verifier.
...
mod tests {
    ...
    fn headers_with_secret(secret: &str) -> HeaderMap {
        ...
        headers.insert("x-leetium-webhook-secret", secret.parse().unwrap());
        headers
    }
    ...
    fn contract_rejects_empty_signature() {
        ...
        leetium_channels::contract::channel_webhook_verifier_rejects_empty_signature(&verifier);
    }
    ...
    fn contract_rejects_bad_signature() {
        let verifier =
            TeamsChannelWebhookVerifier::new(Some(Secret::new("correct-secret".into())), true);
        let headers = headers_with_secret("wrong-secret");
        leetium_channels::contract::channel_webhook_verifier_rejects_bad_signature(
            &verifier, &headers,
        );
    }
    ...
    fn contract_has_channel_type() {
        let verifier = TeamsChannelWebhookVerifier::new(None, false);
        leetium_channels::contract::channel_webhook_verifier_has_channel_type(&verifier);
    }

    #[test]
    fn contract_has_positive_max_age() {
        let verifier = TeamsChannelWebhookVerifier::new(None, false);
        leetium_channels::contract::channel_webhook_verifier_has_positive_max_age(&verifier);
    }

    #[test]
    fn contract_has_valid_rate_policy() {
        let verifier = TeamsChannelWebhookVerifier::new(None, false);
        leetium_channels::contract::channel_webhook_verifier_has_valid_rate_policy(&verifier);
    }
}
