...
use {
    ...
    leetium_oauth::{OAuthFlow, TokenStore, load_oauth_config},
...
impl OpenAiCodexProvider {
    ...
    async fn get_valid_tokens(&self) -> anyhow::Result<leetium_oauth::OAuthTokens> {
        let tokens = self
            .token_store
            .load("openai-codex")
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "not logged in to openai-codex — run `leetium auth login --provider openai-codex`"
                )
            })?;
    ...
    fn resolve_account_id(tokens: &leetium_oauth::OAuthTokens) -> anyhow::Result<String> {
                        ...
                        let content_blocks = match content {
                            ...
                            UserContent::Multimodal(parts) => {
                                ...
                                parts
                                    .iter()
                                    .map(|p| match p {
                                        leetium_agents::model::ContentPart::Text(t) => {
                                            serde_json::json!({"type": "input_text", "text": t})
                                        },
                                        leetium_agents::model::ContentPart::Image {
                                            media_type,
                                            data,
                                        } => {
                                            let data_uri =
...
fn load_codex_cli_tokens() -> Option<leetium_oauth::OAuthTokens> {
...
#[cfg(test)]
mod tests {
    use leetium_agents::model::UserContent;
    ...
    fn convert_messages_user_multimodal_with_image() {
        use leetium_agents::model::ContentPart;
