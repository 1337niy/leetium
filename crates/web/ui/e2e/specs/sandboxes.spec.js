...
test.describe("Sandboxes page – Image tag truncation", () => {
	test("long image hash tags are truncated in the cached images list", async ({ page }) => {
		...
		const fullTag = `leetium-sandbox:${longHash}`;
		...
		const truncated = `leetium-sandbox:${longHash.slice(0, 6)}\u2026${longHash.slice(-6)}`;
...
test.describe("Sandboxes page – Shared home settings", () => {
	test("shows shared folder status and saves updates", async ({ page }) => {
			...
			if (request.method() === "GET") {
				return route.fulfill({
					...
					body: JSON.stringify({
						enabled: true,
						mode: "shared",
						path: "/tmp/leetium-shared",
						configured_path: "/tmp/leetium-shared",
					}),
				});
			}
				...
				return route.fulfill({
					...
					body: JSON.stringify({
						...
						config: {
							enabled: false,
							mode: "off",
							path: "/tmp/leetium-new-shared",
							configured_path: "/tmp/leetium-new-shared",
						},
					}),
				});
			}
		...
		await expect(sharedHomeSection.getByLabel("Shared folder location")).toHaveValue("/tmp/leetium-shared");

		await sharedHomeSection.getByLabel("Enable shared home folder").uncheck();
		await sharedHomeSection.getByLabel("Shared folder location").fill("/tmp/leetium-new-shared");
		...
		await expect(
			sharedHomeSection.getByText("Saved. Restart Leetium to apply shared folder changes.", { exact: true }),
		).toBeVisible();
...
test.describe("Sandboxes page – Container error handling", () => {
	test("delete failure shows error message that clears on refresh", async ({ page }) => {
		...
		await page.route("**/api/sandbox/containers/leetium-sandbox-ghost", (route, request) => {
		...
		await expect(errorDiv).toContainText("Failed to delete leetium-sandbox-ghost");
