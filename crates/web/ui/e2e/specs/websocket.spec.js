...
test.describe("WebSocket connection lifecycle", () => {
	...
	test("final chat text is kept when it includes tool output plus analysis", async ({ page }) => {
		...
		const toolOutput = "Linux leetium-leetium-sandbox-main 6.12.28 #1 SMP Tue May 20 15:19:05 UTC 2025 aarch64 GNU/Linux";
		const finalText =
			"The command executed successfully. The output shows:\n- Kernel name: Linux\n- Hostname: leetium-leetium-sandbox-main\n\n" +
			toolOutput;
		...
		await expect(
			page.locator("#messages .msg.assistant").filter({ hasText: "leetium-leetium-sandbox-main" }),
		).toBeVisible();
		expect(pageErrors).toEqual([]);
	});
