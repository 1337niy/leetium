...
export default {
	...
	title: "沙盒",
	description:
		"leetium 缓存的用于沙盒执行的容器镜像。你可以删除单个镜像或清理全部。使用基础镜像和 apt 包构建自定义镜像。",
	appleContainerNote:
	...
	recommendation: {
		...
		macosDockerTip:
			"Apple Container 在 macOS 26+ 上提供更强的 VM 级隔离。安装后会自动使用（leetium 优先选择它而非 Docker）。运行：brew install container",
