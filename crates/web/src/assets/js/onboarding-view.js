...
function AuthStep({ onNext, skippable }) {
	...
	return html`<div class="flex flex-col gap-4">
		...
		${
			...
			<div class="text-xs text-[var(--muted)] mt-1">Find this code in the leetium process log (stdout).</div>
		</div>`
		}
...
function IdentityStep({ onNext, onBack }) {
	...
	var [name, setName] = useState(identity.name || "Leetium");
	...
	useEffect(() => {
		...
		refreshGon().then(() => {
			...
			if (refreshed.name) setName((prev) => (prev && prev !== "Leetium" ? prev : refreshed.name));
	...
	async function onSaveSelectedModels() {
		...
		try {
			...
			if (modelIds.length > 0) {
				localStorage.setItem("leetium-model", modelIds[0]);
			}
		...
		saveProviderKey(providerName, keyVal, endpointVal, modelVal)
			.then(async (res) => {
				...
				if (modelVal) {
					...
					localStorage.setItem("leetium-model", modelVal);
				}
...
function TeamsForm({ onConnected, error, setError }) {
	...
	return html`<form onSubmit=${onSubmit} class="flex flex-col gap-3 max-h-80 overflow-y-auto -mr-4 pr-4">
		<div class="rounded-md border border-[var(--border)] bg-[var(--surface2)] p-3 text-xs text-[var(--muted)] flex flex-col gap-1">
			...
			<span>3. Optional CLI shortcut: <code class="text-xs">leetium channels teams bootstrap</code>.</span>
		</div>
		<div>
...
function OpenClawImportStep({ onNext, onBack }) {
		...
		return html`<div class="flex flex-col gap-4">
			...
			${
				result.todos?.length > 0
					? html`<div class="text-xs text-[var(--muted)]">
						<div class="font-medium">Not yet supported in Leetium:</div>
	...
	return html`<div class="flex flex-col gap-4">
		...
		<p class="text-xs text-[var(--muted)] leading-relaxed">
			...
			Select the data you'd like to bring into Leetium.
		</p>
		<p class="text-xs text-[var(--muted)] leading-relaxed">
			This is a read-only copy \u2014 your OpenClaw installation will not be modified or removed.
			You can keep using OpenClaw alongside Leetium, and re-import at any time from Settings.
		</p>
		${
			workspaceMissing
...
function SummaryStep({ onBack, onFinish }) {
	...
	var activeModel = localStorage.getItem("leetium-model");
