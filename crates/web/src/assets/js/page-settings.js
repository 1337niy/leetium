...
function IdentitySection() {
	...
	var storedLocale = localStorage.getItem("leetium-locale");
	...
	function onApplyLanguage() {
		...
		setLocale(nextLanguage)
			.then(() => {
				if (uiLanguage === "auto") {
					localStorage.removeItem("leetium-locale");
				}
...
function SecuritySection() {
	...
	function notifyAuthStatusChanged() {
		window.dispatchEvent(new CustomEvent("leetium:auth-status-changed"));
	}

	function deferNextPasswordChangedRedirect() {
		window.__leetiumSuppressNextPasswordChangedRedirect = true;
	}
		...
		return html`<div class="flex-1 flex flex-col min-w-0 p-4 gap-4 overflow-y-auto">
			...
			<div style="max-width:600px;padding:12px 16px;border-radius:6px;border:1px solid var(--error);background:color-mix(in srgb, var(--error) 5%, transparent);">
				...
				<p class="text-xs text-[var(--muted)]" style="margin:8px 0 0;">
					Anyone with network access can control leetium and your computer. Set up a password to protect your instance.
				</p>
	...
	return html`<div class="flex-1 flex flex-col min-w-0 p-4 gap-4 overflow-y-auto">
			...
			authDisabled && localhostOnly
				? html`<div style="max-width:600px;padding:12px 16px;border-radius:6px;border:1px solid var(--error);background:color-mix(in srgb, var(--error) 5%, transparent);">
					...
					<p class="text-xs text-[var(--muted)]" style="margin:8px 0 0;">
						...
						Add credentials below to require login on localhost and before exposing Leetium to your network.
					</p>
				</div>`
				: null
		}
		...
		${
			localhostOnly && !hasPassword && !hasPasskeys && !authDisabled
				? html`<div class="alert-info-text max-w-form">
					...
					Add credentials to require login on localhost and before exposing Leetium to your network.
				</div>`
				: null
		}
		...
		<div style="max-width:600px;border-top:1px solid var(--border);padding-top:16px;">
			...
			<p class="text-xs text-[var(--muted)] leading-relaxed" style="margin:0 0 12px;">
				API keys authenticate external tools and scripts connecting to leetium over the WebSocket protocol. Pass the key as the <code style="font-family:var(--font-mono);font-size:.75rem;">api_key</code> field in the <code style="font-family:var(--font-mono);font-size:.75rem;">auth</code> object of the <code style="font-family:var(--font-mono);font-size:.75rem;">connect</code> handshake.
			</p>
			${
				akLoading
					? html`<div class="text-xs text-[var(--muted)]">Loading\u2026</div>`
		...
		${
			...
			<div style="padding:12px 16px;border:1px solid var(--error);border-radius:6px;background:color-mix(in srgb, var(--error) 5%, transparent);">
				...
				<p class="text-xs text-[var(--muted)]" style="margin:6px 0 0;">
					...
					Anyone with network access will be able to access leetium and your computer.
...
function ConfigSection() {
		...
		fetch("/api/config")
			...
			.catch((fetchErr) => {
				...
				if (errMsg.includes("pattern")) {
					errMsg = "Failed to connect to server. Please check if leetium is running.";
				}
				setErr(errMsg);
				setConfigLoading(false);
				rerender();
			...
			fetch("/api/gon", { method: "GET" })
				.then((r) => {
					if (r.ok) {
						...
						setRestarting(false);
						setErr("Server did not come back up. Check if leetium is running.");
						rerender();
					}
				})
				.catch(() => {
	...
	return html`<div class="flex-1 flex flex-col min-w-0 p-4 gap-4 overflow-y-auto">
		...
		<p class="text-xs text-[var(--muted)] leading-relaxed" style="max-width:700px;margin:0;">
			Edit the full leetium configuration. This includes server, tools, LLM providers, auth, and all other settings.
			Test your changes before saving. Changes require a restart to take effect.${" "}
			<a href="https://docs.leetnex.ru/configuration.html" target="_blank" rel="noopener"
			...
			<div>
				...
				${
					...
					<div class="text-xs text-[var(--error)]" style="margin-top:8px;">
						QMD feature is not enabled. Rebuild leetium with <code style="font-family:var(--font-mono);font-size:.7rem;">--features qmd</code>
					</div>
				`
				}
	...
	return html`<div class="flex-1 flex flex-col min-w-0 p-4 gap-4 overflow-y-auto">
		...
		${
			...
			<div style="max-width:600px;padding:12px 16px;border-radius:6px;border:1px solid var(--border);background:var(--surface);">
				...
				<p class="text-xs text-[var(--muted)]" style="margin:8px 0 0;">
					On Safari, push notifications are only available for installed apps. Add leetium to your Dock using <strong>File → Add to Dock</strong> (or Share → Add to Dock on iOS), then open it from there.
				</p>
			</div>
		`
				: null
