...
function saveSharedHomeConfig() {
	...
	fetch("/api/sandbox/shared-home", {
		...
		.then((data) => {
			...
			sharedHomeMsg.value = "Saved. Restart Leetium to apply shared folder changes.";
...
function backendRecommendation(info) {
	...
	if (os === "macos" && backend === "docker") {
		return {
			level: "info",
			text: "Apple Container provides stronger VM-level isolation on macOS 26+. Install it for automatic use (leetium prefers it over Docker). Run: brew install container",
		};
	}
...
function ImagesPage() {
    ...
    <div class="flex-1 flex flex-col min-w-0 p-4 gap-4 overflow-y-auto">
      ...
      <p class="text-sm text-[var(--muted)] leading-relaxed max-w-form" style="margin:0;">
        Container images cached by leetium for sandbox execution. You can delete individual images or prune all. Build custom images from a base with apt packages.
