...
use leetium_gateway::share_store::{ShareSnapshot, ShareVisibility, SharedMessageRole};
...
pub fn render_share_html(
    ...
    identity: &leetium_config::ResolvedIdentity,
    share_id: &str,
...
fn first_share_message_preview(snapshot: &ShareSnapshot) -> String {
    ...
    if out.is_empty() {
        "Shared conversation snapshot from Leetium".to_string()
    } else {
...
fn share_user_label(identity: &leetium_config::ResolvedIdentity) -> String {
    identity
        .user_name
        .as_deref()
        .map(str::trim)
...
fn share_assistant_label(identity: &leetium_config::ResolvedIdentity) -> String {
...
fn image_dimensions_from_data_url(data_url: &str) -> Option<(u32, u32)> {
    ...
    let bytes = base64::engine::general_purpose::STANDARD
        .decode(body.trim())
        .ok()?;
    let metadata = leetium_media::image_ops::get_image_metadata(&bytes).ok()?;
...
{}\
<text x=\"1122\" y=\"584\" text-anchor=\"end\" fill=\"#9ca3af\" font-size=\"22\" font-family=\"Inter, system-ui, sans-serif\">By Leetium</text>\
</svg>",
...
mod tests {
    use {
        super::*,
        leetium_gateway::share_store::{SharedMessage, SharedMessageRole},
    };

    fn default_identity() -> leetium_config::ResolvedIdentity {
        leetium_config::ResolvedIdentity {
            name: "Leetium".to_owned(),
    ...
    fn render_share_og_svg_produces_svg() {
        ...
        assert!(svg.contains("By Leetium"));
    }
    ...
    fn share_template_renders_theme_toggle_and_audio() {
        let messages = vec![ShareMessageView {
            ...
            role_label: "\u{1F916} Leetium".to_string(),
        ...
        let template = ShareHtmlTemplate {
            ...
            share_image_url: "https://www.leetnex.ru/og-social.jpg?v=4",
            share_image_alt: "alt",
            assistant_name: "Leetium",
    ...
    fn map_share_message_views_skips_system_and_notice() {
        let identity = leetium_config::ResolvedIdentity {
            name: "Leetium".to_owned(),
    ...
    fn share_labels_use_identity_user_and_emoji() {
        ...
        assert_eq!(share_assistant_label(&identity), "\u{1F916} Leetium");
    }
    ...
    fn share_labels_fallback_when_identity_fields_missing() {
        ...
        assert_eq!(share_assistant_label(&identity), "leetium");
    }
    ...
    fn map_share_message_views_includes_tool_result_media_and_links() {
        use leetium_gateway::share_store::{SharedImageAsset, SharedImageSet, SharedMapLinks};
