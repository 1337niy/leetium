...
fn share_cookie_name(share_id: &str) -> String {
    format!("leetium_share_{}", share_id)
}
...
pub async fn share_page_handler(
    ...
    let static_path = leetium_config::data_dir()
        .join("shares")
    ...
    let cookie_access_granted = jar.get(&cookie_name).is_some_and(|cookie| {
        leetium_gateway::share_store::ShareStore::verify_access_key(&share, cookie.value())
    });
    let query_access_granted = query
        .k
        .as_deref()
        .is_some_and(|key| leetium_gateway::share_store::ShareStore::verify_access_key(&share, key));

    if share.visibility == leetium_gateway::share_store::ShareVisibility::Private
    ...
    let snapshot: leetium_gateway::share_store::ShareSnapshot =
...
fn serve_static_share_html(body: String) -> axum::response::Response {
    ...
    let csp = "default-src 'none'; \
               ...
               img-src 'self' data: https://www.leetnex.ru; \
               media-src 'self' data:; \
