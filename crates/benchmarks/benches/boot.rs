...
fn config_template_generation() -> String {
    divan::black_box(leetium_config::template::default_config_template(8080))
}

/// Benchmark constructing a `LeetiumConfig` with all defaults.
#[divan::bench]
fn config_default_construction() -> leetium_config::LeetiumConfig {
    divan::black_box(leetium_config::LeetiumConfig::default())
}

/// Benchmark loading + parsing a TOML config from disk (the full boot path).
#[divan::bench]
fn config_load_toml(bencher: divan::Bencher) {
    let toml_content = leetium_config::template::default_config_template(8080);
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("leetium.toml");
    std::fs::write(&path, &toml_content).unwrap();

    bencher.bench_local(|| divan::black_box(leetium_config::loader::load_config(&path).unwrap()));
}

/// Benchmark config round-trip: serialize LeetiumConfig to TOML, then deserialize.
#[divan::bench]
fn config_serde_roundtrip() {
    let config = leetium_config::LeetiumConfig::default();
    let toml_str = divan::black_box(toml::to_string_pretty(&config).unwrap());
    let _: leetium_config::LeetiumConfig = divan::black_box(toml::from_str(&toml_str).unwrap());
}
...
fn config_validate_toml(bencher: divan::Bencher) {
    ...
    bencher.bench_local(|| {
        divan::black_box(leetium_config::validate::validate_toml_str(&toml_content))
    });
}
...
fn context_window_lookup(model_id: &str) -> u32 {
    divan::black_box(leetium_providers::context_window_for_model(model_id))
}

#[divan::bench(args = MODEL_IDS)]
fn vision_support_lookup(model_id: &str) -> bool {
    divan::black_box(leetium_providers::supports_vision_for_model(model_id))
}

#[divan::bench]
fn namespaced_model_id() -> String {
    divan::black_box(leetium_providers::namespaced_model_id("openai", "gpt-4o"))
}
...
fn session_key_to_filename(key: &str) -> String {
    divan::black_box(leetium_sessions::store::SessionStore::key_to_filename(key))
}
...
fn sanitize_tool_result(bencher: divan::Bencher, payload_bytes: usize) {
    ...
    bencher.bench_local(|| {
        divan::black_box(leetium_agents::runner::sanitize_tool_result(&input, 50_000))
    });
}
...
fn tool_result_to_content_vision(bencher: divan::Bencher, payload_bytes: usize) {
    let input = build_sanitize_input(payload_bytes);
    bencher.bench_local(|| {
        divan::black_box(leetium_agents::runner::tool_result_to_content(
            &input, 50_000, true,
        ))
    });
}
        ...
        match i % 6 {
            ...
            2 => values.push(serde_json::json!({
                ...
                "tool_calls": [{
                    ...
                    "function": {
                        "name": "web.search",
                        "arguments": r#"{"q":"leetium release notes"}"#,
                    }
                }],
            })),
...
fn values_to_chat_messages(bencher: divan::Bencher, n: usize) {
    ...
    bencher
        .bench_local(|| divan::black_box(leetium_agents::model::values_to_chat_messages(&values)));
}
...
fn env_substitution(bencher: divan::Bencher) {
    let input = r#"
        api_key = "${LEETIUM_API_KEY}"
        base_url = "${LEETIUM_BASE_URL:-https://api.example.com}"
        name = "no-vars-here"
        port = 8080
    "#;

    bencher.bench_local(|| divan::black_box(leetium_config::env_subst::substitute_env(input)));
}
...
fn full_config_boot_path(bencher: divan::Bencher) {
    ...
    bencher.bench_local(|| {
        let config = leetium_config::loader::load_config(&path).unwrap();
        let _ = leetium_config::validate::validate_toml_str(&toml_content);
