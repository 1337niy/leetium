...
const WORKTREE_DIR: &str = ".leetium-worktrees";
...
impl WorktreeManager {
    ...
    pub async fn create(project_dir: &Path, session_id: &str) -> Result<PathBuf> {
        ...
        let branch = format!("leetium/{session_id}");
    ...
    ///
    /// Injects `LEETIUM_WORKSPACE_NAME` (session id) and `LEETIUM_ROOT_PATH`
    ...
    pub async fn run_setup(
        ...
        let output = Command::new("sh")
            ...
            .current_dir(wt_dir)
            .env("LEETIUM_WORKSPACE_NAME", session_id)
            .env("LEETIUM_ROOT_PATH", project_dir.as_os_str())
            .output()
            .await
...
/// Copy `.leetium/` config directory from project root to a worktree, if it
/// exists in the project but not in the worktree.
pub fn copy_project_config(project_dir: &Path, wt_dir: &Path) -> Result<()> {
    let src = project_dir.join(".leetium");
    let dst = wt_dir.join(".leetium");
...
mod tests {
    ...
    async fn test_create_and_list_worktree() {
        ...
        assert_eq!(list[0].branch, "leetium/test-session");
    }
    ...
    async fn test_run_setup_with_env_vars() {
        ...
        let cmd = format!(
            "echo \"$LEETIUM_WORKSPACE_NAME:$LEETIUM_ROOT_PATH\" > {}",
            marker.display()
        );
    ...
    async fn test_copy_project_config() {
        ...
        // Create .leetium/config.toml in project root.
        let config_dir = dir.path().join(".leetium");
        std::fs::create_dir_all(&config_dir).unwrap();
        std::fs::write(config_dir.join("config.toml"), "key = 'value'").unwrap();

        copy_project_config(dir.path(), &wt).unwrap();

        assert!(wt.join(".leetium/config.toml").exists());
        let content = std::fs::read_to_string(wt.join(".leetium/config.toml")).unwrap();
    ...
    async fn test_copy_project_config_skips_if_exists() {
        ...
        // Create .leetium in both project and worktree.
        std::fs::create_dir_all(dir.path().join(".leetium")).unwrap();
        std::fs::write(dir.path().join(".leetium/config.toml"), "original").unwrap();
        std::fs::create_dir_all(wt.join(".leetium")).unwrap();
        std::fs::write(wt.join(".leetium/config.toml"), "existing").unwrap();
    ...
    async fn test_create_from_base() {
        ...
        let list = WorktreeManager::list(dir.path()).await.unwrap();
        assert_eq!(list.len(), 1);
        assert_eq!(list[0].branch, "leetium/base-test");
    }
}
