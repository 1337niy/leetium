...
pub fn check_hook_eligibility(metadata: &HookMetadata) -> EligibilityResult {
    ...
    // Config check is skipped for now (requires LeetiumConfig access).
...
mod tests {
    ...
    fn missing_env_is_ineligible() {
        let mut meta = minimal_metadata();
        meta.requires.env = vec!["LEETIUM_TEST_NONEXISTENT_VAR_XYZ".into()];
        let result = check_hook_eligibility(&meta);
        assert!(!result.eligible);
        assert_eq!(result.missing_env, vec!["LEETIUM_TEST_NONEXISTENT_VAR_XYZ"]);
    }
