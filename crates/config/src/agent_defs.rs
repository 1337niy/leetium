...
//!
//! Scans `~/.leetium/agents/` and `.leetium/agents/` for `.md` files,
...
///
/// Scans `~/.leetium/agents/` (user-global) then `.leetium/agents/` (project-local).
...
pub fn discover_agent_defs() -> HashMap<String, AgentPreset> {
    ...
    // User-global: ~/.leetium/agents/
    let user_dir = crate::loader::data_dir().join("agents");
    load_defs_from_dir(&user_dir, &mut defs);

    // Project-local: .leetium/agents/
    let project_dir = std::path::PathBuf::from(".leetium").join("agents");
