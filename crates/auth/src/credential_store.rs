...
impl CredentialStore {
    ...
    pub async fn with_config(
        pool: SqlitePool,
        auth_config: &leetium_config::AuthConfig,
    ) -> anyhow::Result<Self> {
        let store = Self {
            pool,
    ...
    async fn persist_auth_disabled(&self, disabled: bool) -> anyhow::Result<()> {
        ...
        .execute(&self.pool)
        .await?;
        leetium_config::update_config(|c| c.auth.disabled = disabled)?;
        Ok(())
    }
...
impl leetium_tools::exec::EnvVarProvider for CredentialStore {
...
mod tests {
    ...
    async fn vault_store(password: &str) -> (CredentialStore, Arc<Vault>) {
        ...
        let store = CredentialStore::with_vault(
            pool,
            &leetium_config::AuthConfig::default(),
            Some(vault.clone()),
        )
        .await
        .unwrap();
