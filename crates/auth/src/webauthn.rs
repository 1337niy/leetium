...
impl WebAuthnState {
    ...
    ///
    /// `rp_id` is typically the hostname (e.g. "localhost" or "leetium.example.com").
...
mod tests {
    ...
    fn normalize_host_strips_port_and_lowercases() {
        ...
        assert_eq!(normalize_host("leetium.local"), "leetium.local");
    ...
    fn registry_matches_extra_origin_host() {
        ...
        let extra = vec![Url::parse("https://leetium.localhost:18080").unwrap()];
        ...
        assert!(registry.get_for_host("leetium.localhost:18080").is_some());
        assert!(registry.contains_host("leetium.localhost"));
    }
}
