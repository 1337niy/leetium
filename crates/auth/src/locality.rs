...
mod tests {
    ...
    fn is_loopback_host_variants() {
        ...
        assert!(is_loopback_host("leetium.localhost"));
        assert!(is_loopback_host("leetium.localhost:8080"));

        assert!(!is_loopback_host("example.com"));
        assert!(!is_loopback_host("example.com:18789"));
        assert!(!is_loopback_host("192.168.1.1:18789"));
        assert!(!is_loopback_host("leetium.example.com"));
    }
    ...
    fn not_local_when_host_is_external() {
        ...
        headers.insert(
            ...
            "leetium.example.com".parse().unwrap(),
        );
        assert!(!is_local_connection(&headers, addr, false));
    }
    ...
    /// loopback. Without `LEETIUM_BEHIND_PROXY`, this would appear local.
    #[test]
    fn bare_proxy_without_env_var_appears_local() {
        ...
        assert!(is_local_connection(&headers, addr, false));
        // Setting LEETIUM_BEHIND_PROXY fixes it.
        assert!(!is_local_connection(&headers, addr, true));
    }
