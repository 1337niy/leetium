...
pub async fn start_socket_mode(
    ...
    config: SlackAccountConfig,
    accounts: AccountStateMap,
    message_log: Option<Arc<dyn leetium_channels::message_log::MessageLog>>,
    event_sink: Option<Arc<dyn leetium_channels::ChannelEventSink>>,
...
pub(crate) async fn handle_inbound(
    ...
    if !is_dm {
        match config.mention_mode {
            leetium_channels::gating::MentionMode::Mention => {
                if !is_mention {
                    return;
                }
            },
            leetium_channels::gating::MentionMode::None => return,
            leetium_channels::gating::MentionMode::Always => {},
        }
    }
    ...
    if let Some(sink) = &event_sink {
        ...
        leetium_metrics::counter!(
            ...
            leetium_metrics::labels::CHANNEL => "slack"
        )
        .increment(1);
