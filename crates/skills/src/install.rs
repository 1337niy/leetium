...
#[cfg(feature = "metrics")]
use leetium_metrics::{counter, histogram, skills as skills_metrics};
...
pub async fn install_skill(source: &str, install_dir: &Path) -> anyhow::Result<Vec<SkillMetadata>> {
    ...
    #[cfg(feature = "metrics")]
    counter!("leetium_skills_git_clone_fallback_total").increment(1);
...
async fn install_via_http(
    ...
    let resp = client
        .get(&url)
        .header("User-Agent", "leetium-skills")
        .send()
        .await?;
...
pub fn default_install_dir() -> anyhow::Result<PathBuf> {
    Ok(leetium_config::data_dir().join("installed-skills"))
}
