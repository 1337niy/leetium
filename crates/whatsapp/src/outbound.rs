...
use {
    ...
    wa_rs_binary::jid::Jid,
    wa_rs_proto::whatsapp as wa,
    wa_rs::{ChatStateType, upload::UploadResponse},
};
...
fn decode_data_url(url: &str) -> ChannelResult<Vec<u8>> {
    let comma_pos = url
        .find(',')
        .ok_or_else(|| leetium_channels::Error::invalid_input("invalid data URI: no comma"))?;
    let base64_data = &url[comma_pos + 1..];
    base64::engine::general_purpose::STANDARD
        .decode(base64_data)
        .map_err(|e| leetium_channels::Error::invalid_input(format!("base64 decode: {e}")))
}
...
impl WhatsAppOutbound {
    fn get_client(
        &self,
        account_id: &str,
    ) -> ChannelResult<std::sync::Arc<wa_rs::client::Client>> {
        let accounts = self.accounts.read().unwrap_or_else(|e| e.into_inner());
        accounts
            .get(account_id)
            .map(|s| std::sync::Arc::clone(&s.client))
            .ok_or_else(|| leetium_channels::Error::unknown_account(account_id))
    }
...
impl ChannelOutbound for WhatsAppOutbound {
    async fn send_text(
        ...
        let jid: Jid = to
            .parse()
            .map_err(|e| leetium_channels::Error::invalid_input(format!("invalid JID: {e:?}")))?;
        ...
        let msg_id = client
            ...
            .await
            .map_err(|e| leetium_channels::Error::unavailable(format!("whatsapp send_text: {e}")))?;
        self.record_sent_id(account_id, &msg_id);

        #[cfg(feature = "metrics")]
        leetium_metrics::counter!(
            leetium_metrics::channels::MESSAGES_SENT_TOTAL,
    ...
    async fn send_media(
        ...
        let upload = client.upload(bytes, media_type).await.map_err(|e| {
            leetium_channels::Error::unavailable(format!("whatsapp media upload: {e}"))
        })?;

        let msg = build_media_message(&media.mime_type, caption, &upload);

        let msg_id = client.send_message(jid, msg).await.map_err(|e| {
            leetium_channels::Error::unavailable(format!("whatsapp send_media: {e}"))
        })?;
        self.record_sent_id(account_id, &msg_id);
    ...
    async fn send_typing(&self, account_id: &str, to: &str) -> ChannelResult<()> {
        ...
        client
            ...
            .await
            .map_err(|e| leetium_channels::Error::unavailable(format!("whatsapp chatstate: {e}")))?;
        Ok(())
    }
}
