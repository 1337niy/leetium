...
fn status() -> Result<()> {
    ...
    let detected = leetium_browser::detect::detect_browsers(browser.chrome_path.as_deref());
    if detected.is_empty() {
        println!("  detected:       none");
    } else {
        let labels = leetium_browser::detect::installed_browser_labels(&detected);
...
fn enable() -> Result<()> {
    let path = leetium_config::update_config(|config| {
