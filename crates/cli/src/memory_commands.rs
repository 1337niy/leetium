...
fn memory_db_path() -> std::path::PathBuf {
    leetium_config::data_dir().join("memory.db")
}
...
async fn search_memory(query: &str, limit: usize, json: bool) -> anyhow::Result<()> {
    ...
    let store = leetium_memory::store_sqlite::SqliteMemoryStore::new(pool);
    let results = leetium_memory::search::keyword_only_search(&store, query, limit).await?;
...
fn print_json(results: &[leetium_memory::search::SearchResult]) -> anyhow::Result<()> {
...
fn print_human(results: &[leetium_memory::search::SearchResult]) {
...
async fn show_status() -> anyhow::Result<()> {
    ...
    let manager = leetium_memory::manager::MemoryManager::keyword_only(config, Box::new(store));
...
mod tests {
    ...
    async fn test_search_with_results() {
        ...
        leetium_memory::schema::run_migrations(&pool).await.unwrap();
        ...
        let store = leetium_memory::store_sqlite::SqliteMemoryStore::new(pool);
        let results = leetium_memory::search::keyword_only_search(&store, "rust", 5)
            .await
            .unwrap();
    ...
    async fn test_search_no_results() {
        ...
        let pool = open_memory_pool().await.unwrap();
        let store = leetium_memory::store_sqlite::SqliteMemoryStore::new(pool);
        let results = leetium_memory::search::keyword_only_search(&store, "nonexistent", 5)
            .await
            .unwrap();
    ...
    async fn test_status_with_db() {
        ...
        let store = leetium_memory::store_sqlite::SqliteMemoryStore::new(ro_pool);
        let config = leetium_memory::config::MemoryConfig {
            db_path: db_path.to_string_lossy().to_string(),
            ..Default::default()
        };
    ...
    fn test_print_json_output() {
        use leetium_memory::search::SearchResult;
