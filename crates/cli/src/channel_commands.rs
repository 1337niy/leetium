...
pub struct TeamsBootstrapArgs {
    /// Local account key used by Leetium and in the webhook path.
    #[arg(long)]
    ...
    /// Public base URL of your Leetium instance (for webhook endpoint generation).
    #[arg(long)]
    ...
    force: bool,
    /// Print generated values without writing `leetium.toml`.
    #[arg(long, default_value_t = false)]
    dry_run: bool,
...
fn run_teams_bootstrap(args: TeamsBootstrapArgs) -> Result<()> {
    ...
    let raw_base_url = match args.base_url {
        Some(value) if !value.trim().is_empty() => value.trim().to_string(),
        _ => prompt_required(
            "Public base URL for this Leetium instance",
    ...
    let path = leetium_config::update_config(move |cfg| {
        cfg.channels
            .msteams
    ...
    println!("Restart Leetium gateway to apply channel changes.");
    println!();
    print_setup_links();
...
fn default_gateway_base_url(config: &leetium_config::LeetiumConfig) -> String {
