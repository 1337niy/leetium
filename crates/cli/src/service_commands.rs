//! `leetium service` subcommands — install/manage the gateway as an OS service.
//!
//! - **macOS**: launchd user agent (`~/Library/LaunchAgents/org.leetium.plist`)
//! - **Linux**: systemd user unit (`~/.config/systemd/user/leetium.service`)
...
/// `leetium service` subcommands.
#[derive(Subcommand)]
pub enum ServiceAction {
    /// Install leetium as an OS service (launchd on macOS, systemd on Linux).
    Install {
    ...
    /// Uninstall the leetium service.
    Uninstall,

    /// Show the current status of the leetium service.
    Status,

    /// Stop the leetium service.
    Stop,

    /// Restart the leetium service.
    Restart,
...
pub fn handle_service(action: ServiceAction) -> Result<()> {
    match action {
        ServiceAction::Install {
            ...
            let log_path = data_dir.join("leetium.log");
            ...
            if cfg!(target_os = "macos") {
                install_launchd(&leetium_bin, &opts, &log_path)?;
            } else if cfg!(target_os = "linux") {
                install_systemd(&leetium_bin, &opts, &log_path)?;
            } else {
                anyhow::bail!("service install not supported on {}", std::env::consts::OS);
            }

            println!("Leetium service installed and started.");
        ...
        ServiceAction::Uninstall => {
            ...
            println!("Leetium service uninstalled.");
            Ok(())
        },
        ...
        ServiceAction::Status => {
            ...
            println!("Leetium service: {status}");
            Ok(())
        },
        ...
        ServiceAction::Stop => {
            ...
            println!("Leetium service stopped.");
            Ok(())
        },
        ...
        ServiceAction::Restart => {
            ...
            println!("Leetium service restarted.");
            Ok(())
        },

        ServiceAction::Logs => {
            let data_dir = leetium_config::data_dir();
            println!("{}", data_dir.join("leetium.log").display());
            Ok(())
        },
    }
}
...
const LAUNCHD_LABEL: &str = "org.leetium.gateway";
const SYSTEMD_UNIT: &str = "leetium.service";
...
fn generate_launchd_plist(leetium_bin: &Path, opts: &GatewayServiceOpts, log_path: &Path) -> String {
...
fn install_launchd(leetium_bin: &Path, opts: &GatewayServiceOpts, log_path: &Path) -> Result<()> {
    ...
    let plist = generate_launchd_plist(leetium_bin, opts, log_path);
...
fn generate_systemd_unit(leetium_bin: &Path, opts: &GatewayServiceOpts, log_path: &Path) -> String {
    let bin = leetium_bin.display();
    let log = log_path.display();
...
fn install_systemd(leetium_bin: &Path, opts: &GatewayServiceOpts, log_path: &Path) -> Result<()> {
    ...
    let unit = generate_systemd_unit(leetium_bin, opts, log_path);
...
mod tests {
    ...
    fn launchd_plist_basic() {
        let bin = PathBuf::from("/opt/homebrew/bin/leetium");
        let opts = GatewayServiceOpts {
            bind: None,
            port: None,
            log_level: "info".into(),
        };
        let log = PathBuf::from("/tmp/leetium.log");

        let plist = generate_launchd_plist(&bin, &opts, &log);

        assert!(plist.starts_with("<?xml"));
        assert!(plist.contains("org.leetium.gateway"));
        assert!(plist.contains("/opt/homebrew/bin/leetium"));
        assert!(plist.contains("--log-level"));
        assert!(plist.contains("info"));
        assert!(plist.contains("<key>RunAtLoad</key>"));
        assert!(plist.contains("<key>KeepAlive</key>"));
        assert!(plist.contains("/tmp/leetium.log"));
        assert!(plist.contains("</plist>"));
        // Args should NOT include a "gateway" subcommand — just `leetium` directly.
    ...
    fn systemd_unit_basic() {
        ...
        let log = PathBuf::from("/var/log/leetium.log");
        ...
        assert!(unit.contains("Leetium Gateway"));
        assert!(unit.contains("/usr/bin/leetium --log-level info"));
        assert!(unit.contains("Restart=on-failure"));
        assert!(unit.contains("RestartSec=10"));
        assert!(unit.contains("/var/log/leetium.log"));
