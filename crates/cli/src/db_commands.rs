...
#[derive(Subcommand)]
pub enum DbAction {
    /// Delete all database files completely (leetium.db and memory.db).
    Reset,
...
fn db_paths() -> (PathBuf, PathBuf) {
    ...
    let main_db = data_dir.join("leetium.db");
...
async fn reset_databases() -> anyhow::Result<()> {
    ...
    if deleted {
        println!("Database files deleted. Run `leetium db migrate` to recreate them.");
    } else {
...
async fn run_migrations() -> anyhow::Result<()> {
    ...
    leetium_memory::schema::run_migrations(&memory_pool)
        .await
...
mod tests {
    ...
    fn test_db_paths_uses_data_dir() {
        ...
        let paths = db_paths();
        assert!(
            paths.0.to_string_lossy().contains("leetium.db"),
            "main db path should contain leetium.db"
        );
        assert!(
    ...
    async fn test_reset_deletes_files() {
        ...
        let main_db = temp.path().join("leetium.db");
        let memory_db = temp.path().join("memory.db");
        let main_wal = temp.path().join("leetium.db-wal");
        let main_shm = temp.path().join("leetium.db-shm");
    ...
    async fn test_migrations_run_successfully_in_temp_dir() {
        ...
        leetium_sessions::run_migrations(&pool).await.unwrap();
        leetium_cron::run_migrations(&pool).await.unwrap();
        leetium_gateway::run_migrations(&pool).await.unwrap();
