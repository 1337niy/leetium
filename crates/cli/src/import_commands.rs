...
fn handle_detect(json_output: bool) -> anyhow::Result<()> {
    let Some(detection) = leetium_openclaw_import::detect() else {
        if json_output {
    ...
    let scan = leetium_openclaw_import::scan(&detection);
    if json_output {
        return print_json(serde_json::json!({
            "detected": true,
...
fn handle_import_all(dry_run: bool, json_output: bool) -> anyhow::Result<()> {
    ...
    let report = leetium_openclaw_import::import(
        &detection,
        &leetium_openclaw_import::ImportSelection::all(),
        &config_dir,
        &data_dir,
    );
...
fn handle_import_select(
    ...
    let report =
        leetium_openclaw_import::import(&detection, &parsed.selection, &config_dir, &data_dir);
...
struct ParsedSelection {
    selection: leetium_openclaw_import::ImportSelection,
    unknown_categories: Vec<String>,
}

fn parse_selection(categories: &[String], warn_unknown: bool) -> ParsedSelection {
    let mut sel = leetium_openclaw_import::ImportSelection::default();
...
fn print_scan_summary(scan: &leetium_openclaw_import::ImportScan) {
    print_scan_item("Identity", scan.identity_available, None);
    print_scan_item("Providers", scan.providers_available, None);
    print_scan_item(
        "Skills",
...
fn print_selection(sel: &leetium_openclaw_import::ImportSelection) {
    let items = [
...
fn format_channel_scan_detail(scan: &leetium_openclaw_import::ImportScan) -> Option<String> {
...
fn print_report(report: &leetium_openclaw_import::report::ImportReport) {
    use leetium_openclaw_import::report::ImportStatus;
    ...
    if !report.todos.is_empty() {
        println!();
        println!("TODO (not yet supported in Leetium):");
