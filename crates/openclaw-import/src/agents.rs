...
pub struct ImportedAgent {
    ...
    /// Sanitized Leetium-side agent ID; `"main"` for the default agent.
    pub leetium_id: String,
    pub is_default: bool,
...
/// Sanitize an OpenClaw agent ID into a valid Leetium agent ID.
///
...
/// theme. The default agent gets `leetium_id = "main"`.
...
mod tests {
    ...
    fn import_agents_single_default() {
        ...
        assert_eq!(result.agents[0].leetium_id, "main");
    ...
    fn import_agents_multiple() {
        ...
        assert_eq!(result.agents[0].leetium_id, "main");
        assert!(result.agents[0].is_default);
        assert_eq!(result.agents[1].leetium_id, "research");
    ...
    fn import_agents_synthesizes_from_detection() {
        ...
        assert_eq!(result.agents[0].leetium_id, "main");
        assert!(result.agents[0].is_default);
        assert_eq!(result.agents[1].leetium_id, "helper");
        assert!(!result.agents[1].is_default);
    }
