...
async fn root_redirects_to_onboarding_when_not_onboarded() {
    ...
    assert!(body.contains("<title>leetium onboarding</title>"));
...
async fn gateway_startup_with_llm_wiring_does_not_block() {
    ...
    let session_store1 = Arc::new(leetium_sessions::store::SessionStore::new(
    ...
    sqlx::query("CREATE TABLE IF NOT EXISTS projects (id TEXT PRIMARY KEY)")
        ...
        .await
        .unwrap();
    leetium_sessions::metadata::SqliteSessionMetadata::init(&db_pool1)
        .await
        .unwrap();
    let session_metadata1 = Arc::new(leetium_sessions::metadata::SqliteSessionMetadata::new(
        db_pool1,
    ));
    if !registry.read().await.is_empty() {
        state
            .set_chat(Arc::new(LiveChatService::new(
                Arc::clone(&registry),
                Arc::new(tokio::sync::RwLock::new(DisabledModelsStore::default())),
                leetium_gateway::chat::GatewayChatRuntime::from_state(Arc::clone(&state)),
    ...
    let session_store2 = Arc::new(leetium_sessions::store::SessionStore::new(
    ...
    sqlx::query("CREATE TABLE IF NOT EXISTS projects (id TEXT PRIMARY KEY)")
        ...
        .await
        .unwrap();
    leetium_sessions::metadata::SqliteSessionMetadata::init(&db_pool2)
        .await
        .unwrap();
    let session_metadata2 = Arc::new(leetium_sessions::metadata::SqliteSessionMetadata::new(
        db_pool2,
    ));
    state2
        .set_chat(Arc::new(LiveChatService::new(
            Arc::clone(&registry2),
            Arc::new(tokio::sync::RwLock::new(DisabledModelsStore::default())),
            leetium_gateway::chat::GatewayChatRuntime::from_state(Arc::clone(&state2)),
