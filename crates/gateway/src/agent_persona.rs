...
pub enum AgentError {
    ...
    #[error(transparent)]
    Config(#[from] leetium_config::Error),
}

impl From<AgentError> for leetium_protocol::ErrorShape {
    fn from(err: AgentError) -> Self {
        use leetium_protocol::error_codes;
        match &err {
...
impl AgentPersonaStore {
    ...
    pub fn ensure_main_workspace_seeded(&self) -> Result<PathBuf> {
        let main_workspace = leetium_config::agent_workspace_dir("main");
        ...
        for file_name in &[
            ...
            let src = leetium_config::data_dir().join(file_name);
        ...
        let src_memory_dir = leetium_config::data_dir().join("memory");
    ...
    pub async fn create(&self, params: CreateAgentParams) -> Result<AgentPersona> {
        ...
        leetium_config::save_identity_for_agent(&params.id, &identity)?;
    ...
    pub async fn update(&self, id: &str, params: UpdateAgentParams) -> Result<AgentPersona> {
        ...
        leetium_config::save_identity_for_agent(id, &identity)?;
    ...
    pub async fn delete(&self, id: &str) -> Result<()> {
        ...
        let workspace = leetium_config::agent_workspace_dir(id);
        if workspace.exists() {
...
fn synthesize_main_agent(is_default: bool) -> AgentPersona {
    let identity =
        leetium_config::load_identity_for_agent("main").or_else(leetium_config::load_identity);
    AgentPersona {
        id: "main".to_string(),
        name: identity
            .as_ref()
