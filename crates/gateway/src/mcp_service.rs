...
use {
    leetium_agents::tool_registry::{AgentTool, ToolRegistry},
    leetium_mcp::tool_bridge::{McpAgentTool, McpToolBridge},
};
...
pub async fn sync_mcp_tools(
    manager: &leetium_mcp::McpManager,
...
fn parse_server_config(
    params: &Value,
    existing: Option<&leetium_mcp::McpServerConfig>,
) -> Result<leetium_mcp::McpServerConfig, ServiceError> {
    let transport = match params.get("transport").and_then(|v| v.as_str()) {
        Some("sse") => leetium_mcp::TransportType::Sse,
        Some(_) => leetium_mcp::TransportType::Stdio,
        None => existing
            .map(|cfg| cfg.transport)
            .unwrap_or(leetium_mcp::TransportType::Stdio),
    };
    ...
    if matches!(transport, leetium_mcp::TransportType::Stdio) && command.trim().is_empty() {
    ...
    if matches!(transport, leetium_mcp::TransportType::Sse)
        && url
            .as_deref()
    ...
    let oauth = if let Some(v) = params.get("oauth") {
        if v.is_null() {
            ...
            let scopes: Vec<String> = v
                ...
                .unwrap_or_default();
            Some(leetium_mcp::registry::McpOAuthConfig {
                client_id,
                auth_url,
                token_url,
                scopes,
    ...
    Ok(leetium_mcp::McpServerConfig {
        command,
        args,
        env,
        enabled,
...
pub struct LiveMcpService {
    manager: Arc<leetium_mcp::McpManager>,
...
impl LiveMcpService {
    pub fn new(manager: Arc<leetium_mcp::McpManager>) -> Self {
        Self {
            manager,
    ...
    pub fn manager(&self) -> &Arc<leetium_mcp::McpManager> {
        &self.manager
    }
}
    ...
    async fn add(&self, params: Value) -> ServiceResult {
            ...
            .await
        {
            ...
            Err(e) => {
                if matches!(
                    e,
                    leetium_mcp::Error::Manager(leetium_mcp::McpManagerError::OAuthRequired { .. })
                ) {
...
#[cfg(test)]
mod tests {
    use {super::*, leetium_mcp::McpRegistry};
    ...
    fn parse_server_config_allows_sse_without_command() {
        ...
        assert!(matches!(cfg.transport, leetium_mcp::TransportType::Sse));
    ...
    fn parse_server_config_update_preserves_existing_sse_fields() {
        let existing = leetium_mcp::McpServerConfig {
            transport: leetium_mcp::TransportType::Sse,
    ...
    fn parse_server_config_update_preserves_oauth_when_omitted() {
        let existing = leetium_mcp::McpServerConfig {
            transport: leetium_mcp::TransportType::Sse,
            url: Some("https://mcp.linear.app/mcp".to_string()),
            oauth: Some(leetium_mcp::McpOAuthConfig {
    ...
    async fn test_sync_mcp_tools_empty_manager() {
        let manager = leetium_mcp::McpManager::new(McpRegistry::new());
