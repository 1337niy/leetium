...
/// Gateway-side onboarding service backed by `leetium_onboarding::service::LiveOnboardingService`.
pub struct GatewayOnboardingService {
    inner: leetium_onboarding::service::LiveOnboardingService,
    session_metadata: Arc<leetium_sessions::metadata::SqliteSessionMetadata>,
...
impl GatewayOnboardingService {
    ...
    async fn create_imported_agents(
        ...
        for agent in &agents.agents {
            ...
            match self.agent_persona_store.get(&agent.leetium_id).await {
                Ok(Some(_)) => {
                    tracing::debug!(
                        id = %agent.leetium_id,
            ...
            let name = agent
                .name
                .clone()
                .unwrap_or_else(|| agent.leetium_id.clone());

            let params = crate::agent_persona::CreateAgentParams {
                id: agent.leetium_id.clone(),
                name,
                emoji: None,
    ...
    async fn sync_imported_sessions_to_sqlite(&self, data_dir: &Path) -> Result<(), String> {
        ...
        let legacy_metadata = leetium_sessions::metadata::SessionMetadata::load(metadata_path)
...
impl OnboardingService for GatewayOnboardingService {
    ...
    async fn openclaw_detect(&self) -> ServiceResult {
        let detection = leetium_openclaw_import::detect();
        match detection {
            Some(d) => {
                let scan = leetium_openclaw_import::scan(&d);
                tracing::info!(
    ...
    async fn openclaw_import(&self, params: Value) -> ServiceResult {
        ...
        let selection = leetium_openclaw_import::ImportSelection {
            identity: params
                .get("identity")
                .and_then(|v| v.as_bool())
                .unwrap_or(false),
        ...
        let report = leetium_openclaw_import::import(&detection, &selection, &config_dir, &data_dir);
...
fn parse_geo_location(value: &Value) -> Option<leetium_config::GeoLocation> {
    ...
    Some(leetium_config::GeoLocation {
        latitude,
        longitude,
        place,
        updated_at,
...
mod tests {
    ...
    async fn sync_imported_sessions_preserves_timestamps_and_preview() {
        ...
        sqlx::query("CREATE TABLE IF NOT EXISTS projects (id TEXT PRIMARY KEY)")
            ...
            .await
            .unwrap();
        leetium_sessions::metadata::SqliteSessionMetadata::init(&pool)
            .await
            .unwrap();
        sqlx::query(
        ...
        let service = GatewayOnboardingService::new(
            leetium_onboarding::service::LiveOnboardingService::new(dir.path().join("leetium.toml")),
            Arc::new(leetium_sessions::metadata::SqliteSessionMetadata::new(
                pool.clone(),
            )),
