...
use {leetium_channels::ChannelPlugin, leetium_protocol::TICK_INTERVAL_MS};

use leetium_providers::ProviderRegistry;
...
/// Gateway implementation of [`leetium_tools::location::LocationRequester`].
///
...
impl leetium_tools::location::LocationRequester for GatewayLocationRequester {
    async fn request_location(
        &self,
        conn_id: &str,
        precision: leetium_tools::location::LocationPrecision,
    ) -> leetium_tools::Result<leetium_tools::location::LocationResult> {
        use leetium_tools::location::{LocationError, LocationResult};
        ...
        let event = leetium_protocol::EventFrame::new(
            "location.request",
...

        {
            ...
            let client = clients.get(conn_id).ok_or_else(|| {
                leetium_tools::Error::message(format!("no client connection for conn_id {conn_id}"))
            })?;
            if !client.send(&event_json) {
                return Err(leetium_tools::Error::message(format!(
        ...
        if let Some(loc) = result.get("location") {
            ...
            Ok(LocationResult {
                location: Some(leetium_tools::location::BrowserLocation {
                    latitude: lat,
                    longitude: lon,
                    accuracy,
                }),
    ...
    fn cached_location(&self) -> Option<leetium_config::GeoLocation> {
    ...
    async fn request_channel_location(
        ...
        let session_meta = self
            ...
            .session_metadata
            .as_ref()
            .ok_or_else(|| leetium_tools::Error::message("session metadata not available"))?;
        let entry = session_meta.get(session_key).await.ok_or_else(|| {
            leetium_tools::Error::message(format!("no session metadata for key {session_key}"))
        })?;
        let binding_json = entry.channel_binding.ok_or_else(|| {
            leetium_tools::Error::message(format!("no channel binding for session {session_key}"))
        })?;
        let reply_target: leetium_channels::ChannelReplyTarget =
        ...
        let outbound = self
            ...
            .ok_or_else(|| leetium_tools::Error::message("no channel outbound available"))?;
        outbound
            .send_text(
            ...
            .await
            .map_err(|e| leetium_tools::Error::external("send location request", e))?;
...
fn should_prebuild_sandbox_image(
    mode: &leetium_tools::sandbox::SandboxMode,
    packages: &[String],
) -> bool {
    !matches!(mode, leetium_tools::sandbox::SandboxMode::Off) && !packages.is_empty()
}

fn instance_slug(config: &leetium_config::LeetiumConfig) -> String {
    ...
    let base = raw_name
        .unwrap_or_else(|| "leetium".to_string())
        .to_lowercase();
...
fn sandbox_container_prefix(instance_slug: &str) -> String {
    format!("leetium-{instance_slug}-sandbox")
}

fn browser_container_prefix(instance_slug: &str) -> String {
    format!("leetium-{instance_slug}-browser")
}
...
fn approval_manager_from_config(config: &leetium_config::LeetiumConfig) -> ApprovalManager {
...
pub struct AppState {
    ...
    #[cfg(feature = "graphql")]
    pub graphql_schema: leetium_graphql::LeetiumSchema,
}
...
pub fn build_gateway_base(
    ...
    let graphql_schema = {
        ...
        leetium_graphql::build_schema(services, state.graphql_broadcast.clone())
    };
...
fn validate_proxy_tls_configuration(
    ...
    if behind_proxy && tls_enabled && !allow_tls_behind_proxy {
        anyhow::bail!(
            "LEETIUM_BEHIND_PROXY=true with Leetium TLS enabled is usually a proxy misconfiguration. Run with --no-tls (or LEETIUM_NO_TLS=true). If your proxy upstream is HTTPS/TCP passthrough by design, set LEETIUM_ALLOW_TLS_BEHIND_PROXY=true."
        );
    }
    Ok(())
}
...
fn detect_openclaw_with_startup_logs() -> Option<leetium_openclaw_import::OpenClawDetection> {
    match leetium_openclaw_import::detect() {
        Some(detection) => {
            info!(
    ...
    tokio::spawn(async move {
        ...
        if let Some(detection) = detection {
            ...
            if sessions_dir.is_dir() {
                match leetium_openclaw_import::watcher::ImportWatcher::start(sessions_dir) {
                    Ok((_watcher, mut rx)) => {
                            ...
                            loop {
                                tokio::select! {
                                    Some(_event) = rx.recv() => {
                                        ...
                                        let report = leetium_openclaw_import::import_sessions_only(
...
pub fn openclaw_detected_for_ui() -> bool {
    leetium_openclaw_import::detect().is_some()
}
...
fn log_startup_config_storage_diagnostics() {
    let config_dir = leetium_config::config_dir().unwrap_or_else(|| PathBuf::from(".leetium"));
    let discovered_config = leetium_config::loader::find_config_file();
    let expected_config = leetium_config::find_or_default_config_path();
    ...
    if let Some(path) = discovered_config {
        ...
        warn!(
            path = %expected_config.display(),
            "no config file detected on startup; Leetium is running with in-memory defaults until config is persisted"
        );
    }
...
pub(crate) struct BannerMeta {
    ...
    pub browser_tool_for_warmup: Option<Arc<dyn leetium_agents::tool_registry::AgentTool>>,
    pub config: leetium_config::schema::LeetiumConfig,
    #[cfg(feature = "tailscale")]
...
pub async fn prepare_gateway(
    ...
    if let Some(dir) = config_dir {
        leetium_config::set_config_dir(dir);
    }
    if let Some(ref dir) = data_dir {
        leetium_config::set_data_dir(dir.clone());
    }

    // Resolve auth from environment (LEETIUM_TOKEN / LEETIUM_PASSWORD).
    let token = std::env::var("LEETIUM_TOKEN").ok();
    let password = std::env::var("LEETIUM_PASSWORD").ok();

    // Cloud deploy platform — hides local-only providers (local-llm, ollama).
    let deploy_platform = std::env::var("LEETIUM_DEPLOY_PLATFORM").ok();
    let resolved_auth = auth::resolve_auth(token, password.clone());

    // Load config file (leetium.toml / .yaml / .json) if present.
    let mut config = leetium_config::discover_and_load();
    ...
    let behind_proxy = env_flag_enabled("LEETIUM_BEHIND_PROXY");
    let allow_tls_behind_proxy = env_flag_enabled("LEETIUM_ALLOW_TLS_BEHIND_PROXY");
    #[cfg(feature = "tls")]
    ...
    if behind_proxy && tls_enabled_for_gateway && allow_tls_behind_proxy {
        warn!(
            "LEETIUM_ALLOW_TLS_BEHIND_PROXY=true is set; ensure your proxy uses HTTPS upstream or TLS passthrough to avoid redirect loops"
        );
    }
    ...
    if !providers_available_at_startup {
        let config_path = leetium_config::find_or_default_config_path();
        let provider_keys_path = leetium_config::config_dir()
    ...
    if !has_explicit_provider_settings {
        if auto_detected_provider_sources.is_empty() {
            ...
            let import_token_store = leetium_oauth::TokenStore::new();
    ...
    let onboarding_config_path = leetium_config::find_or_default_config_path();
    let live_onboarding =
        leetium_onboarding::service::LiveOnboardingService::new(onboarding_config_path);
    ...
    #[cfg(feature = "voice")]
    {
        ...
        services.tts = Arc::new(LiveTtsService::new(leetium_voice::TtsConfig::default()));
    ...
    {
        let mcp_registry_path = leetium_config::data_dir().join("mcp-servers.json");
        let mcp_reg = leetium_mcp::McpRegistry::load(&mcp_registry_path).unwrap_or_default();
        ...
        for (name, entry) in &config.mcp.servers {
            if !merged.servers.contains_key(name) {
                let transport = match entry.transport.as_str() {
                    "sse" => leetium_mcp::registry::TransportType::Sse,
                    _ => leetium_mcp::registry::TransportType::Stdio,
                };
                let oauth = entry
                    .oauth
                    .as_ref()
                    .map(|o| leetium_mcp::registry::McpOAuthConfig {
        ...
        let mcp_manager = Arc::new(leetium_mcp::McpManager::new(merged));
    ...
    let db_pool = {
        ...
        let pool = sqlx::pool::PoolOptions::new()
            ...
            .await
            .expect("failed to open leetium.db");
        debug!(
    ...
    leetium_projects::run_migrations(&db_pool)
        .await
        .expect("failed to run projects migrations");
    leetium_sessions::run_migrations(&db_pool)
        .await
        .expect("failed to run sessions migrations");
    leetium_cron::run_migrations(&db_pool)
        .await
    ...
    #[cfg(feature = "vault")]
    leetium_vault::run_migrations(&db_pool)
        .await
    ...
    #[cfg(feature = "vault")]
    let vault: Option<Arc<leetium_vault::Vault>> = {
        match leetium_vault::Vault::new(db_pool.clone()).await {
            Ok(v) => {
    ...
    let explicit_rp_id = std::env::var("LEETIUM_WEBAUTHN_RP_ID")
    ...
    let explicit_origin = std::env::var("LEETIUM_WEBAUTHN_ORIGIN")
    ...
    let webauthn_registry = {
        ...
        if let Some(ref rp_id) = explicit_rp_id {
            ...
            // Local: register localhost + leetium.localhost as extras.
            let localhost_origin = format!("{default_scheme}://localhost:{port}");
            let leetium_localhost: Vec<webauthn_rs::prelude::Url> =
                webauthn_rs::prelude::Url::parse(&format!(
                    "{default_scheme}://leetium.localhost:{port}"
                ))
                .into_iter()
                .collect();
            try_add("localhost", &localhost_origin, &leetium_localhost);
    ...
    // If LEETIUM_PASSWORD is set and no password in DB yet, migrate it.
    if let Some(ref pw) = password
        && !credential_store.is_setup_complete()
    {
        info!("migrating LEETIUM_PASSWORD env var to credential store");
        if let Err(e) = credential_store.set_initial_password(pw).await {
            tracing::warn!("failed to migrate env password: {e}");
        }
    }

    let message_log: Arc<dyn leetium_channels::message_log::MessageLog> = Arc::new(
    ...
    if projects_toml_path.exists() {
        ...
        let old_store = leetium_projects::TomlProjectStore::new(projects_toml_path.clone());
        let sqlite_store = leetium_projects::SqliteProjectStore::new(db_pool.clone());
        if let Ok(projects) =
            <leetium_projects::TomlProjectStore as ProjectStore>::list(&old_store).await
        {
            for p in projects {
    ...
    let project_store: Arc<dyn ProjectStore> =
        Arc::new(leetium_projects::SqliteProjectStore::new(db_pool.clone()));
    ...
    let session_state_store = Arc::new(leetium_sessions::state_store::SessionStateStore::new(
        db_pool.clone(),
    ));
    ...
    let cron_store: Arc<dyn leetium_cron::store::CronStore> =
        match leetium_cron::store_file::FileStore::default_path() {
            Ok(fs) => Arc::new(fs),
            Err(e) => {
                tracing::warn!("cron file store unavailable ({e}), using in-memory");
                Arc::new(leetium_cron::store_memory::InMemoryStore::new())
            },
        };

    // System event: inject text into the main session and trigger an agent response.
    let sys_state = Arc::clone(&deferred_state);
    let on_system_event: leetium_cron::service::SystemEventFn = Arc::new(move |text| {
    ...
    let events_queue = leetium_cron::system_events::SystemEventsQueue::new();
    ...
    let on_agent_turn: leetium_cron::service::AgentTurnFn = Arc::new(move |req| {
        let st = Arc::clone(&agent_state);
        let eq = Arc::clone(&agent_events_queue);
        Box::pin(async move {
            let state = st
                .get()
                .ok_or_else(|| leetium_cron::Error::message("gateway not ready"))?;
            ...
            let is_heartbeat_turn = matches!(
                &req.session_target,
                leetium_cron::types::SessionTarget::Named(name) if name == "heartbeat"
            );
            ...
            if is_heartbeat_turn && !has_pending_events {
                ...
                let heartbeat_path = leetium_config::heartbeat_path();
                let heartbeat_file_exists = heartbeat_path.exists();
                let heartbeat_md = leetium_config::load_heartbeat_md();
                if heartbeat_file_exists && heartbeat_md.is_none() && !has_prompt_override {
                    ...
                    return Ok(leetium_cron::service::AgentTurnResult {
                        output: leetium_cron::heartbeat::HEARTBEAT_OK.to_string(),
                        input_tokens: None,
                        output_tokens: None,
                    });
                }
            ...
            let session_key = match &req.session_target {
                leetium_cron::types::SessionTarget::Named(name) => {
                    format!("cron:{name}")
                },
            ...
            if matches!(
                req.session_target,
                leetium_cron::types::SessionTarget::Named(_)
            ) {
                let _ = chat
            ...
            let prompt_text = if is_heartbeat_turn {
                ...
                if events.is_empty() {
                    ...
                    leetium_cron::heartbeat::build_event_enriched_prompt(&events, &req.message)
                }
            } else {
                req.message.clone()
            };
            ...
            let result = chat
                .send_sync(params)
                .await
                .map_err(|e| leetium_cron::Error::message(e.to_string()));
            ...
            let delivery_text = if is_heartbeat_turn {
                let hb_cfg = state.inner.read().await.heartbeat_config.clone();
                leetium_cron::heartbeat::strip_heartbeat_token(
                    &text,
                    leetium_cron::heartbeat::StripMode::Trim,
    ...
    let on_cron_notify: leetium_cron::service::NotifyFn =
        Arc::new(move |notification: leetium_cron::types::CronNotification| {
            let state_opt = deferred_for_cron.get();
            let Some(state) = state_opt else {
                return;
            };
            let (event, payload) = match &notification {
                leetium_cron::types::CronNotification::Created { job } => {
                    ("cron.job.created", serde_json::json!({ "job": job }))
                },
                leetium_cron::types::CronNotification::Updated { job } => {
                    ("cron.job.updated", serde_json::json!({ "job": job }))
                },
                leetium_cron::types::CronNotification::Removed { job_id } => {
    ...
    // Build rate limit config from leetium config.
    let rate_limit_config = leetium_cron::service::RateLimitConfig {
        max_per_window: config.cron.rate_limit_max,
        window_ms: config.cron.rate_limit_window_secs * 1000,
    };

    let cron_service = leetium_cron::service::CronService::with_events_queue(
        cron_store,
        on_system_event,
        on_agent_turn,
        Some(on_cron_notify),
    ...
    let sandbox_router = Arc::new(leetium_tools::sandbox::SandboxRouter::new(
    ...
    #[cfg(feature = "trusted-network")]
    {
        let (audit_tx, audit_rx) =
            tokio::sync::mpsc::channel::<leetium_network_filter::NetworkAuditEntry>(1024);
        ...
        if sandbox_config.network == leetium_network_filter::NetworkPolicy::Trusted {
            let domain_mgr = Arc::new(
                leetium_network_filter::domain_approval::DomainApprovalManager::new(
                    &sandbox_config.trusted_domains,
                    std::time::Duration::from_secs(30),
                ),
            );
            let proxy_addr: SocketAddr =
                ([0, 0, 0, 0], leetium_network_filter::DEFAULT_PROXY_PORT).into();
            let proxy = leetium_network_filter::proxy::NetworkProxyServer::new(
                proxy_addr,
            ...
            leetium_common::http::init_shared_http_client(Some(&url));
    ...
    // support image building return Ok(None) and the spawn is harmless.
    {
        ...
        let base_image = router
            ...
            .image
            .clone()
            .unwrap_or_else(|| leetium_tools::sandbox::DEFAULT_SANDBOX_IMAGE.to_string());
    ...
    // install the configured sandbox packages directly on the host in the background.
    {
        ...
        if sandbox_router.backend_name() == "none"
            && !packages.is_empty()
            && leetium_tools::sandbox::is_debian_host()
        {
            ...
            tokio::spawn(async move {
                ...
                match leetium_tools::sandbox::provision_host_packages(&packages).await {
                    Ok(Some(result)) => {
                        info!(
    ...
    if sandbox_router.backend_name() != "none" {
        ...
        tokio::spawn(async move {
            if let Some(prefix) = prefix {
                match leetium_tools::sandbox::clean_all_containers(&prefix).await {
                    Ok(0) => {},
                    Ok(n) => info!(
                        removed = n,
    ...
    if config.tools.browser.enabled
        && !matches!(
            ...
            leetium_tools::sandbox::SandboxMode::Off
        )
        && sandbox_router.backend_name() != "none"
    {
        ...
        tokio::spawn(async move {
            ...
            match leetium_browser::container::ensure_image(&sandbox_image) {
                Ok(()) => {
    ...
    let msteams_webhook_plugin: Arc<tokio::sync::RwLock<leetium_msteams::MsTeamsPlugin>>;
    #[cfg(feature = "slack")]
    let slack_webhook_plugin: Arc<tokio::sync::RwLock<leetium_slack::SlackPlugin>>;
    ...
    // Wire channel store, registry, and channel plugins.
    {
        ...
        let channel_sink: Arc<dyn leetium_channels::ChannelEventSink> = Arc::new(
        ...
        let tg_plugin = Arc::new(tokio::sync::RwLock::new(
            leetium_telegram::TelegramPlugin::new()
        ...
        let msteams_plugin = Arc::new(tokio::sync::RwLock::new(
            leetium_msteams::MsTeamsPlugin::new()
                .with_message_log(Arc::clone(&message_log))
                .with_event_sink(Arc::clone(&channel_sink)),
        ));
        ...
        let discord_plugin = Arc::new(tokio::sync::RwLock::new(
            leetium_discord::DiscordPlugin::new()
                .with_message_log(Arc::clone(&message_log))
                .with_event_sink(Arc::clone(&channel_sink)),
        ));
        registry
        ...
        #[cfg(feature = "whatsapp")]
        {
            ...
            let whatsapp_plugin = Arc::new(tokio::sync::RwLock::new(
                leetium_whatsapp::WhatsAppPlugin::new(wa_data_dir)
                    .with_message_log(Arc::clone(&message_log))
                    .with_event_sink(Arc::clone(&channel_sink)),
            ));
            registry
        ...
        let outbound_router = Arc::clone(&router) as Arc<dyn leetium_channels::ChannelOutbound>;
        services = services.with_channel_outbound(Arc::clone(&outbound_router));
        services = services.with_channel_stream_outbound(
            router as Arc<dyn leetium_channels::ChannelStreamOutbound>,
        );
    ...
    let memory_manager: Option<Arc<leetium_memory::manager::MemoryManager>> = {
        // Build embedding provider(s) for the fallback chain.
        let mut embedding_providers: Vec<(
            String,
            Box<dyn leetium_memory::embeddings::EmbeddingProvider>,
        )> = Vec::new();
        ...
        if mem_cfg.disable_rag {
            ...
            if let Some(ref provider_name) = mem_cfg.provider {
                match provider_name.as_str() {
                    "local" => {
                        // Local GGUF embeddings require the `local-embeddings` feature on leetium-memory.
                        #[cfg(feature = "local-embeddings")]
                        {
                            let cache_dir = mem_cfg
                                ...
                                .map(PathBuf::from)
                                .unwrap_or_else(
                                    leetium_memory::embeddings_local::LocalGgufEmbeddingProvider::default_cache_dir,
                                );
                            match leetium_memory::embeddings_local::LocalGgufEmbeddingProvider::ensure_model(
                                cache_dir,
                            )
                            .await
                            {
                                Ok(path) => {
                                    match leetium_memory::embeddings_local::LocalGgufEmbeddingProvider::new(
                                        path,
                                    ) {
            ...
            if embedding_providers.is_empty() {
                ...
                if ollama_ok {
                    ...
                    let e = leetium_memory::embeddings_openai::OpenAiEmbeddingProvider::new(
                        String::new(),
                    )
        ...
        let embedder: Option<Box<dyn leetium_memory::embeddings::EmbeddingProvider>> = if mem_cfg
            .disable_rag
        {
            None
            ...
            if embedding_providers.len() == 1 {
                ...
                Some(Box::new(
                    leetium_memory::embeddings_fallback::FallbackEmbeddingProvider::new(
                        embedding_providers,
                    ),
                ))
            }
        ...
        match memory_pool_result {
            Ok(memory_pool) => {
                if let Err(e) = leetium_memory::schema::run_migrations(&memory_pool).await {
                    ...
                    let config = leetium_memory::config::MemoryConfig {
                    ...
                    let store = Box::new(leetium_memory::store_sqlite::SqliteMemoryStore::new(
                        memory_pool,
                    ));
                    ...
                    let manager = Arc::new(if let Some(embedder) = embedder {
                        leetium_memory::manager::MemoryManager::new(config, store, embedder)
                    } else {
                        leetium_memory::manager::MemoryManager::keyword_only(config, store)
                    });
                    ...
                    tokio::spawn(async move {
                        ...
                        #[cfg(feature = "file-watcher")]
                        {
                            ...
                            match leetium_memory::watcher::MemoryFileWatcher::start(watch_dirs) {
                                Ok((_watcher, mut rx)) => {
    ...
    #[cfg(feature = "metrics")]
    let metrics_handle = {
        let metrics_config = leetium_metrics::MetricsRecorderConfig {
            enabled: config.metrics.enabled,
            prefix: None,
            global_labels: vec![
                ("service".to_string(), "leetium-gateway".to_string()),
                ("version".to_string(), env!("CARGO_PKG_VERSION").to_string()),
            ],
        };
        match leetium_metrics::init_metrics(metrics_config) {
            Ok(handle) => {
    ...
    let metrics_store: Option<Arc<dyn crate::state::MetricsStore>> = {
        ...
        match leetium_metrics::SqliteMetricsStore::new(&metrics_db_path).await {
            Ok(store) => {
                info!(
    ...
    let setup_code_display =
        if !credential_store.is_setup_complete() && !credential_store.is_auth_disabled() {
            let code = std::env::var("LEETIUM_E2E_SETUP_CODE")
    ...
    let browser_tool_for_warmup: Option<Arc<dyn leetium_agents::tool_registry::AgentTool>>;
    ...
    // Wire live chat service (needs state reference, so done after state creation).
    {
        ...
        let exec_cb: leetium_tools::exec::ExecCompletionFn = Arc::new(move |event| {
        ...
        let mut exec_tool = leetium_tools::exec::ExecTool::default()
        ...
        let cron_tool = leetium_tools::cron_tool::CronTool::new(Arc::clone(&cron_service));

        let mut tool_registry = leetium_agents::tool_registry::ToolRegistry::new();
        let process_tool = leetium_tools::process::ProcessTool::new()
            .with_sandbox_router(Arc::clone(&sandbox_router));

        let sandbox_packages_tool = leetium_tools::sandbox_packages::SandboxPackagesTool::new()
            .with_sandbox_router(Arc::clone(&sandbox_router));

        tool_registry.register(Box::new(exec_tool));
        tool_registry.register(Box::new(leetium_tools::calc::CalcTool::new()));
        #[cfg(feature = "wasm")]
        {
            ...
            if let Err(e) = leetium_tools::wasm_tool_runner::register_wasm_tools(
                &mut tool_registry,
                &wasm_limits,
                epoch_interval_ms,
        ...
        tool_registry.register(Box::new(
            leetium_tools::send_image::SendImageTool::new()
                .with_sandbox_router(Arc::clone(&sandbox_router)),
        ));
        if let Some(t) = leetium_tools::web_search::WebSearchTool::from_config_with_env_overrides(
        ...
        if let Some(t) = leetium_tools::browser::BrowserTool::from_config(&config.tools.browser) {
        ...
        #[cfg(feature = "caldav")]
        {
            if let Some(t) = leetium_caldav::tool::CalDavTool::from_config(&config.caldav) {
                tool_registry.register(Box::new(t));
            }
        }
        ...
        if let Some(ref mm) = memory_manager {
            tool_registry.register(Box::new(leetium_memory::tools::MemorySearchTool::new(
                Arc::clone(mm),
            )));
            tool_registry.register(Box::new(leetium_memory::tools::MemoryGetTool::new(
                Arc::clone(mm),
            )));
            tool_registry.register(Box::new(leetium_memory::tools::MemorySaveTool::new(
                Arc::clone(mm),
            )));
        }
        ...
        {
            let node_info_provider: Arc<dyn leetium_tools::nodes::NodeInfoProvider> = Arc::new(
                crate::node_exec::GatewayNodeInfoProvider::new(Arc::clone(&state)),
            );
            tool_registry.register(Box::new(leetium_tools::nodes::NodesListTool::new(
                Arc::clone(&node_info_provider),
            )));
            tool_registry.register(Box::new(leetium_tools::nodes::NodesDescribeTool::new(
                Arc::clone(&node_info_provider),
            )));
            tool_registry.register(Box::new(leetium_tools::nodes::NodesSelectTool::new(
                Arc::clone(&node_info_provider),
            )));
        }
        ...
        tool_registry.register(Box::new(
            leetium_tools::session_state::SessionStateTool::new(Arc::clone(&session_state_store)),
        ));
        ...
        let create_session: CreateSessionFn = Arc::new(move |req: CreateSessionRequest| {
            ...
            Box::pin(async move {
                ...
                state
                    ...
                    .await
                    .map_err(|e| leetium_tools::Error::message(e.to_string()))?;
                ...
                let entry = metadata.get(&key).await.ok_or_else(|| {
                    leetium_tools::Error::message(format!("session '{key}' not found after create"))
                })?;
                Ok(serde_json::json!({
                    "entry": {
                        "id": entry.id,
        ...
        tool_registry.register(Box::new(leetium_tools::task_list::TaskListTool::new(
            &data_dir,
        )));
        ...
        {
            tool_registry.register(Box::new(leetium_tools::skill_tools::CreateSkillTool::new(
                data_dir.clone(),
            )));
            tool_registry.register(Box::new(leetium_tools::skill_tools::UpdateSkillTool::new(
                data_dir.clone(),
            )));
            tool_registry.register(Box::new(leetium_tools::skill_tools::DeleteSkillTool::new(
                data_dir.clone(),
            )));
        }
        ...
        tool_registry.register(Box::new(
            leetium_tools::branch_session::BranchSessionTool::new(
                Arc::clone(&session_store),
                Arc::clone(&session_metadata),
            ),
        ));
        ...
        tool_registry.register(Box::new(leetium_tools::location::LocationTool::new(
            location_requester,
        )));

        // Register map tool for showing static map images with links.
        let map_provider = match config.tools.maps.provider {
            leetium_config::schema::MapProvider::GoogleMaps => {
                leetium_tools::map::MapProvider::GoogleMaps
            },
            leetium_config::schema::MapProvider::AppleMaps => {
                leetium_tools::map::MapProvider::AppleMaps
            },
            leetium_config::schema::MapProvider::OpenStreetMap => {
                leetium_tools::map::MapProvider::OpenStreetMap
            },
        };
        tool_registry.register(Box::new(leetium_tools::map::ShowMapTool::with_provider(
            map_provider,
        )));
        ...
        if let Some(default_provider) = registry.read().await.first_with_tools() {
            ...
            let on_spawn_event: leetium_tools::spawn_agent::OnSpawnEvent = Arc::new(move |event| {
                use leetium_agents::runner::RunnerEvent;
            ...
            let spawn_tool = leetium_tools::spawn_agent::SpawnAgentTool::new(
                Arc::clone(&registry),
                default_provider,
                base_tools,
            )
    ...
    #[cfg(feature = "file-watcher")]
    {
        let search_paths = leetium_skills::discover::FsSkillDiscoverer::default_paths();
        let watch_dirs: Vec<PathBuf> = search_paths.into_iter().map(|(p, _)| p).collect();
        if let Ok((_watcher, mut rx)) = leetium_skills::watcher::SkillWatcher::start(watch_dirs) {
            ...
            axum::routing::post(
                move |axum::extract::Path(account_id): axum::extract::Path<String>,
                    ...
                    async move {
                        ...
                        {
                            merged_headers.insert("x-leetium-webhook-secret", val);
                        }
                        ...
                        match crate::channel_webhook_middleware::channel_webhook_gate(
                            ...
                            Ok((_, leetium_channels::ChannelWebhookDedupeResult::Duplicate)) => (
                                StatusCode::OK,
                                Json(serde_json::json!({ "ok": true, "deduplicated": true })),
                            )
                                .into_response(),
                            Ok((verified, leetium_channels::ChannelWebhookDedupeResult::New)) => {
    ...
    tokio::spawn(async move {
        let client = match reqwest::Client::builder()
            .user_agent(format!("leetium-gateway/{}", update_state.version))
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Some(ref handle) = metrics_state.metrics_handle {
                    ...
                    leetium_metrics::gauge!(leetium_metrics::system::UPTIME_SECONDS)
                        .set(server_start.elapsed().as_secs_f64());
                    let session_count =
                        metrics_state.inner.read().await.active_sessions.len() as f64;
                    leetium_metrics::gauge!(leetium_metrics::session::ACTIVE).set(session_count);

                    let prometheus_text = handle.render();
                    let snapshot =
                        leetium_metrics::MetricsSnapshot::from_prometheus_text(&prometheus_text);
                ...
                match event_rx.recv().await {
                    Ok(event) => {
                        let (event_name, payload) = match event {
                            leetium_tools::sandbox::SandboxEvent::Preparing {
                                session_key,
                                backend,
                                image,
                            } => (
                            ...
                            leetium_tools::sandbox::SandboxEvent::Prepared {
                                session_key,
                                backend,
                                image,
                            } => (
                            ...
                            leetium_tools::sandbox::SandboxEvent::PrepareFailed {
                                session_key,
                                backend,
                                image,
                                error,
                            ...
                            leetium_tools::sandbox::SandboxEvent::Provisioning {
                                container,
                                packages,
                            } => (
                            ...
                            leetium_tools::sandbox::SandboxEvent::Provisioned { container } => (
                                "sandbox.image.provision",
                                serde_json::json!({
                                    "phase": "done",
                                    "container": container,
                            ...
                            leetium_tools::sandbox::SandboxEvent::ProvisionFailed {
                                container,
                                error,
                            } => (
                                "sandbox.image.provision",
            ...
            loop {
                match rx.recv().await {
                    Ok(entry) => {
                        ...
                        if entry.target.starts_with("leetium_gateway::broadcast") {
                            continue;
                        }
                        if let Ok(payload) = serde_json::to_value(&entry) {
                            broadcast(
        ...
        if hb.enabled && has_prompt {
            if existing_job.is_some() {
                ...
                let patch = CronJobPatch {
                    ...
                    enabled: Some(true),
                    sandbox: Some(leetium_cron::types::CronSandboxConfig {
                ...
                let create = CronJobCreate {
                    ...
                    enabled: true,
                    system: true,
                    sandbox: leetium_cron::types::CronSandboxConfig {
                        enabled: hb.sandbox_enabled,
                        image: hb.sandbox_image.clone(),
                    },
                    wake_mode: leetium_cron::types::CronWakeMode::default(),
                };
...
/// the `tailscale` feature is enabled on `leetium-gateway`.
...
pub async fn start_gateway(
    ...
    let _mdns_daemon = {
        let host = hostname::get()
            ...
            .unwrap_or_else(|| "leetium".to_string());
        let instance = format!("Leetium on {host}");
    ...
    let (skill_count, repo_count) = {
        use leetium_skills::discover::{FsSkillDiscoverer, SkillDiscoverer};
        ...
        let rc = leetium_skills::manifest::ManifestStore::default_path()
            .ok()
            .map(|p| {
                let store = leetium_skills::manifest::ManifestStore::new(p);
    ...
    let mut lines = vec![
        format!("leetium gateway v{}", state.version),
        format!(
            "protocol v{}, listening on {}://{} ({})",
            leetium_protocol::PROTOCOL_VERSION,
            scheme,
            display_host,
            if tls_active {
                "HTTP/2 + HTTP/1.1"
        ...
        format!(
            "config: {}",
            leetium_config::find_or_default_config_path().display()
        ),
    ...
    if tls_active {
        ...
        lines.push("run `leetium trust-ca` to remove browser warnings".into());
    }
    ...
    if let Some(ref hooks) = state.inner.read().await.hook_registry {
        let payload = leetium_common::hooks::HookPayload::GatewayStart {
...
async fn health_handler(State(state): State<AppState>) -> impl IntoResponse {
    ...
    Json(serde_json::json!({
        ...
        "protocol": leetium_protocol::PROTOCOL_VERSION,
        "connections": count,
    }))
}
...
pub(crate) use leetium_auth::locality::is_local_connection;
...
fn is_same_origin(origin: &str, host: &str) -> bool {
    ...
    // Subdomains of .localhost (e.g. leetium.localhost) are also loopback per RFC 6761.
    let is_loopback =
...
fn builtin_hook_metadata() -> Vec<(
    &'static str,
    &'static str,
    Vec<leetium_common::hooks::HookEvent>,
    &'static str,
)> {
    use leetium_common::hooks::HookEvent;
    vec![
        (
            "boot-md",
        ...
        (
            "command-logger",
            "Logs all slash-command invocations to a JSONL audit file at ~/.leetium/logs/commands.log.",
...
/// Seed a skeleton example hook into `~/.leetium/hooks/example/` on first run.
///
...
fn seed_example_hook() {
    let hook_dir = leetium_config::data_dir().join("hooks/example");
...
/// Seed the `dcg-guard` hook into `~/.leetium/hooks/dcg-guard/` on first run.
///
...
fn seed_dcg_guard_hook() {
    let hook_dir = leetium_config::data_dir().join("hooks/dcg-guard");
    let hook_md = hook_dir.join("HOOK.md");
    if hook_md.exists() {
        return;
    }
...
/// Seed built-in personal skills into `~/.leetium/skills/`.
///
...
fn seed_skill_if_missing(name: &str, content: &str) {
    let skill_dir = leetium_config::data_dir().join(format!("skills/{name}"));
...
pub(crate) fn sync_persona_into_preset(
    agents: &mut leetium_config::AgentsConfig,
    persona: &crate::agent_persona::AgentPersona,
) {
    let soul = leetium_config::load_soul_for_agent(&persona.id);
...
3. Click **Reload** in the Hooks UI (or restart leetium)
...
echo "$(date -Iseconds) tool=$tool" >> /tmp/leetium-hook.log
...
This hook is **seeded by default** into `~/.leetium/hooks/dcg-guard/` on first
...
- If HEARTBEAT.md exists but is empty/comment-only and there is no explicit
  heartbeat.prompt override, Leetium skips heartbeat LLM turns to avoid token use.
-->"#;
...
pub(crate) async fn discover_and_build_hooks(
    ...
    Option<Arc<leetium_common::hooks::HookRegistry>>,
    ...
    let mut registry = leetium_common::hooks::HookRegistry::new();
    ...
    {
        let data = leetium_config::data_dir();
...
mod tests {
    ...
    fn approval_manager_uses_config_values() {
        let mut cfg = leetium_config::LeetiumConfig::default();
    ...
    async fn command_hook_dispatch_saves_session_memory_file() {
        ...
        registry.register(Arc::new(
            leetium_plugins::bundled::session_memory::SessionMemoryHook::new(
                tmp.path().to_path_buf(),
                Arc::clone(&session_store),
            ),
        ));

        let payload = leetium_common::hooks::HookPayload::Command {
    ...
    async fn websocket_header_auth_accepts_valid_session_cookie() {
        ...
        let store = Arc::new(
            auth::CredentialStore::with_config(pool, &leetium_config::AuthConfig::default())
                .await
                .unwrap(),
        );
    ...
    #[test]
    fn same_origin_leetium_localhost() {
        // leetium.localhost ↔ localhost loopback variants
        assert!(is_same_origin(
            "https://leetium.localhost:8080",
            "localhost:8080"
        ));
        assert!(is_same_origin(
            "https://leetium.localhost:8080",
    ...
    fn prebuild_runs_only_when_mode_enabled_and_packages_present() {
        ...
        assert!(should_prebuild_sandbox_image(
            &leetium_tools::sandbox::SandboxMode::All,
            &packages
        ));
        assert!(should_prebuild_sandbox_image(
            &leetium_tools::sandbox::SandboxMode::NonMain,
            &packages
        ));
        assert!(!should_prebuild_sandbox_image(
            &leetium_tools::sandbox::SandboxMode::Off,
    ...
    fn proxy_tls_validation_rejects_common_misconfiguration() {
        ...
        assert!(message.contains("LEETIUM_BEHIND_PROXY=true"));
    ...
    fn env_value_with_overrides_uses_override_when_process_env_missing() {
        let unique_key = format!("LEETIUM_TEST_LOOKUP_{}", std::process::id());
    ...
    fn sync_persona_into_preset_creates_new_entry() {
        let mut agents = leetium_config::AgentsConfig::default();
    ...
    fn sync_persona_preserves_existing_preset_fields() {
        let mut agents = leetium_config::AgentsConfig::default();
        let existing = leetium_config::AgentPreset {
            model: Some("haiku".into()),
            timeout_secs: Some(30),
            tools: leetium_config::PresetToolPolicy {
                deny: vec!["exec".into()],
                ..Default::default()
            },
            ..Default::default()
