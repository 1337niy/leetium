...
pub fn channel_webhook_gate(
    ...
    #[cfg(feature = "metrics")]
    {
        use leetium_metrics::{counter, labels};
        counter!(
            leetium_metrics::channel_webhook::REQUESTS_TOTAL,
    ...
    let envelope = match verifier.verify(headers, body) {
        Ok(env) => {
            #[cfg(feature = "metrics")]
            {
                use leetium_metrics::{counter, histogram, labels};
                counter!(
                    leetium_metrics::channel_webhook::VERIFIED_TOTAL,
                    labels::CHANNEL => verifier.channel_type().as_str()
                )
                .increment(1);
                histogram!(
                    leetium_metrics::channel_webhook::VERIFY_DURATION_SECONDS,
                    labels::CHANNEL => verifier.channel_type().as_str()
                )
        ...
        Err(rejection) => {
            #[cfg(feature = "metrics")]
            {
                use leetium_metrics::{counter, labels};
                counter!(
                    leetium_metrics::channel_webhook::REJECTED_TOTAL,
                    labels::CHANNEL => verifier.channel_type().as_str(),
    ...
    if let Err(rejection) = rate_limiter.check(
        ...
        #[cfg(feature = "metrics")]
        {
            use leetium_metrics::{counter, labels};
            counter!(
                leetium_metrics::channel_webhook::RATE_LIMITED_TOTAL,
                labels::CHANNEL => verifier.channel_type().as_str()
            )
            .increment(1);
        }
    ...
    let dedup_result = if let Some(ref key) = envelope.idempotency_key {
        ...
        if store.check_and_insert(key) {
            #[cfg(feature = "metrics")]
            {
                use leetium_metrics::{counter, labels};
                counter!(
                    leetium_metrics::channel_webhook::DEDUPED_TOTAL,
                    labels::CHANNEL => verifier.channel_type().as_str()
                )
                .increment(1);
            }
...
mod tests {
    use {super::*, bytes::Bytes, leetium_channels::plugin::ChannelType};
    ...
    fn gate_rate_limits_after_burst() {
        ...
        impl ChannelWebhookVerifier for TinyRateVerifier {
            ...
            fn rate_policy(&self) -> leetium_channels::ChannelWebhookRatePolicy {
                leetium_channels::ChannelWebhookRatePolicy {
