...
pub async fn exec_on_node(
    ...
    let invoke_event = leetium_protocol::EventFrame::new(
        "node.invoke.request",
        serde_json::json!({
            "invokeId": invoke_id,
            "command": "system.run",
...
/// Bridge that implements [`leetium_tools::exec::NodeExecProvider`] by
...
#[async_trait]
impl leetium_tools::exec::NodeExecProvider for GatewayNodeExecProvider {
...
fn node_to_info(n: &crate::nodes::NodeSession) -> leetium_tools::nodes::NodeInfo {
    leetium_tools::nodes::NodeInfo {
        ...
        providers: n
            .providers
            .iter()
            .map(|p| leetium_tools::nodes::NodeProviderInfo {
...
/// Bridge that implements [`leetium_tools::nodes::NodeInfoProvider`] by
...
#[async_trait]
impl leetium_tools::nodes::NodeInfoProvider for GatewayNodeInfoProvider {
    async fn list_nodes(&self) -> Vec<leetium_tools::nodes::NodeInfo> {
        let inner = self.state.inner.read().await;
        inner.nodes.list().iter().map(|n| node_to_info(n)).collect()
    }

    async fn describe_node(&self, node_ref: &str) -> Option<leetium_tools::nodes::NodeInfo> {
...
mod tests {
    ...
    fn filter_env_safe_only() {
        ...
        env.insert("LEETIUM_AUTH_TOKEN".into(), "token".into());
        ...
        assert!(!filtered.contains_key("LEETIUM_AUTH_TOKEN"));
