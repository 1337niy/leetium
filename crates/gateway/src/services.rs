...
//!
//! Pure trait definitions and simple noop implementations live in `leetium-service-traits`.
...
pub use leetium_service_traits::*;
...
fn security_audit(event: &str, details: Value) {
    let dir = leetium_config::data_dir().join("logs");
...
impl SkillsService for NoopSkillsService {
    ...
    async fn install(&self, params: Value) -> ServiceResult {
        ...
        let install_dir =
            leetium_skills::install::default_install_dir().map_err(ServiceError::message)?;
        let skills = leetium_skills::install::install_skill(source, &install_dir)
            .await
        ...
        let items: Vec<_> = skills
            ...
            .map(|s| {
                ...
                let protected = matches!(
                    s.source,
                    Some(leetium_skills::types::SkillSource::Personal)
                        | Some(leetium_skills::types::SkillSource::Project)
    ...
    async fn remove(&self, params: Value) -> ServiceResult {
        ...
        let install_dir =
            leetium_skills::install::default_install_dir().map_err(ServiceError::message)?;
        leetium_skills::install::remove_repo(source, &install_dir)
            .await
            .map_err(ServiceError::message)?;
    ...
    async fn repos_list(&self) -> ServiceResult {
        let install_dir =
            leetium_skills::install::default_install_dir().map_err(ServiceError::message)?;
        let manifest_path = leetium_skills::manifest::ManifestStore::default_path()
            .map_err(ServiceError::message)?;
        let store = leetium_skills::manifest::ManifestStore::new(manifest_path);
        ...
        let repos: Vec<_> = manifest
            ...
            .map(|repo| {
                ...
                let format = if repo.format == leetium_skills::formats::PluginFormat::Skill {
                    let repo_dir = install_dir.join(&repo.repo_name);
                    leetium_skills::formats::detect_format(&repo_dir)
                } else {
                    repo.format
                };
                serde_json::json!({
        ...
        if let Ok(entries) = std::fs::read_dir(&install_dir) {
            for entry in entries.flatten() {
                ...
                let format = leetium_skills::formats::detect_format(&path);
    ...
    async fn repos_list_full(&self) -> ServiceResult {
        use leetium_skills::requirements::check_requirements;
        ...
        let repos: Vec<_> = manifest
            ...
            .map(|repo| {
                ...
                let adapter_entries = match format {
                    leetium_skills::formats::PluginFormat::Skill => None,
                    _ => leetium_skills::formats::scan_with_adapter(&repo_dir, format)
                        .and_then(|r| r.ok()),
                };
                ...
                let skills: Vec<_> = repo
                    ...
                    .map(|s| {
                        ...
                        if let Some(ref entries) = adapter_entries {
                            ...
                            let meta_json = leetium_skills::parse::read_meta_json(&skill_dir);
                            let (description, display_name, elig) =
                                if let Ok(content) = std::fs::read_to_string(&skill_md) {
                                    if let Ok(meta) = leetium_skills::parse::parse_metadata(
                                        &content, &skill_dir,
                                    ) {
    ...
    async fn skill_detail(&self, params: Value) -> ServiceResult {
        ...
        match repo.format {
            leetium_skills::formats::PluginFormat::Skill => {
                let skill_dir = install_dir.join(&skill_state.relative_path);
                let skill_md = skill_dir.join("SKILL.md");
                let raw = std::fs::read_to_string(&skill_md)
                    .map_err(|e| format!("failed to read SKILL.md: {e}"))?;
                let content = leetium_skills::parse::parse_skill(&raw, &skill_dir)
            ...
            format => {
                ...
                let entries = leetium_skills::formats::scan_with_adapter(&repo_dir, format)
...
fn detect_and_mark_repo_drift(
    manifest: &mut leetium_skills::types::SkillsManifest,
    install_dir: &Path,
) -> (bool, HashSet<String>) {
...
fn delete_discovered_skill(source_type: &str, params: &Value) -> ServiceResult {
    ...
    if !leetium_skills::parse::validate_name(skill_name) {
        return Err(format!("invalid skill name '{skill_name}'").into());
    }

    let search_dir = if source_type == "personal" {
        leetium_config::data_dir().join("skills")
    } else {
        leetium_config::data_dir().join(".leetium/skills")
    };
...
fn toggle_skill(params: &Value, enabled: bool) -> ServiceResult {
    ...
    let manifest_path =
        leetium_skills::manifest::ManifestStore::default_path().map_err(ServiceError::message)?;
    let store = leetium_skills::manifest::ManifestStore::new(manifest_path);
    let mut manifest = store.load().map_err(ServiceError::message)?;
...
// ── Browser (Real implementation — depends on leetium-browser) ───────────────

/// Real browser service using BrowserManager.
pub struct RealBrowserService {
    config: leetium_browser::BrowserConfig,
    manager: tokio::sync::OnceCell<Arc<leetium_browser::BrowserManager>>,
}

impl RealBrowserService {
    pub fn new(config: &leetium_config::schema::BrowserConfig, container_prefix: String) -> Self {
        let mut browser_config = leetium_browser::BrowserConfig::from(config);
    ...
    pub fn from_config(
        config: &leetium_config::schema::LeetiumConfig,
    ...
    async fn manager(&self) -> Arc<leetium_browser::BrowserManager> {
        Arc::clone(
            self.manager
                .get_or_init(|| async {
                    ...
                    match tokio::task::spawn_blocking(move || {
                        ...
                        Arc::new(leetium_browser::BrowserManager::new(config))
                    })
                    .await
                    {
                        Ok(manager) => manager,
    ...
    fn manager_if_initialized(&self) -> Option<Arc<leetium_browser::BrowserManager>> {
...
impl BrowserService for RealBrowserService {
    async fn request(&self, params: Value) -> ServiceResult {
        let request: leetium_browser::BrowserRequest =
...
pub struct GatewayServices {
    ...
    pub channel_registry: Option<Arc<leetium_channels::ChannelRegistry>>,
    ...
    pub session_metadata: Option<Arc<leetium_sessions::metadata::SqliteSessionMetadata>>,
    /// Optional session store for message-index lookups (e.g. deduplication).
    pub session_store: Option<Arc<leetium_sessions::store::SessionStore>>,
    ...
    pub agents_config: Option<Arc<tokio::sync::RwLock<leetium_config::AgentsConfig>>>,
}
...
impl GatewayServices {
    ...
    pub fn with_channel_registry(
        mut self,
        registry: Arc<leetium_channels::ChannelRegistry>,
    ) -> Self {
    ...
    pub fn channel_outbound_arc(&self) -> Option<Arc<dyn leetium_channels::ChannelOutbound>> {
    ...
    pub fn with_session_metadata(
        mut self,
        meta: Arc<leetium_sessions::metadata::SqliteSessionMetadata>,
    ) -> Self {
        self.session_metadata = Some(meta);
        self
    }

    pub fn with_session_store(mut self, store: Arc<leetium_sessions::store::SessionStore>) -> Self {
    ...
    pub fn with_agents_config(
        mut self,
        agents_config: Arc<tokio::sync::RwLock<leetium_config::AgentsConfig>>,
    ) -> Self {
