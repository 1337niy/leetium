...
use leetium_config::{MessageQueueMode, ToolMode};
...
use {
    ...
    leetium_skills::discover::SkillDiscoverer,
    leetium_tools::policy::{ToolPolicy, profile_tools},
};
...
use {
    ...
    leetium_service_traits::{ChatService, ModelService, ServiceError, ServiceResult},
};
...
#[allow(dead_code)]
pub fn model_matches_allowlist(model: &leetium_providers::ModelInfo, patterns: &[String]) -> bool {
...
pub fn model_matches_allowlist_with_provider(
    model: &leetium_providers::ModelInfo,
...
async fn run_single_probe(
    ...
    display_name: String,
    provider_name: String,
    provider: Arc<dyn leetium_agents::model::LlmProvider>,
...
struct PromptPersona {
    ...
    user: leetium_config::UserProfile,
...
fn resolve_prompt_agent_id(session_entry: Option<&SessionEntry>) -> String {
    ...
    if leetium_config::agent_workspace_dir(agent_id).exists() {
...
fn load_prompt_persona_for_agent(agent_id: &str) -> PromptPersona {
    ...
    if let Some(file_identity) = leetium_config::load_identity_for_agent(agent_id) {
    ...
    if let Some(file_user) = leetium_config::load_user() {
    ...
    PromptPersona {
        ...
        identity,
        user,
        soul_text: leetium_config::load_soul_for_agent(agent_id),
        agents_text: leetium_config::load_agents_md_for_agent(agent_id),
        tools_text: leetium_config::load_tools_md_for_agent(agent_id),
        memory_text: leetium_config::load_memory_md_for_agent(agent_id),
    }
}
...
fn default_user_prompt_timezone() -> Option<String> {
    let user = leetium_config::load_user()?;
    user.timezone
        .as_ref()
...
fn effective_tool_policy(config: &leetium_config::LeetiumConfig) -> ToolPolicy {
...
fn apply_runtime_tool_filters(
    base: &ToolRegistry,
    config: &leetium_config::LeetiumConfig,
    _skills: &[leetium_skills::types::SkillMetadata],
    mcp_disabled: bool,
) -> ToolRegistry {
...
impl DisabledModelsStore {
    fn config_path() -> Option<PathBuf> {
        leetium_config::config_dir().map(|d| d.join("disabled-models.json"))
    }
...
impl LiveModelService {
    ...
    fn priority_rank(order: &HashMap<String, usize>, model: &leetium_providers::ModelInfo) -> usize {
        let full = normalize_model_key(&model.id);
    ...
    fn prioritize_models<'a>(
        ...
        models: impl Iterator<Item = &'a leetium_providers::ModelInfo>,
    ) -> Vec<&'a leetium_providers::ModelInfo> {
        let mut ordered: Vec<(usize, &'a leetium_providers::ModelInfo)> =
...
pub struct LiveChatService {
    ...
    hook_registry: Option<Arc<leetium_common::hooks::HookRegistry>>,
    ...
    failover_config: leetium_config::schema::FailoverConfig,
}
...
impl LiveChatService {
    pub fn new(
        ...
        Self {
            ...
            failover_config: leetium_config::schema::FailoverConfig::default(),
        }
    }

    pub fn with_failover(mut self, config: leetium_config::schema::FailoverConfig) -> Self {
    ...
    pub fn with_hooks(mut self, registry: leetium_common::hooks::HookRegistry) -> Self {
        self.hook_registry = Some(Arc::new(registry));
        self
    }

    pub fn with_hooks_arc(mut self, registry: Arc<leetium_common::hooks::HookRegistry>) -> Self {
    ...
    async fn resolve_provider(
        ...
        session_key: &str,
        history: &[Value],
    ) -> error::Result<Arc<dyn leetium_agents::model::LlmProvider>> {
        let reg = self.providers.read().await;
    ...
    async fn resolve_project_context(
        ...
        let files = match leetium_projects::context::load_context_files(Path::new(dir)) {
            Ok(f) => f,
            Err(e) => {
        ...
        let project: leetium_projects::Project = serde_json::from_value(val.clone()).ok()?;
        let worktree_dir = self
            ...
            .and_then(|_| {
                let wt_path = Path::new(dir).join(".leetium-worktrees").join(session_key);
                if wt_path.exists() {
                    Some(wt_path)
                } else {
                    None
        ...
        let ctx = leetium_projects::ProjectContext {
            project,
            context_files: files,
            worktree_dir,
        };
...
impl ChatService for LiveChatService {
    ...
    async fn send(&self, mut params: Value) -> ServiceResult {
                            ...
                            match block_type {
                                ...
                                "image_url" => {
                                    ...
                                    Some(ContentBlock::ImageUrl {
                                        image_url: leetium_sessions::message::ImageUrl {
                                            url: url.to_string(),
                                        },
                                    })
                                },
        ...
        if let Some(shell_command) = explicit_shell_command {
            ...
            let deferred_channel_target =
                params
                    ...
                    .cloned()
                    .and_then(|value| {
                        match serde_json::from_value::<leetium_channels::ChannelReplyTarget>(value) {
            ...
            let permit: OwnedSemaphorePermit = match session_sem.clone().try_acquire_owned() {
                Ok(p) => p,
                Err(_) => {
                    let queue_mode = leetium_config::discover_and_load().chat.message_queue_mode;
                    info!(
                        session = %session_key,
                        mode = ?queue_mode,
                        client_seq = ?client_seq,
        ...
        let provider: Arc<dyn leetium_agents::model::LlmProvider> = {
            let reg = self.providers.read().await;
            ...
            if self.failover_config.enabled {
                ...
                if fallbacks.is_empty() {
                    ...
                    Arc::new(leetium_agents::provider_chain::ProviderChain::new(chain))
                }
            } else {
                primary
            }
        ...
        if let Some(ref hooks) = self.hook_registry {
            let channel = params
                ...
                .and_then(|v| v.as_str())
                .map(String::from);
            let payload = leetium_common::hooks::HookPayload::MessageReceived {
        ...
        let discoverer = leetium_skills::discover::FsSkillDiscoverer::new(search_paths);
        ...
        let agent_timeout_secs = leetium_config::discover_and_load().tools.agent_timeout_secs;
    ...
    async fn compact(&self, params: Value) -> ServiceResult {
        ...
        if let Some(ref hooks) = self.hook_registry {
            let payload = leetium_common::hooks::HookPayload::BeforeCompaction {
                session_key: session_key.clone(),
            ...
            && let Ok(provider) = self.resolve_provider(&session_key, &history).await
        {
            ...
            let writer: Arc<dyn leetium_agents::memory_writer::MemoryWriter> = Arc::new(
                AgentScopedMemoryWriter::new(Arc::clone(mm), session_agent_id.clone()),
            );
            match leetium_agents::silent_turn::run_silent_memory_turn(
                provider,
        ...
        if let Some(mm) = self.state.memory_manager() {
            let memory_dir = leetium_config::agent_workspace_dir(&session_agent_id).join("memory");
        ...
        if let Some(ref hooks) = self.hook_registry {
            let payload = leetium_common::hooks::HookPayload::AfterCompaction {
                session_key: session_key.clone(),
                ...
                .await
            {
                Ok(val) => {
                    ...
                    let context_files = if let Some(d) = dir {
                        match leetium_projects::context::load_context_files(Path::new(d)) {
                            Ok(files) => files
                                .iter()
                                .map(|f| {
                                    serde_json::json!({
...
struct ChannelReplyTargetKey {
    channel_type: leetium_channels::ChannelType,
    account_id: String,
    chat_id: String,
    message_id: Option<String>,
}

impl From<&leetium_channels::ChannelReplyTarget> for ChannelReplyTargetKey {
    fn from(target: &leetium_channels::ChannelReplyTarget) -> Self {
        Self {
...
struct ChannelStreamWorker {
    sender: leetium_channels::StreamSender,
}
...
struct ChannelStreamDispatcher {
    outbound: Arc<dyn leetium_channels::plugin::ChannelStreamOutbound>,
    targets: Vec<leetium_channels::ChannelReplyTarget>,
...
impl ChannelStreamDispatcher {
    async fn for_session(state: &Arc<dyn ChatRuntime>, session_key: &str) -> Option<Self> {
        ...
        let targets: Vec<leetium_channels::ChannelReplyTarget> = state
    ...
    async fn send_delta(&mut self, delta: &str) {
        ...
        let event = leetium_channels::StreamEvent::Delta(delta.to_string());
    ...
    async fn finish(&mut self) {
        self.send_terminal(leetium_channels::StreamEvent::Done).await;
        self.join_workers().await;
    }

    async fn send_terminal(&mut self, event: leetium_channels::StreamEvent) {
...
fn resolve_agent_memory_target_path(agent_id: &str, file: &str) -> anyhow::Result<PathBuf> {
    ...
    let workspace = leetium_config::agent_workspace_dir(agent_id);
...
struct AgentScopedMemoryWriter {
    manager: Arc<leetium_memory::manager::MemoryManager>,
    agent_id: String,
}

impl AgentScopedMemoryWriter {
    fn new(manager: Arc<leetium_memory::manager::MemoryManager>, agent_id: String) -> Self {
        Self { manager, agent_id }
    }
}

#[async_trait]
impl leetium_agents::memory_writer::MemoryWriter for AgentScopedMemoryWriter {
    async fn write_memory(
        &self,
        file: &str,
        content: &str,
        append: bool,
    ) -> anyhow::Result<leetium_agents::memory_writer::MemoryWriteResult> {
        ...
        Ok(leetium_agents::memory_writer::MemoryWriteResult {
...
impl AgentTool for AgentScopedMemorySearchTool {
    ...
    async fn execute(&self, params: Value) -> anyhow::Result<Value> {
        ...
        let mut results: Vec<leetium_memory::search::SearchResult> = self
            .manager
        ...
        let include_citations = leetium_memory::search::SearchResult::should_include_citations(
            &results,
...
impl AgentTool for AgentScopedMemorySaveTool {
    ...
    async fn execute(&self, params: Value) -> anyhow::Result<Value> {
        ...
        use leetium_agents::memory_writer::MemoryWriter;
...
/// - `Off` — no tools at all
fn effective_tool_mode(provider: &dyn leetium_agents::model::LlmProvider) -> ToolMode {
...
async fn run_with_tools(
    ...
    runtime_context: Option<&PromptRuntimeContext>,
    user_message_index: usize,
    skills: &[leetium_skills::types::SkillMetadata],
    hook_registry: Option<Arc<leetium_common::hooks::HookRegistry>>,
    accept_language: Option<String>,
    conn_id: Option<String>,
...
async fn send_channel_logbook_follow_up_to_targets(
    outbound: Arc<dyn leetium_channels::plugin::ChannelOutbound>,
    targets: Vec<leetium_channels::ChannelReplyTarget>,
    logbook_html: &str,
) {
    ...
    for target in targets {
        ...
        tasks.push(tokio::spawn(async move {
            ...
            match target.channel_type {
                leetium_channels::ChannelType::Telegram => match tts_payload {
                    Some(mut payload) => {
                        ...
                        if text_already_streamed {
                            ...
                            <= leetium_telegram::markdown::TELEGRAM_CAPTION_LIMIT
                        {
...
async fn generate_tts_audio(
    ...
    let text = leetium_voice::tts::sanitize_text_for_tts(text);
    let text = text.trim();
    if text.is_empty() {
...
async fn build_tts_payload(
    state: &Arc<dyn ChatRuntime>,
    session_key: &str,
    target: &leetium_channels::ChannelReplyTarget,
    text: &str,
) -> Option<leetium_common::types::ReplyPayload> {
    use leetium_common::types::{MediaAttachment, ReplyPayload};
    ...
    let sanitized = leetium_voice::tts::sanitize_text_for_tts(text);
...
mod tests {
    use {
        super::*,
        anyhow::Result,
        leetium_agents::{model::LlmProvider, tool_registry::AgentTool},
        leetium_common::types::ReplyPayload,
        std::{
            pin::Pin,
            sync::{
                Arc,
    ...
    impl LlmProvider for StaticProvider {
        ...
        async fn complete(
            ...
            _tools: &[Value],
        ) -> Result<leetium_agents::model::CompletionResponse> {
    ...
    struct MockChatRuntime {
        channel_replies: Mutex<HashMap<String, Vec<leetium_channels::ChannelReplyTarget>>>,
        channel_status_log: Mutex<HashMap<String, Vec<String>>>,
        channel_outbound: Option<Arc<dyn leetium_channels::ChannelOutbound>>,
        channel_stream_outbound: Option<Arc<dyn leetium_channels::ChannelStreamOutbound>>,
        tts: leetium_service_traits::NoopTtsService,
        project: leetium_service_traits::NoopProjectService,
        mcp: leetium_service_traits::NoopMcpService,
    }
    ...
    impl MockChatRuntime {
        ...
        fn with_channel_outbound(
            mut self,
            outbound: Arc<dyn leetium_channels::ChannelOutbound>,
        ) -> Self {
        ...
        fn with_channel_stream_outbound(
            mut self,
            outbound: Arc<dyn leetium_channels::ChannelStreamOutbound>,
        ) -> Self {
    ...
    impl ChatRuntime for MockChatRuntime {
        ...
        async fn drain_channel_replies(
            &self,
            session_key: &str,
        ) -> Vec<leetium_channels::ChannelReplyTarget> {
            self.channel_replies
                .lock()
                .await
                .remove(session_key)
        ...
        fn sandbox_router(&self) -> Option<&Arc<leetium_tools::sandbox::SandboxRouter>> {
            None
        }

        fn memory_manager(&self) -> Option<&Arc<leetium_memory::manager::MemoryManager>> {
            None
        }

        async fn cached_location(&self) -> Option<leetium_config::GeoLocation> {
            None
        }
        ...
        fn channel_outbound(&self) -> Option<Arc<dyn leetium_channels::ChannelOutbound>> {
            self.channel_outbound.clone()
        }

        fn channel_stream_outbound(
            &self,
        ) -> Option<Arc<dyn leetium_channels::ChannelStreamOutbound>> {
            self.channel_stream_outbound.clone()
        }

        fn tts_service(&self) -> &dyn leetium_service_traits::TtsService {
            &self.tts
        }

        fn project_service(&self) -> &dyn leetium_service_traits::ProjectService {
            &self.project
        }

        fn mcp_service(&self) -> &dyn leetium_service_traits::McpService {
            &self.mcp
        }

        async fn chat_service(&self) -> Arc<dyn ChatService> {
            Arc::new(leetium_service_traits::NoopChatService)
        }
    ...
    fn resolve_channel_runtime_context_extracts_channel_binding() {
        let binding = leetium_channels::ChannelReplyTarget {
            channel_type: leetium_channels::ChannelType::Telegram,
    ...
    #[async_trait]
    impl leetium_channels::plugin::ChannelOutbound for MockChannelOutbound {
        async fn send_text(
            &self,
            _account_id: &str,
            _to: &str,
    ...
    #[async_trait]
    impl leetium_channels::plugin::ChannelOutbound for RecordingChannelOutbound {
        async fn send_text(
            &self,
            _account_id: &str,
            _to: &str,
    ...
    #[async_trait]
    impl leetium_channels::plugin::ChannelStreamOutbound for MockChannelStreamOutbound {
        async fn send_stream(
            &self,
            _account_id: &str,
            _to: &str,
            reply_to: Option<&str>,
            mut stream: leetium_channels::StreamReceiver,
        ) -> leetium_channels::Result<()> {
            if self.fail {
                return Err(leetium_channels::Error::unavailable("stream failed"));
            }
            self.reply_tos
                .lock()
                .await
            ...
            while let Some(event) = stream.recv().await {
                match event {
                    leetium_channels::StreamEvent::Delta(delta) => {
                        self.deltas.lock().await.push(delta);
                    },
                    leetium_channels::StreamEvent::Done | leetium_channels::StreamEvent::Error(_) => {
                        break;
                    },
                }
            }
    ...
    async fn sqlite_pool() -> sqlx::SqlitePool {
        ...
        leetium_projects::run_migrations(&pool)
            .await
    ...
    async fn deliver_channel_replies_waits_for_outbound_sends() {
        ...
        let outbound: Arc<dyn leetium_channels::plugin::ChannelOutbound> =
            Arc::new(MockChannelOutbound {
                calls: Arc::clone(&calls),
                delay: Duration::from_millis(50),
            });
        let targets = vec![leetium_channels::ChannelReplyTarget {
            channel_type: leetium_channels::ChannelType::Telegram,
    ...
    async fn deliver_channel_replies_skips_targets_already_streamed() {
        ...
        let target = leetium_channels::ChannelReplyTarget {
            channel_type: leetium_channels::ChannelType::Telegram,
            account_id: "acct".to_string(),
            chat_id: "123".to_string(),
    ...
    async fn deliver_channel_replies_streamed_targets_get_logbook_follow_up() {
        ...
        let outbound: Arc<dyn leetium_channels::plugin::ChannelOutbound> = outbound_impl;
        let state: Arc<dyn ChatRuntime> =
            Arc::new(MockChatRuntime::new().with_channel_outbound(outbound));

        let target = leetium_channels::ChannelReplyTarget {
            channel_type: leetium_channels::ChannelType::Discord,
            account_id: "acct".to_string(),
            chat_id: "123".to_string(),
            message_id: Some("42".to_string()),
        };
    ...
    async fn channel_stream_dispatcher_records_completed_targets() {
        ...
        let stream_outbound: Arc<dyn leetium_channels::plugin::ChannelStreamOutbound> =
    ...
    fn runtime_filters_apply_policy_without_skill_tool_restrictions() {
        ...
        let skills = vec![leetium_skills::types::SkillMetadata {
            name: "my-skill".into(),
    ...
    fn allowed_models_empty_shows_all() {
        let m = leetium_providers::ModelInfo {
            id: "anthropic::claude-opus-4-5".into(),
            provider: "anthropic".into(),
            display_name: "Claude Opus 4.5".into(),
            created_at: None,
    ...
    fn allowed_models_numeric_pattern_does_not_match_extended_variants() {
        let exact = leetium_providers::ModelInfo {
            id: "openai::gpt-5.2".into(),
            provider: "openai".into(),
            display_name: "GPT-5.2".into(),
            created_at: None,
        };
        let extended = leetium_providers::ModelInfo {
    ...
    fn allowed_models_does_not_filter_ollama_when_provider_is_aliased() {
        let aliased = leetium_providers::ModelInfo {
    ...
    async fn model_test_rejects_missing_model_id() {
        let service = LiveModelService::new(
            Arc::new(RwLock::new(ProviderRegistry::from_env_with_config(
                &leetium_config::schema::ProvidersConfig::default(),
            ))),
    ...
    fn to_user_content_multimodal_with_image() {
        use leetium_sessions::message::{ContentBlock, ImageUrl as SessionImageUrl};
    ...
    fn resolve_agent_memory_target_path_maps_to_agent_workspace() {
        let workspace = leetium_config::agent_workspace_dir("ops");
        assert_eq!(
    ...
    fn path_in_agent_memory_scope_is_isolated_per_agent() {
        let ops_workspace = leetium_config::agent_workspace_dir("ops");
        ...
        let root_memory = leetium_config::data_dir().join("memory").join("root.md");
