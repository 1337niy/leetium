...
use crate::{Result, config_dir::leetium_config_dir, types::OAuthTokens};

/// File-based token storage at `~/.config/leetium/oauth_tokens.json`.
#[derive(Debug, Clone)]
...
impl TokenStore {
    pub fn new() -> Self {
        let path = leetium_config_dir().join("oauth_tokens.json");
        Self { path }
    }
