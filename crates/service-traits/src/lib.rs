...
impl From<ServiceError> for leetium_protocol::ErrorShape {
    fn from(err: ServiceError) -> Self {
        Self::new(leetium_protocol::error_codes::UNAVAILABLE, err.to_string())
    }
}
...
impl AgentService for NoopAgentService {
    ...
    async fn identity_get(&self) -> ServiceResult {
        Ok(serde_json::json!({ "name": "leetium", "avatar": null }))
    }
