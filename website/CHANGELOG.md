...
- Machine-readable install discovery files under `/.well-known/` for agent-driven Leetium setup.
- Stable channel pointer at `/.well-known/leetium-install/channels/stable.json` and immutable per-release manifests at `/.well-known/leetium-install/releases/<version>.json`.
